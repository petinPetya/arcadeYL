...
from yookassa.domain.models.payout_data.payout_destination_class_map import (
    PayoutDestinationClassMap,
)
...
class PayoutResponse(ResponseObject):
    ...
    def payout_destination(self, value):
        ...
        if (
            ...
            and "type" in value
            and value["type"] not in PayoutDestinationClassMap().response
        ):
            value["type"] = PaymentMethodType.UNKNOWN
        self.__payout_destination = PayoutDestinationFactory().create(
            value, self.context()
        )
