...
class ProductCode:
    ...
    def calc_result(self) -> str:
        ...
        if (
            self.type == self.TYPE_EAN_8
            or self.type == self.TYPE_EAN_13
            or self.type == self.TYPE_ITF_14
        ):
    ...
    def _fill_data(self, data: Dict):
        ...
        if self.type in (
            ...
            self.set_mark_code_info(
                f"{self.AI_GTIN}{self.gtin}{self.AI_SERIAL}{self.serial}"
            )
        elif self.type in (self.TYPE_GS_1M, self.TYPE_GS_10):
            self.gtin = data["match1"]
            ...
            self.gtin = (
                data["code"][: self.MAX_PRODUCT_CODE_LENGTH]
                if len(data["code"]) > self.MAX_PRODUCT_CODE_LENGTH
                else data["code"]
            )
            self.serial = (
                data["code"][: self.MAX_MARK_CODE_LENGTH]
                if len(data["code"]) > self.MAX_MARK_CODE_LENGTH
                else data["code"]
            )
    ...
    def _base_convert(
        self, num_str: str, from_base: int = 10, to_base: int = 16
    ) -> str:
