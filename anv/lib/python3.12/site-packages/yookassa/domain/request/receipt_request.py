...
from yookassa.domain.models.receipt_data.additional_user_props import (
    AdditionalUserProps,
)
...
class ReceiptRequest(RequestObject):
    ...
    def timezone(self, value):
        ...
        if value is not None and value > 11:  # noqa: E501
            raise ValueError(
                "Invalid value for `timezone`, must be a value less than or equal to `11`"
            )  # noqa: E501
        if value is not None and value < 1:  # noqa: E501
            raise ValueError(
                "Invalid value for `timezone`, must be a value greater than or equal to `1`"
            )  # noqa: E501
        ...
        if isinstance(value, list):
            ...
            for item in value:
                if isinstance(item, dict):
                    ...
                    raise TypeError(
                        "Invalid receipt_industry_details data type in ReceiptRequest.receipt_industry_details"
                    )
            self.__receipt_industry_details = items
        else:
            raise TypeError(
                "Invalid receipt_industry_details value type in ReceiptRequest"
            )
    ...
    def receipt_operational_details(self, value):
        ...
        if isinstance(value, dict):
            ...
            raise TypeError(
                "Invalid receipt_operational_details data type in ReceiptRequest"
            )
        ...
        if isinstance(value, list):
            ...
            for item in value:
                if isinstance(item, dict):
                    ...
                    raise TypeError(
                        "Invalid settlement type in ReceiptRequest.settlements"
                    )
    ...
    def validate(self):
        ...
        if not email and not phone:
            self.__set_validation_error(
                "Both email and phone values are empty in ReceiptRequest.customer"
            )
        ...
        if (
            self.type is ReceiptType.REFUND
            and self.refund_id is None
            and self.payment_id is None
        ):
            self.__set_validation_error(
                "ReceiptRequest.refund_id or ReceiptRequest.payment_id not specified"
            )
