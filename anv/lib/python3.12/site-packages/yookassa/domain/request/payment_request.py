...
from yookassa.domain.models.payment_data.payment_order.payment_order_factory import (
    PaymentOrderFactory,
)
...
class PaymentRequest(RequestObject):
    ...
    def payment_method_data(self, value):
        ...
        """
        if isinstance(value, dict):
            self.__payment_method_data = PaymentDataFactory().create(
                value, self.context()
            )
    ...
    def merchant_customer_id(self, value):
        ...
        if cast_value:
            if len(cast_value) <= MERCHANT_CUSTOMER_MAX_LENGTH:
                ...
                raise ValueError(
                    "The value of the merchant_customer_id parameter is too long. Max length is {}".format(
                        MERCHANT_CUSTOMER_MAX_LENGTH
                    )
                )  # noqa: E501
    ...
    def statements(self, value):
        ...
        if value is None:
            ...
            for item in value:
                if isinstance(item, dict):
                    self.__statements.append(
                        StatementFactory().create(item, self.context())
                    )
            ...
            raise TypeError(
                "Invalid statements data type in payment_request.statements"
            )
    ...
    def validate(self):
        ...
        if amount.value <= 0.0:
            self.__set_validation_error(
                "Invalid payment amount value: " + str(amount.value)
            )

        if self.receipt is not None and self.receipt.has_items:
            if self.receipt.customer is None:
                self.__set_validation_error(
                    "Customer is empty in PaymentRequest.receipt"
                )
            ...
            if not email and not phone:
                self.__set_validation_error(
                    "Both email and phone values are empty in PaymentRequest.receipt.customer"
                )
            ...
            if self.receipt.tax_system_code is None:
                for item in self.receipt.items:
                    if item.vat_code is None:
                        self.__set_validation_error(
                            "Item vat_code and receipt tax_system_id not specified"
                        )

        if self.payment_token:
            if self.payment_method_id:
                self.__set_validation_error(
                    "Both payment_token and payment_method_id values are specified"
                )

            if self.payment_method_data:
                self.__set_validation_error(
                    "Both payment_token and payment_data values are specified"
                )

        elif self.payment_method_id:
            if self.payment_method_data:
                self.__set_validation_error(
                    "Both payment_method_id and payment_data values are specified"
                )

        if (
            self.payment_method_data
            and isinstance(self.payment_method_data, PaymentDataBankCard)
