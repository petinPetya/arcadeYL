...
from yookassa.domain.models.invoice_data.delivery_method_factory import (
...
class InvoiceRequest(RequestObject):
    ...
    def payment_data(self, value):
        ...
        if isinstance(value, dict):
            ...
            raise TypeError(
                "Invalid payment_data data type in InvoiceRequest.payment_data"
            )
    ...
    def cart(self, value):
        ...
        if value is None:  # noqa: E501
            raise ValueError(
                "Invalid value for `cart`, must not be `None`"
            )  # noqa: E501
    ...
    def delivery_method_data(self, value):
        ...
        """
        if isinstance(value, dict):
            self.__delivery_method_data = DeliveryMethodFactory().create(
                value, self.context()
            )
            ...
            raise TypeError(
                "Invalid delivery_method_data data type in InvoiceRequest.delivery_method_data"
            )
    ...
    def description(self, value):
        ...
        if value is not None and len(value) > DESCRIPTION_MAX_LENGTH:
            raise ValueError(
                "Invalid value for `description`, length must be less than or equal to `{}`".format(
    ...
    def validate(self):
        ...
        if (
            self.payment_data.receipt is not None
            and self.payment_data.receipt.has_items
        ):
            if self.payment_data.receipt.customer is None:
                self.__set_validation_error(
                    "Customer is empty in InvoiceRequest.payment_data.receipt"
                )
            ...
            if not email and not phone:
                self.__set_validation_error(
                    "Both email and phone values are empty in InvoiceRequest.payment_data.receipt.customer"
                )
            ...
            if self.payment_data.receipt.tax_system_code is None:
                for item in self.payment_data.receipt.items:
                    if item.vat_code is None:
                        self.__set_validation_error(
                            "Item vat_code and receipt tax_system_id not specified in InvoiceRequest.payment_data.receipt"
                        )
