...
class RefundRequest(RequestObject):
    ...
    def refund_method_data(self, value):
        ...
        """
        if isinstance(value, dict):
            self.__refund_method_data = RefundDataFactory().create(
                value, self.context()
            )
            ...
            raise TypeError(
                "Invalid refund_method_data data type in RefundRequest.refund_method_data"
            )
    ...
    def validate(self):
        ...
        if self.receipt is not None and self.receipt.has_items:
            ...
            if not email and not phone:
                self.__set_validation_error(
                    "Both email and phone values are empty in RefundRequest.receipt.customer"
                )
