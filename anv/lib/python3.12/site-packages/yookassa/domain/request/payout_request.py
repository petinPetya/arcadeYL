...
from yookassa.domain.models.payout_data.payout_destination_factory import (
    PayoutDestinationFactory,
)
...
class PayoutRequest(RequestObject):
    ...
    def payout_destination_data(self, value):
        ...
        """
        if isinstance(value, dict):
            self.__payout_destination_data = PayoutDestinationFactory().create(
                value, self.context()
            )
    ...
    def description(self, value):
        ...
        if len(cast_value) <= DESCRIPTION_MAX_LENGTH:
            ...
            raise ValueError(
                "The value of the description parameter is too long. Max length is {}".format(
    ...
    def self_employed(self, value):
        ...
        if isinstance(value, dict):
            ...
            raise TypeError(
                "Invalid self_employed data type in PayoutRequest.self_employed"
            )
    ...
    def receipt_data(self, value):
        ...
        if isinstance(value, dict):
            ...
            raise TypeError(
                "Invalid receipt_data data type in PayoutRequest.receipt_data"
            )
    ...
    def personal_data(self, value):
        ...
        if isinstance(value, list):
            if len(value) > 2:
                raise ValueError(
                    "Invalid value for `personal_data`, number of items must be less than or equal to `2`"
                )  # noqa: E501
            if len(value) < 1:
                raise ValueError(
                    "Invalid value for `personal_data`, number of items must be greater than or equal to `1`"
                )  # noqa: E501
            ...
            for personal_dataData in value:
                if isinstance(personal_dataData, dict):
                    ...
                    raise TypeError(
                        "Invalid personal_data data type in PayoutRequest.personal_data"
                    )
    ...
    def validate(self):
        ...
        if amount.value <= 0.0:
            self.__set_validation_error(
                "Invalid payout amount value: " + str(amount.value)
            )

        if (
            (self.payout_token and self.payout_destination_data)
            or (self.payout_token and self.payment_method_id)
            or (self.payout_destination_data and self.payment_method_id)
        ):
            self.__set_validation_error(
                "Both payout_token, payout_destination_data and payment_method_id values are specified"
            )

        elif (
            self.payout_token is None
            and self.payout_destination_data is None
            and self.payment_method_id is None
        ):
            self.__set_validation_error(
                "Both payout_token, payout_destination_data and payment_method_id values are not specified"
            )
