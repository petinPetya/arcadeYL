...
from yookassa.domain.models.self_employed_data.confirmation import (
    SelfEmployedConfirmation,
)
from yookassa.domain.models.self_employed_data.confirmation_factory import (
...
class SelfEmployedRequest(RequestObject):
    ...
    def confirmation(self, value):
        ...
        if isinstance(value, dict):
            self.__confirmation = SelfEmployedConfirmationFactory().create(
                value, self.context()
            )
            ...
            raise TypeError(
                "Invalid confirmation data type in SelfEmployedRequest.confirmation"
            )
    ...
    def validate(self):
        ...
        if self.itn is None and self.phone is None:
            self.__set_validation_error(
                "Both itn and phone values are empty in self_employed_request"
            )
