...
class PayoutDestinationSbp(PayoutDestination):
    ...
    def bank_id(self, value):
        ...
        if value is not None and len(value) > 12:
            raise ValueError(
                "Invalid value for `bank_id`, length must be less than or equal to `12`"
            )  # noqa: E501
        if value is not None and not re.search(
            r"^[a-zA-Z0-9]{12}$", value
        ):  # noqa: E501
            raise ValueError(
                r"Invalid value for `bank_id`, must be a follow pattern or equal to `/[a-zA-Z0-9]{12}/`"
            )  # noqa: E501
