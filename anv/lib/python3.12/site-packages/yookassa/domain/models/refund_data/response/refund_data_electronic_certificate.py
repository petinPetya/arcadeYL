...
from yookassa.domain.models.refund_data.electronic_certificate_refund_article import (
...
class RefundDataElectronicCertificate(ResponseRefundData):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            self.type is None
            or self.type is not PaymentMethodType.ELECTRONIC_CERTIFICATE
        ):
    ...
    def electronic_certificate(self, value):
        ...
        if isinstance(value, dict):
            self.__electronic_certificate = ElectronicCertificateRefundDataResponse(
                value
            )
        elif isinstance(value, ElectronicCertificateRefundDataResponse):
            self.__electronic_certificate = value
        else:
            raise TypeError(
                "Invalid electronic_certificate data type in RefundDataElectronicCertificate.electronic_certificate"
            )
            ...
            for articlesData in value:
                if isinstance(articlesData, dict):
                    articles_array.append(
                        ElectronicCertificateRefundArticle(articlesData)
                    )
                elif isinstance(articlesData, ElectronicCertificateRefundArticle):
                    articles_array.append(articlesData)
                else:
                    raise TypeError(
                        "Invalid articles data type in RefundDataElectronicCertificate.articles"
                    )

            self.__articles = articles_array
        else:
            raise TypeError(
                "Invalid articles value type in RefundDataElectronicCertificate"
            )
