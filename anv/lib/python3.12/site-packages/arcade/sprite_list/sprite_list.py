...
class SpriteList(SpriteSequence[SpriteType]):
    ...
    def append(self, sprite: SpriteType) -> None:
        ...
        if self._initialized:
            if sprite.texture is None:
                raise ValueError(
                    "Sprite must have a texture when added to a SpriteList"
                )
    ...
    def enable_spatial_hashing(self, spatial_hash_cell_size: int = 128) -> None:
        ...
        """
        if (
            self.spatial_hash is None
            or self.spatial_hash.cell_size != spatial_hash_cell_size
        ):
            from .spatial_hash import SpatialHash
    ...
    def draw(
        ...
        *,
        ...
        if (
            len(self.sprite_list) == 0
            or not self._visible
            or self.alpha_normalized == 0.0
        ):
            return
...
class SpriteListData:
    ...
    def get_nearby_sprite_indices(
        self, pos: Point, size: Point, length: int
    ) -> list[int]:
        """
...
class SpriteListBufferData(SpriteListData):
    ...
    def __init__(
        self, ctx: ArcadeContext, capacity: int, atlas: TextureAtlasBase
    ) -> None:
        self.ctx = ctx
        self._buf_capacity = capacity
        self._idx_capacity = capacity
    ...
    def get_nearby_sprite_indices(
        ...
        if emit_count == 0:
            return []
        return [
            i for i in struct.unpack(f"{emit_count}i", buffer.read(size=emit_count * 4))
        ]
