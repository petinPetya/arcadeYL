...
class BasicSprite:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._scale = (
            (scale, scale) if isinstance(scale, float | int) else (scale[0], scale[1])
        )  # noqa: UP038
    ...
    def color(self, color: RGBOrA255):
        ...
        if _a:
            if len(_a) > 1:
                raise ValueError(
                    f"iterable must unpack to 3 or 4 values not {len(color)}"
                )
            a = _a[0]
        else:
            a = self._color.a
    ...
    def rescale_relative_to_point(
        self, point: Point2, scale_by: AsFloat | Point2
    ) -> None:
    ...
    def register_sprite_list(
        self: SpriteType, new_list: SpriteList[SpriteType]
    ) -> None:
        """
    ...
    def draw_hit_box(
        self, color: RGBOrA255 = BLACK, line_thickness: float = 2.0
    ) -> None:
        """
    ...
    def collides_with_list(
        self, sprite_list: SpriteList[SpriteType]
    ) -> list[SpriteType]:
