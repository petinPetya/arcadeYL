...
class PymunkPhysicsObject:
    ...
    def __init__(
        self, body: pymunk.Body | None = None, shape: pymunk.Shape | None = None
    ):
...
class PymunkPhysicsEngine:
    ...
    * :py:meth:`set_position`
    ...
    def __init__(
        ...
        gravity=(0, 0),
        damping: float = 1.0,
        maximum_incline_on_ground: float = 0.708,
    ) -> None:
        # -- Pymunk
    ...
    def add_sprite(
        ...
        if damping is not None:
            sprite.pymunk.damping = (
                damping  # pyright: ignore [reportGeneralTypeIssues=false]
            )

        if gravity is not None:
            sprite.pymunk.gravity = (
                gravity  # pyright: ignore [reportGeneralTypeIssues=false]
            )
        ...
        if max_vertical_velocity is not None:
            sprite.pymunk.max_vertical_velocity = (
                max_vertical_velocity  # pyright: ignore
            )

        if max_horizontal_velocity is not None:
            sprite.pymunk.max_horizontal_velocity = (
                max_horizontal_velocity  # pyright: ignore
            )
        ...
        if moment_of_inertia is None:
            moment_of_inertia = pymunk.moment_for_box(
                mass, (sprite.width, sprite.height)
            )
    ...
    def set_position(
        self, sprite: Sprite, position: pymunk.Vec2d | tuple[float, float]
    ):
    ...
    def add_collision_handler(
        ...
        def _f1(arbiter, space, data):
            ...
            if (
                sprite_a is not None
                and sprite_b is not None
                and begin_handler is not None
            ):
                should_process_collision = begin_handler(
