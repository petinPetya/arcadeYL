...
class UIScrollBar(UIWidget):
    ...
    def _scroll_bar_size(self):
        ...
        content_size = (
            self.scroll_area.surface.height
            if self.vertical
            else self.scroll_area.surface.width
        )
        view_size = (
            self.scroll_area.content_height
            if self.vertical
            else self.scroll_area.content_width
        )
    ...
    def _thumb_rect(self):
        ...
        if self.vertical:
            ...
            return XYWH(
                scroll_bar_x, scroll_bar_y, self.content_width, self._scroll_bar_size()
            )

        else:
            scroll_bar_x = (
                self._scroll_bar_size() / 2 + available_track_size * scroll_progress
            )
            scroll_bar_y = self.content_height / 2
            return XYWH(
                scroll_bar_x, scroll_bar_y, self._scroll_bar_size(), self.content_height
            )
...
class UIScrollArea(UILayout):
    ...
    def do_render(self, surface: Surface):
        ...
        self.surface.draw(
            LBWH(-offset_x, -offset_y, self.content_width, self.content_height)
        )
    ...
    def on_event(self, event: UIEvent) -> bool | None:
        ...
        if isinstance(event, UIMouseDragEvent) and not self.rect.point_in_rect(
            event.pos
        ):
            return EVENT_UNHANDLED
        ...
        if isinstance(event, UIMouseScrollEvent) and self.rect.point_in_rect(event.pos):
            ...
            if not self.overscroll_x:
                ...
                scroll_range = min(
                    0, scroll_range
                )  # clip to 0 if content is smaller than surface
