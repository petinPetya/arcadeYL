        ...
        if self._bg_color:
            if self._bg_color.a == 255 or self._strong_background:
                ...
                arcade.draw_rect_filled(
                    LBWH(0, 0, self.width, self.height), color=self._bg_color
                )
...
class UIInteractiveWidget(UIWidget):
    ...
    def on_event(self, event: UIEvent) -> bool | None:
        ...
        if (
            ...
            if self.rect.point_in_rect(event.pos):
                if not self.disabled:
                    ...
                    return (
                        EVENT_HANDLED  # TODO should we return the result from on_click?
                    )
        ...
        if self.focused:
            ...
            if isinstance(event, UIControllerButtonPressEvent) and event.button in (
                "a",
            ):
                self.pressed = True
                self._grap_active()  # make this the active widget
            ...
            if self.pressed:
                keyboard_interaction = (
                    ...
                    and event.symbol == arcade.key.SPACE
                )
