...
class UIBoxLayout(UILayout):
    ...
    def _update_size_hints(self):
        ...
        min_child_sizes = [
            UILayout.min_size_of(child)
            for child in self.children
            if child.visible is not None
        ]
    ...
    def do_layout(self):
        ...
        constraints = [
            (
                _C.from_widget_height(child)
                if self.vertical
                else _C.from_widget_width(child)
            )
        ...
        constraints = [
            (
                _C.from_widget_width(child)
                if self.vertical
                else _C.from_widget_height(child)
            )
            for child, _ in children_to_render
        ]
    ...
    * the minimum ``size_hint_max`` of its widgets
    ...
    def do_layout(self):
        ...
        row_sizes = _box_axis_algorithm(
            row_constraints,
            self.content_height - (self.row_count - 1) * self._vertical_spacing,
        )
...
def _box_orthogonal_algorithm(
    constraints: list[_C], container_size: float
) -> list[float]:
    ...
    for c in constraints:
        ...
        c._final_size = min(
            max(c.min, size), c.max
        )  # clamp width to min and max values
...
def _box_axis_algorithm(constraints: list[_C], container_size: float) -> list[float]:
    ...
    if total_hint > 1:
        ...
        total_adjustable_hint = sum(
            c.hint - c.min / container_size for c in constraints
        )
        ...
        if total_adjustable_hint > 0:
            ...
            for c in constraints:
                ...
                c.hint -= possible_adjustment * (
                    adjustable_size / total_adjustable_hint
                )
