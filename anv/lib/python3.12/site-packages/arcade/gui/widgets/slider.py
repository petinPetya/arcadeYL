...
class UIBaseSlider(UIInteractiveWidget, metaclass=ABCMeta):
    ...
    def _x_for_value(self, value: float):
        ...
        return (
            x + self._cursor_width + val * (self.content_width - 2 * self._cursor_width)
        )
    ...
    def norm_value(self, value):
        """Normalized value between 0.0 and 1.0"""
        new_value = min(
            value * (self.max_value - self.min_value) + self.min_value, self.max_value
        )
    ...
    def _thumb_x(self, nx):
        ...
        x = min(
            rect.right - self._cursor_width, max(nx, rect.left + self._cursor_width)
        )
        if self.width == 0:
            self.norm_value = 0
        else:
