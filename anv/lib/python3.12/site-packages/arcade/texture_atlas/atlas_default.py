...
class DefaultTextureAtlas(TextureAtlasBase):
    ...
    def _allocate_texture(self, texture: Texture) -> tuple[int, AtlasRegion]:
        ...
        texture_region.texture_coordinates = (
            Transform.transform_texture_coordinates_order(
                texture_region.texture_coordinates, texture._vertex_order
            )
        )
        self._texture_regions[texture.atlas_name] = (
            texture_region  # add or update region
        )
    ...
    def write_image(self, image: PIL.Image.Image, x: int, y: int) -> None:
        ...
        if self._border > 0:
            ...
            if self._border > 1:
                strip_top = strip_top.resize(
                    (image.width, self._border), Resampling.NEAREST
                )
                strip_bottom = strip_bottom.resize(
                    (image.width, self._border), Resampling.NEAREST
                )
                strip_left = strip_left.resize(
                    (self._border, image.height), Resampling.NEAREST
                )
                strip_right = strip_right.resize(
                    (self._border, image.height), Resampling.NEAREST
                )
