...
class Context(ABC):
    ...
    def __init__(
        ...
        # States
        self._blend_func: Tuple[int, int] | Tuple[int, int, int, int] = (
            self.BLEND_DEFAULT
        )
    ...
    def _create_default_framebuffer(self) -> DefaultFrameBuffer:
        raise NotImplementedError(
            "The enabled graphics backend does not support this method."
        )
    ...
    @abstractmethod
    def compute_shader(
        self, *, source: str, common: Iterable[str] = ()
    ) -> ComputeShader:
        """
...
class Info(ABC):
    ...
    def __init__(self, ctx):
        ...
        self.UNIFORM_BUFFER_OFFSET_ALIGNMENT = self.get(
            enums.UNIFORM_BUFFER_OFFSET_ALIGNMENT
        )
        ...
        self.MAX_COMBINED_TEXTURE_IMAGE_UNITS = self.get(
            enums.MAX_COMBINED_TEXTURE_IMAGE_UNITS
        )
        """
        ...
        self.MAX_VERTEX_TEXTURE_IMAGE_UNITS = self.get(
            enums.MAX_VERTEX_TEXTURE_IMAGE_UNITS
        )
        """
        Maximum supported texture image units that can be used to access texture
        maps from the vertex shader.
        ...
        self.MAX_VERTEX_UNIFORM_COMPONENTS = self.get(
            enums.MAX_VERTEX_UNIFORM_COMPONENTS
        )
        """
        ...
        self.MAX_TEXTURE_MAX_ANISOTROPY = self.get_float(
            enums.MAX_TEXTURE_MAX_ANISOTROPY, 1.0
        )
        """The highest supported anisotropy value. Usually 8.0 or 16.0."""

        self.MAX_VIEWPORT_DIMS: Tuple[int, int] = self.get_int_tuple(
            enums.MAX_VIEWPORT_DIMS, 2
        )
        """
