...
class OpenGLVertexArray(VertexArray):
    ...
    def render_indirect(
        self, buffer: Buffer, mode: GLuintLike, count, first, stride
    ) -> None:
        """
        ...
        if stride % 4 != 0 or stride < 0:
            raise ValueError(
                f"stride must be positive integer in multiples of 4, not {stride}."
            )
    ...
    def transform_interleaved(
        ...
        if self._ibo is not None:
            ...
            gl.glDrawElementsInstanced(
                mode, vertices or count, gl.GL_UNSIGNED_INT, None, instances
            )
        else:
