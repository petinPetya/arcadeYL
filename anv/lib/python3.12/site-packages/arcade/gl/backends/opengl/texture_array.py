...
class OpenGLTextureArray(TextureArray):
    ...
    def __init__(
        ...
        *,
        ...
        self._target = (
            ...
            if self._samples == 0
            else gl.GL_TEXTURE_2D_MULTISAMPLE_ARRAY
        )
    ...
    def _texture_2d_array(self, data):
        ...
        if data is not None:
            ...
            self._validate_data_size(
                data, byte_length, self._width, self._height, self._layers
            )
    ...
    def anisotropy(self, value):
        self._anisotropy = max(
            1.0, min(value, self._ctx.info.MAX_TEXTURE_MAX_ANISOTROPY)
        )
        gl.glActiveTexture(gl.GL_TEXTURE0 + self._ctx.default_texture_unit)
        gl.glBindTexture(self._target, self._glo)
    ...
    def write(
        self, data: BufferOrBufferProtocol, level: int = 0, viewport=None
    ) -> None:
        ...
        if isinstance(data, Buffer):
            ...
            gl.glTexSubImage3D(
                self._target, level, x, y, w, h, l, self._format, self._type, 0
            )
    ...
    def bind_to_image(
        self, unit: int, read: bool = True, write: bool = True, level: int = 0
    ):
        """
        ...
        if self._ctx.gl_api == "opengles" and not self._immutable:
            raise ValueError(
                "Textures bound to image units must be created with immutable=True"
            )
        ...
        if read and write:
            ...
            raise ValueError(
                "Illegal access mode. The texture must at least be read or write only"
            )

        gl.glBindImageTexture(
            unit, self._glo, level, 0, 0, access, self._internal_format
        )
