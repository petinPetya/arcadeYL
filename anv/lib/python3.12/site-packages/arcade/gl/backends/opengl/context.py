...
class OpenGLContext(Context):
    ...
    def __init__(
        ...
        if self.gl_api == "opengl":
            ...
            self._ext_separate_shader_objects_enabled = (
                self.gl_version >= (4, 1) or have_ext
            )
    ...
    def blend_func(self, value: Tuple[int, int] | Tuple[int, int, int, int]):
        ...
        if len(value) == 2:
            ...
            raise ValueError(
                f"blend_func takes a tuple of 2 or 4 values, got {len(value)}"
            )
    ...
    def program(
        ...
        *,
        ...
        source_te = (
            ShaderSource(
                self, tess_evaluation_shader, common, gl.GL_TESS_EVALUATION_SHADER
            )
        ...
        out_attributes = (
            list(varyings) if varyings is not None else []
        )  # type: List[str]
        ...
        return OpenGLProgram(
            ...
            fragment_shader=(
                source_fs.get_source(defines=defines) if source_fs else None
            ),
            geometry_shader=(
                source_geo.get_source(defines=defines) if source_geo else None
            ),
            tess_control_shader=(
                source_tc.get_source(defines=defines) if source_tc else None
            ),
            tess_evaluation_shader=(
                source_te.get_source(defines=defines) if source_te else None
            ),
