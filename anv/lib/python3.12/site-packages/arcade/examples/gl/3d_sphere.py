...
class Sphere3D(arcade.Window):
    ...
    def on_draw(self):
        ...
        self.sphere.render(
            self.program, vertices=int(self.sphere.num_vertices * self.vert_count)
        )
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.ESCAPE:
            ...
            self.text_depth.text = (
                f"F1: Toggle depth test ({self.ctx.DEPTH_TEST in self.flags})"
            )
            ...
            self.text_cull.text = (
                f"F2: Toggle cull face ({self.ctx.CULL_FACE in self.flags})"
            )
