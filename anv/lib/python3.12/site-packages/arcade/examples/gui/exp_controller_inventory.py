...
class EquipmentUI(UIBoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.head_slot.on_click = lambda _: self.dispatch_event(
            "on_slot_clicked", self.head_slot
        )

        self.chest_slot = self.add(EquipmentSlotUI(equipment, 1))
        self.chest_slot.on_click = lambda _: self.dispatch_event(
            "on_slot_clicked", self.chest_slot
        )

        self.legs_slot = self.add(EquipmentSlotUI(equipment, 2))
        self.legs_slot.on_click = lambda _: self.dispatch_event(
            "on_slot_clicked", self.legs_slot
        )
...
class InventoryModal(ActiveSlotTrackerMixin, UIFocusGroup, UIAnchorLayout):
    def __init__(self, inventory: Inventory, **kwargs):
        ...
        self.add(
            UILabel(
                text="Inventory", font_size=20, font_name="Kenney Blocks", bold=True
            ),
            anchor_y="top",
        )
...
class MyView(UIView, ControllerView):
    def __init__(self):
        ...
        text = self.root.add(
            UILabel(
                text="Open Inventory with 'Select' button on a controller or 'I' key",
                font_size=24,
            )
        )
        text.fit_content()
...
if __name__ == "__main__":
    ...
    ControllerWindow(
        title="Minimal example", width=1280, height=720, resizable=True
    ).show_view(MyView())
    arcade.run()
