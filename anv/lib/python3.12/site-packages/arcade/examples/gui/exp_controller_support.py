...
class ControllerIndicator(UIAnchorLayout):
    ...
    def __init__(self):
        ...
        self._indicator = self.add(
            UIImage(texture=self.BLANK_TEX), anchor_y="bottom", align_y=10
        )
    ...
    def input_prompts(cls, event: UIControllerEvent) -> Texture | None:
        if isinstance(event, UIControllerButtonEvent):
            match event.button:
                ...
                case "start":
                    return cls.get_texture(
                        ":resources:input_prompt/xbox/button_start.png"
                    )
                case "back":
                    return cls.get_texture(
                        ":resources:input_prompt/xbox/button_back.png"
                    )
        ...
        if isinstance(event, UIControllerDpadEvent):
            match event.vector:
                case (1, 0):
                    return cls.get_texture(
                        ":resources:input_prompt/xbox/dpad_right.png"
                    )
                case (-1, 0):
        ...
        if isinstance(event, UIControllerStickEvent) and event.vector.length() > 0.2:
            ...
            if 0.785 > heading > -0.785:
                return cls.get_texture(
                    f":resources:input_prompt/xbox/stick_{stick}_right.png"
                )
            elif 0.785 < heading < 2.356:
                return cls.get_texture(
                    f":resources:input_prompt/xbox/stick_{stick}_up.png"
                )
            elif heading > 2.356 or heading < -2.356:
                return cls.get_texture(
                    f":resources:input_prompt/xbox/stick_{stick}_left.png"
                )
            elif -2.356 < heading < -0.785:
                return cls.get_texture(
                    f":resources:input_prompt/xbox/stick_{stick}_down.png"
                )
...
class MyView(ControllerView, UIView):
    def __init__(self):
        ...
        box.add(
            UIDropdown(default="Option 1", options=["Option 1", "Option 2", "Option 3"])
        )
