...
class DepthOfField:
    ...
    def __init__(
        ...
        self._blur_process = GaussianBlur(
            size, kernel_size=10, sigma=2.0, multiplier=2.0, step=4
        )
    ...
    def process(self):
        self._blurred = self._blur_process.render(
            self._render_target.color_attachments[0]
        )
        self._win.use()
...
class GameView(View):
    ...
    def on_update(self, delta_time: float):
        ...
        raw_focus = self.focus_range * (
            cos(pi * self.focus_change_speed * time) * 0.5 + 0.5
        )
        self.dof.render_program["focus_depth"] = raw_focus / self.focus_range
        self.indicator_label.value = (
            f"Focus depth: {raw_focus:.3f} / {self.focus_range}"
        )
