...
class MultiTextureSpriteGroup(pyglet.graphics.Group):
    ...
    def __eq__(self, other: object | Group | MultiTextureSpriteGroup) -> bool:
        ...
        for name, texture in self._textures.items():
            ...
            if (
                texture.id != other._textures[name].id
                or texture.target != other._textures[name].target
            ):
                return False

        # Made it this far so just check the remainder
        return (
            self.program is other.program
    ...
    def __hash__(self) -> int:
        return hash(
            (self.parent, self.program, self.blend_src, self.blend_dest)
...
_SAMPLER_COORDS = {pyglet.gl.GL_TEXTURE_2D: ".xy", pyglet.gl.GL_TEXTURE_2D_ARRAY: ""}
...
def _get_default_mt_shader(images: dict[str, Texture]) -> ShaderProgram:
    ...
    assert (
        len(images) <= max_tex.value
    ), f"Only {max_tex.value} Texture Units are available."

    # Generate the default vertex shader
    in_tex_coords = "\n".join([f"in vec3 {name}_coords;" for name in images.keys()])
    out_tex_coords = "\n".join(
        [f"out vec3 {name}_coords_frag;" for name in images.keys()]
    )
    tex_coords_assignments = "\n".join(
        [f"{name}_coords_frag = {name}_coords;" for name in images.keys()]
    )
    ...
    in_tex_coords = "\n".join(
        [f"in vec3 {name}_coords_frag;" for name in images.keys()]
    )
    uniform_samplers = "\n".join(
        [
            f"uniform {_SAMPLER_TYPES[tex.target]} {name};"
            for name, tex in images.items()
        ]
    )
    tex_operations = "\n".join(
        [
            f"  color = layer(texture({name}, {name}_coords_frag{_SAMPLER_COORDS[tex.target]}), color);"
            for name, tex in images.items()
        ]
    )
    fragment_source = f"""
    ...
    * The fragment shader is expected to have samplers of the appropriate type
    ...
    def __init__(
        ...
        for name, img in images.items():
            ...
            if not self._texture or (
                self._textures[name].height * self._textures[name].width
            ) > (self._texture.height * self._texture.width):
        ...
        for name, img in images.items():
            if isinstance(img, pyglet.image.Animation):
                ...
                self._animations[name] = {
                    "animation": img,
                    "frame_idx": 0,
                    "next_dt": img.frames[0].duration,
                }
                if img.frames[0].duration:
                    pyglet.clock.schedule_once(
                        self._animate, self._animations[name]["next_dt"], name
                    )
    ...
    def _set_multi_texture(self, key, new_tex: Texture) -> None:
        if new_tex.id is not self._textures[key].id:
            ...
            self._textures[key] = new_tex
            getattr(self._vertex_list, f"{key}_coords")[:] = self._textures[
                key
            ].tex_coords
    ...
    def frame_index(self) -> None:
        raise NotImplementedError(
            "MultiTextureSprite does not support the frame_index property.  Use get_frame_index instead."
        )

    @frame_index.setter
    def frame_index(self, index: int) -> None:
        raise NotImplementedError(
            "MultiTextureSprite does not support the frame_index property.  Use set_frame_index instead."
        )

    @property
    def image(self) -> None:
        raise NotImplementedError(
            "MultiTextureSprite does not support the image property.  Use get_layer instead."
        )

    @image.setter
    def image(self, img: AbstractImage | Animation) -> None:
        raise NotImplementedError(
            "MultiTextureSprite does not support the image property.  Use set_layer instead."
        )
