...
class Emitter(event.EventDispatcher):
    ...
    def __init__(
        ...
        self._group = self.group_class(
            self._texture, blend_src, blend_dest, self.program, group
        )
    ...
    def _create_vertex_list(self):
        ...
        self._vertex_list = self.program.vertex_list(
            ...
            size=(
                "f",
                (texture.width, texture.height, texture.anchor_x, texture.anchor_y)
                * count,
            ),
    ...
    def program(self, program):
        ...
        if self._batch and self._batch.update_shader(
            self._vertex_list, GL_POINTS, self._group, program
        ):
...
class ParticleManager:
    ...
    def __init__(
        ...
        # TODO: remove debug
        self.total_number = 0
        self.total_label = pyglet.text.Label(
            "particles: 0", 10, 10, dpi=256, color=(10, 200, 10), batch=batch
        )
    ...
    def create_emitter(self, x, y, z=0):
        emitter = Emitter(
            ...
            scale_start=self._scale_start,
            scale_end=self._scale_end,
            batch=self._batch,
            group=self._group,
        )
