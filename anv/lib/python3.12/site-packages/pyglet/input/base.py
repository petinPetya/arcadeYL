...
class Control(EventDispatcher):
    ...
    def __init__(
        self, name: None | str, raw_name: None | str = None, inverted: bool = False
    ):
    ...
    def __repr__(self) -> str:
        if self.name:
            return (
                f"{self.__class__.__name__}(name={self.name}, raw_name={self.raw_name})"
            )
        else:
    ...
    def __init__(self, device):
        ...
        def add_axis(control: AbsoluteAxis):
            if not (control.min or control.max):
                warnings.warn(
                    f"Control('{control.name}') min & max values are both 0. Skipping."
                )
                return
            name = control.name
        ...
        def add_hat(control: AbsoluteAxis):
            ...
            _input_map = {
                key: val
                for key, val in zip(
                    range(int(control.min), int(control.max + 1)), _vecs
                )
            }
...
class Controller(EventDispatcher):
    ...
    def _bind_axis_control(
        self, relation: Relation, control: AbsoluteAxis, axis_name: str
    ) -> None:
        if not (control.min or control.max):
            warnings.warn(
                f"Control('{control.name}') min & max values are both 0. Skipping."
        ...
        dpad_defaults = {
            "dpup": Sign.POSITIVE,
            "dpdown": Sign.NEGATIVE,
            "dpleft": Sign.NEGATIVE,
            "dpright": Sign.POSITIVE,
        }
        ...
        if axis_name in ("dpup", "dpdown"):
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_dpad_motion", self, Vec2(self.dpadx, self.dpady)
                )
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_trigger_motion", self, axis_name, normalized_value
                )
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_stick_motion", self, "leftstick", self.left_analog
                )
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_stick_motion", self, "rightstick", self.right_analog
                )

    def _bind_button_control(
        self, relation: Relation, control: Button, button_name: str
    ) -> None:
    ...
    def _bind_dedicated_hat(self, relation: Relation, control: AbsoluteAxis) -> None:
        ...
        _input_map = {
            key: val
            for key, val in zip(range(int(control.min), int(control.max + 1)), _vecs)
        }
            ...
            try:
                if relation.control_type == "button":
                    self._bind_button_control(
                        relation, self._button_controls[relation.index], name
                    )

                elif relation.control_type == "axis":
                    self._bind_axis_control(
                        relation, self._axis_controls[relation.index], name
                    )
                ...
                warnings.warn(
                    f"Could not map physical Control '{relation}' to '{name}'"
                )
                continue
