...
iokit.IOHIDManagerRegisterDeviceRemovalCallback.argtypes = [
    c_void_p,
    c_void_p,
    c_void_p,
]
...
class HIDDevice:
    ...
    def schedule_with_run_loop(self):
        iokit.IOHIDDeviceScheduleWithRunLoop(
            self.device_ref, c_void_p(cf.CFRunLoopGetCurrent()), kCFRunLoopDefaultMode
        )
    ...
    def _get_elements(self):
        cfarray = c_void_p(
            iokit.IOHIDDeviceCopyMatchingElements(self.device_ref, None, 0)
        )
        if not cfarray:
    ...
    def _register_input_value_callback(self):
        ...
        iokit.IOHIDDeviceRegisterInputValueCallback(
            self.device_ref, value_callback, None
        )
    ...
    def get_value(self, element):
        ...
        iokit.IOHIDDeviceGetValue(
            self.device_ref, element.element_ref, byref(value_ref)
        )
        if value_ref:
...
class HIDManager(EventDispatcher):
    def __init__(self):
        ...
        self.manager_ref = c_void_p(
            iokit.IOHIDManagerCreate(None, kIOHIDOptionsTypeNone)
        )
    ...
    def schedule_with_run_loop(self):
        iokit.IOHIDManagerScheduleWithRunLoop(
            self.manager_ref, c_void_p(cf.CFRunLoopGetCurrent()), kCFRunLoopDefaultMode
        )
    ...
    def _register_matching_callback(self):
        ...
        iokit.IOHIDManagerRegisterDeviceMatchingCallback(
            self.manager_ref, matching_callback, None
        )
    ...
    def _register_removal_callback(self):
        ...
        iokit.IOHIDManagerRegisterDeviceRemovalCallback(
            self.manager_ref, removal_callback, None
        )
    ...
    def _create_controls(self):
        ...
        for element in self.device.elements:
            ...
            if element.type in (
                ...
                if (
                    self._axis_filter is not None
                    and element.usagePage not in self._axis_filter
                ):
                    continue
                ...
                if element.isRelative:
                    ...
                    control = AbsoluteAxis(
                        name, element.logicalMin, element.logicalMax, raw_name
                    )
            elif element.type == kIOHIDElementTypeInput_Button:

                # apply button filter to allow filtering for usage pages
                if (
                    self._button_filter is not None
                    and element.usagePage not in self._button_filter
                ):
                    continue
...
def get_joysticks(display=None):
    return [
        Joystick(PygletDevice(display, device))
        for device in _hid_manager.devices
...
def _create_controller(device, display):
    ...
    if not device.transport or not device.transport.upper() in (
        "USB",
        "BLUETOOTH",
    ...
    if device.is_joystick() or device.is_gamepad() or device.is_multi_axis():
        ...
        if mapping := get_mapping(device.get_guid()):
            return Controller(
                PygletDevice(display, device, axis_filter=[0x1, 0x2]), mapping
            )
        else:
            warnings.warn(
...
def get_controllers(display=None):
    return [
        controller
        for controller in [
            _create_controller(device, display)
            for device in _hid_manager.devices
            if device.is_joystick() or device.is_gamepad() or device.is_multi_axis()
        ]
