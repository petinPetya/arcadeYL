...
class DirectInputDevice(base.Device):
    ...
    def _set_format(self):
        ...
        fmt.rgodf = ctypes.cast(
            ctypes.pointer(object_formats), dinput.LPDIOBJECTDATAFORMAT
        )
    ...
    def open(self, window=None, exclusive=False):
        ...
        pyglet.app.platform_event_loop.add_wait_object(
            self._wait_object, self._dispatch_events
        )
    ...
    def matches(self, guid_id, device_instance):
        ...
        if (
            self.id_product_guid == guid_id
            and self.id_name == device_instance.contents.tszProductName
            and self._type == device_instance.contents.dwDevType & 0xFF
            and self._subtype == device_instance.contents.dwDevType & 0xFF00
        ):
            return True
...
GUID_DEVINTERFACE_HID = com.GUID(
...
class DIDeviceManager(EventDispatcher):
    ...
    def register_device_events(self, skip_warning=False, window=None):
        ...
        if not self.registered:
            ...
            if self.window is not None:
                ...
                self._devnotify = _user32.RegisterDeviceNotificationW(
                    self.window._hwnd, ctypes.byref(dbi), DEVICE_NOTIFY_WINDOW_HANDLE
                )
                ...
                if not skip_warning:
                    warnings.warn(
                        "DirectInput Device Manager requires a window to receive device connection events."
                    )
        ...
        if len(pyglet.app.windows) != 0:
            ...
            for existing_window in pyglet.app.windows:
                if existing_window != self.window:
                    self.register_device_events(
                        skip_warning=True, window=existing_window
                    )
                    return
    ...
    def _get_devices(self, display=None):
        ...
        def _device_enum(device_instance, arg):  # DIDEVICEINSTANCE
            ...
            _i_dinput.CreateDevice(
                device_instance.contents.guidInstance, ctypes.byref(device), None
            )
    ...
    def __init__(self, display=None):
            ...
            if di_device not in self._controllers:
                if self._add_controller(di_device):
                    pyglet.app.platform_event_loop.post_event(
                        self, "on_connect", self._controllers[di_device]
                    )
        ...
        def on_disconnect(di_device):
            if di_device in self._controllers:
                ...
                pyglet.app.platform_event_loop.post_event(
                    self, "on_disconnect", _controller
                )
...
def get_joysticks(display=None):
    ...
    return [
        joystick
        for joystick in [_create_joystick(device) for device in _di_manager.devices]
...
def get_controllers(display=None):
    ...
    return [
        controller
        for controller in [_create_controller(device) for device in _di_manager.devices]
