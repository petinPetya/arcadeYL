...
class WintabTabletCanvas(TabletCanvas):
    ...
    def __init__(self, device, window, msg_base=wintab.WT_DEFBASE):
        ...
        context_info.lcPktData = (
            ...
            | wintab.PK_Y
            | wintab.PK_Z
            | wintab.PK_NORMAL_PRESSURE
            | wintab.PK_TANGENT_PRESSURE
        ...
        if _extension_masks:
            window._event_handlers[msg_base + wintab.WT_PACKETEXT] = (
                self._event_wt_packetext
            )
        ...
        self.express_keys = defaultdict(
            lambda: defaultdict(bool)
        )  # [control_id][location_id]
        ...
        for tablet_id in range(get_tablet_count()):
            control_count = self.extension_get(
                wintab.WTX_EXPKEYS2,
                tablet_id,
                0,
                0,
            ...
            if self.override_keys is True:
                for control_id in range(control_count):
                    function_count = self.extension_get(
                        wintab.WTX_EXPKEYS2,
                        tablet_id,
                        control_id,
                        0,
                    ...
                    for function_id in range(function_count):
                        self.extension_set(
                            ...
                            control_id,
                            function_id,
                            wintab.TABLET_PROPERTY_OVERRIDE,
                            wintab.BOOL(True),
                        )
    ...
    def extension_set(
        self, extension, tablet_id, control_id, function_id, property_id, value
    ):
        prop = wintab.EXTPROPERTY()
        prop.version = 0
        prop.tabletIndex = tablet_id
    ...
    def _event_wt_packetext(self, msg, wParam, lParam):
        ...
        if packet.pkBase.nContext == self._context:
            if packet.pkExpKeys.nControl < self.express_key_ct:
                current_state = self.express_keys[packet.pkExpKeys.nControl][
                    packet.pkExpKeys.nLocation
                ]
                new_state = bool(packet.pkExpKeys.nState)
                if current_state != new_state:
                    event_type = (
                        "on_express_key_press"
                        if new_state
                        else "on_express_key_release"
                    )

                    self.express_keys[packet.pkExpKeys.nControl][
                        packet.pkExpKeys.nLocation
                    ] = new_state
...
class WintabTabletCursor:
    def __init__(self, device, index):
        ...
        self.id = (wtinfo_dword(self._cursor, wintab.CSR_TYPE) << 32) | wtinfo_dword(
            self._cursor, wintab.CSR_PHYSID
        )
...
def extension_index(ext):
    ...
    while exists:
        tag = wintab.UINT()
        exists = lib.WTInfoW(
            wintab.WTI_EXTENSIONS + i, wintab.EXT_TAG, ctypes.byref(tag)
        )
        if tag.value == ext:
            index = i
            break
