...
if library_name == "xinput1_4":
    ...
    XInputGetCapabilities.argtypes = [
        ...
        DWORD,
        DWORD,
        POINTER(XINPUT_CAPABILITIES_EX),
    ]
...
CLSID_WbemLocator = com.GUID(
    0x4590F811, 0x1D3A, 0x11D0, 0x89, 0x1F, 0x00, 0xAA, 0x00, 0x4B, 0x2E, 0x24
)
IID_IWbemLocator = com.GUID(
...
class IWbemClassObject(com.pIUnknown):
    _methods_ = [
        ...
        ("Get", com.STDMETHOD(BSTR, LONG, POINTER(VARIANT), c_void_p, c_void_p)),
        # ... long, unneeded
    ]
...
class IEnumWbemClassObject(com.pIUnknown):
    _methods_ = [
        ("Reset", com.STDMETHOD()),
        ("Next", com.STDMETHOD(LONG, ULONG, POINTER(IWbemClassObject), POINTER(ULONG))),
        ("NextAsync", com.STDMETHOD()),
        ("Clone", com.STDMETHOD()),
        ("Skip", com.STDMETHOD()),
    ]
...
class IWbemLocator(com.pIUnknown):
    _methods_ = [
        (
            "ConnectServer",
            com.STDMETHOD(
                BSTR, BSTR, BSTR, LONG, LONG, BSTR, IWbemContext, POINTER(IWbemServices)
            ),
        ),
    ]
...
def get_xinput_guids():
    ...
    ole32.CoCreateInstance(
        CLSID_WbemLocator, None, CLSCTX_INPROC_SERVER, IID_IWbemLocator, byref(locator)
    )
    ...
    hr = locator.ConnectServer(
        name_space, None, None, 0, 0, None, None, byref(services)
    )
    if hr != 0:
        return guids_found
...
class XInputDeviceManager(EventDispatcher):
    ...
    def __init__(self):
        ...
        for i in range(XUSER_MAX_COUNT):
            ...
            if (
                XInputGetState(i, byref(device.xinput_state))
    ...
    def _on_state_change(device):
        ...
        device.controls["righttrigger"].value = (
            device.xinput_state.Gamepad.bRightTrigger
        )
...
class XInputController(Controller):
    ...
    def _add_axis(self, control, name):
        ...
        if name in ("lefttrigger", "righttrigger"):
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_stick_motion", self, "leftstick", self.leftanalog
                )
            ...
            def on_change(value):
                ...
                self.dispatch_event(
                    "on_stick_motion", self, "rightstick", self.rightanalog
                )
    ...
    def rumble_play_weak(self, strength=1.0, duration=0.5):
        self.device.vibration.wRightMotorSpeed = int(
...
def get_controllers():
    return [
        XInputController(device, {"name": device.name, "guid": device.get_guid()})
