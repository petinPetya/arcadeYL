...
if not pyglet.options["win32_disable_xinput"]:
    try:
        from pyglet.input.win32.xinput import (
...
class Win32ControllerManager(base.ControllerManager):
    ...
    def __init__(self):
        ...
        if _xinput_enabled:
            ...
            for (
                xdev
            ) in _xinput_device_manager.all_devices:  # All 4 devices are initialized.
    ...
    def _get_xinput_controllers(self) -> list:
        if not _xinput_enabled:
            return []
        return [
            ctlr for ctlr in self._xinput_controllers.values() if ctlr.device.connected
        ]
