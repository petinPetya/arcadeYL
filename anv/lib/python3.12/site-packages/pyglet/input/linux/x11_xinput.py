    ...
    def __init__(self, display, device_info):
        ...
        for i in range(device_info.num_classes):
            ...
            if cls_class == xi.KeyClass:
                ...
                axes = ctypes.cast(cp.contents.axes, ctypes.POINTER(xi.XAxisInfo))
                for i in range(num_axes):
                    axis = axes[i]
                    if mode == xi.Absolute:
                        self.axes.append(
                            AbsoluteAxis("axis%d" % i, axis.min_value, axis.max_value)
                        )
...
class XInputWindowEventDispatcher:
    ...
    def open_device(self, device_id, device, responder):
        ...
        xi.XSelectExtensionEvent(
            self.window._x_display, self.window._window, array, len(array)
        )
    ...
    def _event_xinput_button_press(self, ev):
        e = ctypes.cast(
            ctypes.byref(ev), ctypes.POINTER(xi.XDeviceButtonEvent)
        ).contents
    ...
    def _event_xinput_motion(self, ev):
        e = ctypes.cast(
            ctypes.byref(ev), ctypes.POINTER(xi.XDeviceMotionEvent)
        ).contents
    ...
    def _event_xinput_proximity_in(self, ev):
        e = ctypes.cast(
            ctypes.byref(ev), ctypes.POINTER(xi.XProximityNotifyEvent)
        ).contents
