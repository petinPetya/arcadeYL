...
class Timeval(ctypes.Structure):
    _fields_ = (("tv_sec", _s64), ("tv_usec", _s64))


class InputEvent(ctypes.Structure):
    _fields_ = (("time", Timeval), ("type", _u16), ("code", _u16), ("value", _s32))
...
class FFReplay(ctypes.Structure):
    _fields_ = (("length", _u16), ("delay", _u16))


class FFTrigger(ctypes.Structure):
    _fields_ = (("button", _u16), ("interval", _u16))
...
class FFRumbleEffect(ctypes.Structure):
    _fields_ = (("strong_magnitude", _u16), ("weak_magnitude", _u16))
...
def _create_control(fileno, event_type, event_code):
    if event_type == EV_ABS:
        ...
        control = EvdevAbsoluteAxis(
            name, minimum, maximum, raw_name, inverted=name == "hat_y"
        )
        control.value = value
...
class EvdevDevice(XlibSelectDevice, Device):
    ...
    def get_guid(self):
        ...
        _id = self._id
        return create_guid(
            _id.bustype, _id.vendor, _id.product, _id.version, self.name, 0, 0
        )
        ...
        for event in self._event_buffer[:n_events]:
            ...
            if (event.type, event.code) == (EV_SYN, SYN_REPORT):
                ...
                for queued_event in self._event_queue:
                    if control := self.control_map.get(
                        (queued_event.type, queued_event.code)
                    ):
...
class FFController(Controller):
    ...
    def open(self, window=None, exclusive=False):
        ...
        self._play_strong_event = InputEvent(
            Timeval(), EV_FF, self._strong_effect.id, 1
        )
        self._stop_strong_event = InputEvent(
            Timeval(), EV_FF, self._strong_effect.id, 0
        )
    ...
    def rumble_play_weak(self, strength=1.0, duration=0.5):
        ...
        effect.u.ff_rumble_effect.weak_magnitude = int(
            max(min(1.0, strength), 0) * 0xFFFF
        )
    ...
    def rumble_play_strong(self, strength=1.0, duration=0.5):
        ...
        effect.u.ff_rumble_effect.strong_magnitude = int(
            max(min(1.0, strength), 0) * 0xFFFF
        )
        effect.ff_replay.length = int(duration * 1000)
        EVIOCSFF(self._fileno, effect)
...
class EvdevControllerManager(ControllerManager, XlibSelectDevice):
    ...
    def select(self):
        ...
        for name in appeared:
            t = threading.Thread(
                target=self._make_controller_thread, args=(name, 10), daemon=True
            )
            t.start()
...
def _create_joystick(device):
    ...
    for control in device.controls:
        if control.event_type == EV_ABS and control.event_code == ABS_X:
            ...
            have_y = True
        elif control.event_type == EV_KEY and control.event_code in (
            BTN_JOYSTICK,
            BTN_GAMEPAD,
        ):
            have_button = True
...
def get_joysticks(display=None):
    return [
        joystick
        for joystick in [_create_joystick(device) for device in get_devices(display)]
        if joystick
    ]
...
def _detect_controller_mapping(device):
    ...
    button_controls = [
        control for control in device.controls if isinstance(control, EvdevButton)
    ]
    axis_controls = [
        control for control in device.controls if isinstance(control, EvdevAbsoluteAxis)
    ]
    hat_controls = [
        control for control in device.controls if control.name in ("hat_x", "hat_y")
    ]
...
def _create_controller(device) -> Controller | None:
    ...
    if not mapping:
        warnings.warn(
            f"Warning: {device} (GUID: {device.get_guid()}) "
