...
class WidgetBase(EventDispatcher):
    ...
    def _check_hit(self, x: int, y: int) -> bool:
        return (
            self._x < x < self._x + self._width and self._y < y < self._y + self._height
        )
...
class PushButton(WidgetBase):
    ...
    def __init__(
        ...
        self._sprite = pyglet.sprite.Sprite(
            self._unpressed_img, x=x, y=y, batch=batch, group=bg_group
        )
    ...
    def on_mouse_release(self, x: int, y: int, buttons: int, modifiers: int) -> None:
        if not self.enabled or not self._pressed:
            return
        self._sprite.image = (
            self._hover_img if self._check_hit(x, y) else self._unpressed_img
        )
        self._pressed = False
...
class ToggleButton(PushButton):
    ...
    def on_mouse_press(self, x: int, y: int, buttons: int, modifiers: int) -> None:
        ...
        self._sprite.image = (
            self._pressed_img if self._pressed else self._get_release_image(x, y)
        )
...
class Slider(WidgetBase):
    ...
    def __init__(
        ...
        self._base_spr = pyglet.sprite.Sprite(
            self._base_img, x, y, batch=batch, group=bg_group
        )
    ...
    def _update_position(self) -> None:
        ...
        self._max_knob_x = (
            self._x + self._base_img.width - self._knob_img.width - self._edge
        )
        x = (
            (self._max_knob_x - self._min_knob_x) * self._value / 100
            + self._min_knob_x
    ...
    def _update_knob(self, x: int) -> None:
        ...
        self._value = abs(
            ((self._knob_spr.x - self._min_knob_x) * 100)
...
class TextEntry(WidgetBase):
    ...
    def __init__(
        ...
        self._layout = IncrementalTextLayout(
            self._doc, x, y, 0, width, height, batch=batch, group=fg_group
        )
