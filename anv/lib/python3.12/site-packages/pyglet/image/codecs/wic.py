...
def _get_bitmap_frame(
    bitmap_decoder: IWICBitmapDecoder, frame_index: int
) -> IWICBitmapFrameDecode:
...
def get_bitmap(
    width: int, height: int, target_fmt: com.GUID = GUID_WICPixelFormat32bppBGRA
) -> IWICBitmap:
    """Create a WIC Bitmap.
    ...
    """
    bitmap = IWICBitmap()
    _factory.CreateBitmap(
        width, height, target_fmt, WICBitmapCacheOnDemand, byref(bitmap)
    )
    return bitmap


def extract_image_data(
    bitmap: IWICBitmap, target_fmt: com.GUID = GUID_WICPixelFormat32bppBGRA
) -> ImageData:
...
class WICDecoder(ImageDecoder):
    ...
    def _load_bitmap_decoder(
        self, filename: str, file: BinaryIO | None = None
    ) -> tuple[IWICBitmapDecoder, IStream]:
        data = file.read()
        ...
        status = self._factory.CreateDecoderFromStream(
            stream, None, WICDecodeMetadataCacheOnDemand, byref(decoder)
        )
        if status != 0:
            stream.Release()
    ...
    def encode(self, image: ImageData, filename: str, file: BinaryIO | None) -> None:
        ...
        if file and istream:
            ...
            if written.value == stream_size:
                ...
                print(
                    f"Failed to read all of the data from stream attempting to save {file}"
                )
