...
class GError(Structure):
    _fields_ = [("domain", GQuark), ("code", gint), ("message", POINTER(gchar))]
...
def _gerror_to_string(error):
    ...
    """
    return "GdkPixBuf Error: domain[{}], code[{}]: {}".format(
        error.contents.domain, error.contents.code, error.contents.message
    )
...
class GdkPixBufLoader:
    ...
    def write(self, data):
        ...
        error = gerror_ptr()
        if not gdkpixbuf.gdk_pixbuf_loader_write(
            self._loader, data, len(data), byref(error)
        ):
            raise ImageDecodeException(_gerror_to_string(error))
...
class GdkPixBuf:
    ...
    def get_pixels(self):
        ...
        memmove(
            buf, pixels, self.rowstride * (self.height - 1) + self.width * self.channels
        )
        return buf
...
class GdkPixBufAnimation:
    ...
    def __iter__(self):
        ...
        return GdkPixBufAnimationIterator(
            self._loader, anim_iter, time, self._gif_delays
        )
