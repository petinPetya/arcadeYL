...
class Animation:
    ...
    def get_transform(
        ...
        frames = [
            AnimationFrame(
                frame.image.get_texture().get_transform(flip_x, flip_y, rotate),
                frame.duration,
            )
            for frame in self.frames
        ]
    ...
    def get_duration(self) -> float:
        ...
        return sum(
            [frame.duration for frame in self.frames if frame.duration is not None]
        )
    ...
    def from_image_sequence(
        cls, sequence: AbstractImageSequence, duration: float, loop: bool = True
    ) -> Animation:
