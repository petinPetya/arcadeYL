...
def load(
    filename: str, file: BinaryIO | None = None, decoder: ImageDecoder | None = None
) -> AbstractImage:
...
def create(
    width: int, height: int, pattern: ImagePattern | None = None
) -> AbstractImage:
...
class ImageData(AbstractImage):
    ...
    def __init__(
        self, width: int, height: int, fmt: str, data: bytes, pitch: int | None = None
    ):
    ...
    def create_texture(self, cls: type[Texture], rectangle: bool = False) -> Texture:
        ...
        texture = cls.create(
            self.width, self.height, GL_TEXTURE_2D, internalformat, blank_data=False
        )
        ...
        self.blit_to_texture(
            texture.target, texture.level, self.anchor_x, self.anchor_y, 0, None
        )
    ...
    def get_mipmapped_texture(self) -> Texture:
        ...
        texture = Texture.create(
            self.width, self.height, GL_TEXTURE_2D, None, blank_data=False
        )
        if self.anchor_x or self.anchor_y:
            texture.anchor_x = self.anchor_x
            texture.anchor_y = self.anchor_y
    ...
    def blit_to_texture(
        ...
        if target == GL_TEXTURE_3D or target == GL_TEXTURE_2D_ARRAY:
            ...
            glTexSubImage3D(
                target, level, x, y, z, self.width, self.height, 1, fmt, gl_type, data
            )
        elif internalformat:
            glTexImage2D(
                target,
            ...
            glTexSubImage2D(
                target, level, x, y, self.width, self.height, fmt, gl_type, data
            )
    ...
    def _convert(self, fmt: str, pitch: int) -> bytes:
        ...
        if pitch != current_pitch:
            ...
            if diff > 0:
                ...
                new_pitch = abs(pitch)
                rows = [
                    data[i : i + new_pitch - diff]
                    for i in range(0, len(data), new_pitch)
                ]
                data = b"".join(rows)
...
class CompressedImageData(AbstractImage):
    ...
    def get_mipmapped_texture(self) -> Texture:
        ...
        for data in self.mipmap_data:
            ...
            level += 1
            glCompressedTexImage2D(
                texture.target, level, self.gl_format, width, height, 0, len(data), data
            )
...
class Texture(AbstractImage):
    ...
    def create(
        ...
        internalformat: int | None = GL_RGBA8,
        min_filter: int | None = None,
        mag_filter: int | None = None,
        fmt: int = GL_RGBA,
    ...
    def get_image_data(self, z: int = 0) -> ImageData:
        ...
        if pyglet.gl.current_context.get_info().get_opengl_api() == "gles":
            ...
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self.id, self.level
            )
            glReadPixels(
                0, 0, self.width, self.height, gl_format, GL_UNSIGNED_BYTE, buf
            )
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(id={self.id}, size={self.width}x{self.height})"
        )
...
class TextureRegion(Texture):
    ...
    def blit_into(self, source: AbstractImage, x: int, y: int, z: int) -> None:
        assert (
            source.width <= self._width and source.height <= self._height
        ), f"{source} is larger than {self}"
...
class Texture3D(Texture, UniformTextureSequence):
    ...
    def create_for_images(cls, images, internalformat=GL_RGBA, blank_data=True):
        ...
        if not all(
            img.width == item_width and img.height == item_height for img in images
        ):
        ...
        blank = (
            (GLubyte * (texture.width * texture.height * texture.images * 4))()
            if blank_data
            else None
        )
        glBindTexture(texture.target, texture.id)
        ...
        for i, image in enumerate(images):
            ...
            items.append(item)
            image.blit_to_texture(
                texture.target, texture.level, image.anchor_x, image.anchor_y, z=i
            )
    ...
    def __setitem__(self, index, value):
        if type(index) is slice:
            ...
            for item, image in zip(self[index], value):
                image.blit_to_texture(
                    self.target, self.level, image.anchor_x, image.anchor_y, item.z
                )
        else:
...
class TextureArray(Texture, UniformTextureSequence):
    ...
    def create(
        ...
        ), f"TextureArray max_depth supported is {max_depth_limit}."
    ...
    def allocate(self, *images: AbstractImage) -> list[TextureArrayRegion]:
        ...
        if len(self.items) + len(images) > self.max_depth:
            raise TextureArrayDepthExceeded(
                "The amount of images being added exceeds the depth of this TextureArray."
            )
        ...
        for i, image in enumerate(images):
            self._verify_size(image)
            item = self.region_class(
                0, 0, start_length + i, image.width, image.height, self
            )
            self.items.append(item)
            image.blit_to_texture(
                self.target,
    ...
    def create_for_image_grid(cls, grid, internalformat: int = GL_RGBA) -> TextureArray:
        texture_array = cls.create(
            grid[0].width, grid[0].height, internalformat, max_depth=len(grid)
        )
    ...
    def __setitem__(self, index, value) -> None:
        if type(index) is slice:
            ...
            for old_item, image in zip(self[index], value):
                self._verify_size(image)
                item = self.region_class(
                    0, 0, old_item.z, image.width, image.height, self
                )
                image.blit_to_texture(
                    self.target, self.level, image.anchor_x, image.anchor_y, old_item.z
                )
...
class ImageGrid(AbstractImage, AbstractImageSequence):
    ...
    def __init__(
        ...
        self.item_width = (
            item_width or (image.width - column_padding * (columns - 1)) // columns
        )
        self.item_height = (
            item_height or (image.height - row_padding * (rows - 1)) // rows
        )
        ...
        if not self._items:
            ...
            for row in range(self.rows):
                ...
                for col in range(self.columns):
                    self._items.append(
                        self.image.get_region(x, y, self.item_width, self.item_height)
                    )
...
class TextureGrid(TextureRegion, UniformTextureSequence):
    ...
    def __setitem__(
        self, index: int | slice, value: AbstractImage | Sequence[AbstractImage]
    ):
        if type(index) is slice:
...
class BufferManager:
    ...
    def get_color_buffer(self) -> ColorBufferImage:
        ...
        if (
            not self._color_buffer
            or viewport_width != self._color_buffer.width
            or viewport_height != self._color_buffer.height
        ):
    ...
    def get_depth_buffer(self) -> DepthBufferImage:
        ...
        viewport_height = viewport[3]
        if (
            not self._depth_buffer
            or viewport_width != self._depth_buffer.width
            or viewport_height != self._depth_buffer.height
        ):
...
class BufferImage(AbstractImage):
    ...
    def get_image_data(self):
        ...
        glPixelStorei(GL_PACK_ALIGNMENT, 1)
        glReadPixels(
            x, y, self.width, self.height, self.gl_format, GL_UNSIGNED_BYTE, buf
        )
...
class ColorBufferImage(BufferImage):
    ...
    def get_texture(self):
        texture = Texture.create(
            self.width, self.height, GL_TEXTURE_2D, GL_RGBA, blank_data=False
        )
        self.blit_to_texture(
            texture.target, texture.level, self.anchor_x, self.anchor_y, 0
        )
        return texture
