...
_uniform_setters: dict[int, tuple[GLDataType, GLFunc, GLFunc, int]] = {
    ...
    gl.GL_SAMPLER_CUBE_MAP_ARRAY: (
        gl.GLint,
        gl.glUniform1iv,
        gl.glProgramUniform1iv,
        1,
    ),
    gl.GL_INT_SAMPLER_CUBE_MAP_ARRAY: (
        gl.GLint,
        gl.glUniform1iv,
        gl.glProgramUniform1iv,
        1,
    ),
    gl.GL_UNSIGNED_INT_SAMPLER_CUBE_MAP_ARRAY: (
        gl.GLint,
        gl.glUniform1iv,
        gl.glProgramUniform1iv,
        1,
    ...
    gl.GL_INT_IMAGE_CUBE_MAP_ARRAY: (
        gl.GLint,
        gl.glUniform1iv,
        gl.glProgramUniform1iv,
        1,
    ...
    gl.GL_UNSIGNED_INT_IMAGE_CUBE_MAP_ARRAY: (
        gl.GLint,
        gl.glUniform1iv,
        gl.glProgramUniform1iv,
        1,
...
class Attribute:
    ...
    def set_pointer(self, ptr: int) -> None:
        ...
        if self._is_int:
            glVertexAttribIPointer(
                self.location, self.count, self.gl_type, self.stride, ptr
            )
        else:
            glVertexAttribPointer(
                self.location,
    ...
    def get_region(
        self, buffer: AttributeBufferObject, start: int, count: int
    ) -> Array[CTypesDataType]:
...
class _UniformArray:
    ...
    def __setitem__(self, key: slice | int, value: Sequence) -> None:
        ...
        if self._uniform.length > 1:
            assert len(value) == self._uniform.length, (
                f"Setting this key requires {self._uniform.length} values, "
    ...
    def __repr__(self) -> str:
        data = [
            tuple(data) if self._uniform.length > 1 else data for data in self._c_array
        ]
...
class _Uniform:
    ...
    def __init__(
        ...
        gl_type, gl_setter_legacy, gl_setter_dsa, length = _uniform_setters[
            uniform_type
        ]
        ...
        if size > 1:
            ...
            self.get = self._create_getter_func(
                program, location, gl_getter, c_array, length
            )
            self.set = self._create_setter_func(
                program, location, gl_setter, c_array, length, ptr, is_matrix, dsa
            )
...
class _UBOBindingManager:
    ...
    def add_explicit_binding(
        self, shader_program: ShaderProgram, ub_name: str, binding: int
    ) -> None:
...
class UniformBlock:
    ...
    def set_binding(self, binding: int) -> None:
        ...
        assert (
            binding != 0
        ), "Binding 0 is reserved for the internal Pyglet 'WindowBlock'."
    ...
    def _introspect_uniforms(self) -> type[Structure]:
        ...
        gl.glGetActiveUniformBlockiv(
            p_id, index, gl.GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, indices_ptr
        )
        gl.glGetActiveUniformsiv(
            p_id, active_count, indices, gl.GL_UNIFORM_OFFSET, offsets_ptr
        )
        ...
        def build_ctypes_struct(name: str, struct_dict: dict) -> type:
            ...
            for field_name, field_type in struct_dict.items():
                if isinstance(field_type, dict):
                    ...
                    if field_name in array_sizes and array_sizes[field_name] > 1:
                        field_type = (
                            element_struct * array_sizes[field_name]
                        )  # noqa: PLW2901
                else:
            ...
            return type(
                name.title(), (Structure,), {"_fields_": fields, "__repr__": rep_func}
            )
            ...
            for part_idx, part in enumerate(parts):
                ...
                if match:  # It's an array
                    ...
                    if part_idx != len(parts) - 1:
                        ...
                        array_sizes[arr_name] = max(
                            array_sizes.get(arr_name, 0), index + 1
                        )
                        ...
                        current_structure = current_structure[
                            arr_name
                        ]  # Move to the correct index of the array
                        continue
    ...
    def _actual_binding_point(self) -> int:
        ...
        binding = gl.GLint()
        gl.glGetActiveUniformBlockiv(
            self.program.id, self.index, gl.GL_UNIFORM_BLOCK_BINDING, binding
        )
...
class UniformBufferObject:
    ...
    def __init__(
        self, view_class: type[Structure], buffer_size: int, binding: int
    ) -> None:
...
def _introspect_attributes(program_id: int) -> dict[str, Any]:
    ...
    for index in range(_get_number(program_id, gl.GL_ACTIVE_ATTRIBUTES)):
        ...
        loc = gl.glGetAttribLocation(
            program_id, create_string_buffer(a_name.encode("utf-8"))
        )
...
def _introspect_uniforms(program_id: int, have_dsa: bool) -> dict[str, _Uniform]:
    ...
    for index in range(_get_number(program_id, gl.GL_ACTIVE_UNIFORMS)):
        ...
        loc = gl.glGetUniformLocation(
            program_id, create_string_buffer(u_name.encode("utf-8"))
        )
        ...
        assert (
            u_name not in uniforms
        ), f"{u_name} exists twice in the shader. Possible name clash with an array."
...
def _introspect_uniform_blocks(
    program: ShaderProgram | ComputeShaderProgram,
    ...
    for index in range(_get_number(program_id, gl.GL_ACTIVE_UNIFORM_BLOCKS)):
        ...
        gl.glGetActiveUniformBlockiv(
            program_id, index, gl.GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, num_active
        )
        gl.glGetActiveUniformBlockiv(
            program_id, index, gl.GL_UNIFORM_BLOCK_DATA_SIZE, block_data_size
        )
        gl.glGetActiveUniformBlockiv(
            program_id, index, gl.GL_UNIFORM_BLOCK_BINDING, binding
        )

        indices = (gl.GLuint * num_active.value)()
        indices_ptr = cast(addressof(indices), POINTER(gl.GLint))
        gl.glGetActiveUniformBlockiv(
            program_id, index, gl.GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, indices_ptr
        )
        ...
        for block_uniform_index in indices:
            uniform_name, u_type, u_size = _query_uniform(
                program_id, block_uniform_index
            )
        ...
        if pyglet.options.shader_bind_management:
            ...
            if binding.value == 0:
                ...
                if _block_name and _block_name != name:
                    msg = (
                        f"{program} explicitly set '{name}' to {binding.value} in the shader. '{_block_name}' has "
                        f"been overridden."
                    )
                    warnings.warn(msg)
...
class Shader:
    ...
    def _get_shader_log(self, shader_id: int) -> str:
        ...
        if result_str.value:
            return (
                f"OpenGL returned the following message when compiling the "
...
class ShaderProgram:
    ...
    def __init__(self, *shaders: Shader) -> None:
        ...
        have_dsa = gl_info.have_version(4, 1) or gl_info.have_extension(
    ...
    def uniforms(self) -> dict[str, Any]:
        ...
        """
        return {
            n: {"location": u.location, "length": u.length, "size": u.size}
            for n, u in self._uniforms.items()
        }
        ...
        **data: Any,
        ...
        for name, fmt in data.items():
            try:
                ...
                if _debug_gl_shaders:
                    msg = (
                        f"The attribute `{name}` was not found in the Shader Program.\n"
    ...
    def vertex_list(
        ...
        **data: Any,
        ...
        return self._vertex_list_create(
            count, mode, None, None, batch=batch, group=group, **data
        )
    ...
    def vertex_list_instanced(
        ...
        **data: Any,
        ...
        assert (
            len(instance_attributes) > 0
        ), "You must provide at least one attribute name to be instanced."
        return self._vertex_list_create(
            count, mode, None, instance_attributes, batch=batch, group=group, **data
        )
    ...
    def vertex_list_indexed(
        ...
        **data: Any,
        ...
        """
        return self._vertex_list_create(
            count, mode, indices, None, batch=batch, group=group, **data
        )
    ...
    def vertex_list_instanced_indexed(
        ...
        **data: Any,
        ...
        return self._vertex_list_create(
            count, mode, indices, instance_attributes, batch=batch, group=group, **data
        )
...
class ComputeShaderProgram:
    ...
    def __init__(self, source: str) -> None:
        ...
        if not (
            gl_info.have_version(4, 3)
            or gl_info.have_extension("GL_ARB_compute_shader")
        ):
            msg = (
        ...
        self.max_work_group_size = self._get_tuple(
            gl.GL_MAX_COMPUTE_WORK_GROUP_SIZE
        )  # x, y, z
        self.max_work_group_count = self._get_tuple(
            gl.GL_MAX_COMPUTE_WORK_GROUP_COUNT
        )  # x, y, z
        self.max_shared_memory_size = self._get_value(
            gl.GL_MAX_COMPUTE_SHARED_MEMORY_SIZE
        )
        self.max_work_group_invocations = self._get_value(
    ...
    @staticmethod
    def dispatch(
        x: int = 1, y: int = 1, z: int = 1, barrier: int = gl.GL_ALL_BARRIER_BITS
    ) -> None:
