...
class VertexList:
    ...
    def __init__(
        self, domain: VertexDomain, start: int, count: int
    ) -> None:  # noqa: D107
        self.domain = domain
        self.start = start
        self.count = count
    ...
    def resize(
        self, count: int, index_count: int | None = None
    ) -> None:  # noqa: ARG002
        """Resize this group.
    ...
    def set_instance_source(
        self, domain: InstancedVertexDomain, instance_attributes: Sequence[str]
    ) -> None:
        assert self.instanced is False, "Vertex list is already an instance."
        assert list(domain.attribute_names.keys()) == list(
            self.domain.attribute_names.keys()
    ...
    def add_instance(self, **kwargs: Any) -> VertexInstance:
        ...
        for buffer, attribute in self.domain.buffer_attributes:
            if attribute.instance:
                assert attribute.name in kwargs, (
                    f"{attribute.name} is defined as an instance attribute, "
                    f"keyword argument not found."
                )
                buffer.set_region(instance_id - 1, 1, kwargs[attribute.name])

        return self.domain._vertexinstance_class(
...
class IndexedVertexList(VertexList):
    ...
    def migrate(
        self, domain: IndexedVertexDomain | InstancedIndexedVertexDomain
    ) -> None:
        ...
        if old_start != self.start:
            ...
            old_indices = old_domain.index_buffer.get_region(
                self.index_start, self.index_count
            )
            old_domain.index_buffer.set_region(
                self.index_start, self.index_count, [i + diff for i in old_indices]
            )

        # copy indices to new domain
        old_array = old_domain.index_buffer.get_region(
            self.index_start, self.index_count
        )
    ...
    def indices(self) -> list[int]:
        ...
        return [
            i - start
            for i in self.domain.index_buffer.get_region(
                self.index_start, self.index_count
            )
        ]
    ...
    def indices(self, data: Sequence[int]) -> None:
        ...
        self.domain.index_buffer.set_region(
            self.index_start, self.index_count, tuple(i + start for i in data)
        )
...
class VertexInstance:
    ...
    def __init__(
        self, vertex_list: VertexList | IndexedVertexList, instance_id: int, start: int
    ) -> None:
        self.id = instance_id
        self.start = start
...
class VertexDomain:
    ...
    def __init__(self, attribute_meta: dict[str, dict[str, Any]]) -> None:  # noqa: D107
        ...
        self.attrib_name_buffers = (
            {}
        )  # dict of AttributeName: AttributeBufferObject (for VertexLists)

        self._property_dict = {}  # name: property(_getter, _setter)

        for name, meta in attribute_meta.items():
            assert (
                meta["format"][0] in _gl_types
            ), f"'{meta['format']}' is not a valid attribute format for '{name}'."
            ...
            self.attribute_names[name] = attribute = shader.Attribute(
                name, location, count, gl_type, normalize, instanced
            )

            # Create buffer:
            self.attrib_name_buffers[name] = buffer = AttributeBufferObject(
                attribute.stride * self.allocator.capacity, attribute
            )
        ...
        self._vertexlist_class = type(
            self._vertex_class.__name__, (self._vertex_class,), self._property_dict
        )
...
class InstancedVertexDomain(VertexDomain):  # noqa: D101
    ...
    def __init__(self, attribute_meta: dict[str, dict[str, Any]]) -> None:
        ...
        for name, attribute in self.attribute_names.items():
            if attribute.instance:
                self._instance_properties[name] = _make_instance_attribute_property(
                    name
                )
            else:
                self._instance_properties[name] = (
                    _make_restricted_instance_attribute_property(name)
                )

        self._vertexinstance_class = type(
            "VertexInstance", (VertexInstance,), self._instance_properties
        )
    ...
    def draw_subset(self, mode: int, vertex_list: VertexList) -> None:
        ...
        glDrawArraysInstanced(
            mode, vertex_list.start, vertex_list.count, self._instances
        )
...
class IndexedVertexDomain(VertexDomain):
    ...
    def __init__(
        self,
        attribute_meta: dict[str, dict[str, Any]],  # noqa: D107
        index_gl_type: int = GL_UNSIGNED_INT,
    ) -> None:
        super().__init__(attribute_meta)
        ...
        self.index_buffer = IndexedBufferObject(
    ...
    def draw(self, mode: int) -> None:
        ...
        if primcount == 0:
            ...
            starts = [
                s * self.index_element_size + self.index_buffer.ptr for s in starts
            ]
            starts = (ctypes.POINTER(GLvoid) * primcount)(
                *(GLintptr * primcount)(*starts)
            )
            sizes = (GLsizei * primcount)(*sizes)
    ...
    def draw_subset(self, mode: int, vertex_list: IndexedVertexList) -> None:
        ...
        glDrawElements(
            ...
            self.index_gl_type,
            self.index_buffer.ptr + vertex_list.index_start * self.index_element_size,
        )
