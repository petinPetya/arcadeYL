...
class AbstractBuffer:
    ...
    @abc.abstractmethod
    def set_data_region(
        self, data: Sequence[int] | CTypesPointer, start: int, length: int
    ) -> None:
...
class BufferObject(AbstractBuffer):
    ...
    def map(self) -> CTypesPointer[ctypes.c_byte]:
        ...
        return ctypes.cast(
            ...
            ctypes.POINTER(ctypes.c_byte * self.size),
        ).contents

    def map_range(
        self, start: int, size: int, ptr_type: type[CTypesPointer]
    ) -> CTypesPointer:
        glBindBuffer(GL_ARRAY_BUFFER, self.id)
        return ctypes.cast(
            glMapBufferRange(GL_ARRAY_BUFFER, start, size, GL_MAP_WRITE_BIT), ptr_type
        ).contents
...
class PersistentBufferObject(AbstractBuffer):
    ...
    def __init__(self, size, attribute, vao):
        ...
        self.data = ctypes.cast(
            glMapBufferRange(GL_ARRAY_BUFFER, 0, size, self.flags), ptr
        ).contents
