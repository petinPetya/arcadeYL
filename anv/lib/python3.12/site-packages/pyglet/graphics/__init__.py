...
def draw(size: int, mode: int, **data: Any) -> None:
    ...
    for name, (fmt, array) in data.items():
        ...
        ), f"Data for {fmt} is incorrect length"
...
def get_default_shader() -> ShaderProgram:
    ...
    try:
        ...
        pyglet.gl.current_context.object_space.pyglet_graphics_default_shader = (
            _shader_program
        )
        return pyglet.gl.current_context.object_space.pyglet_graphics_default_shader
...
def get_default_blit_shader() -> ShaderProgram:
    ...
    try:
        return (
            pyglet.gl.current_context.object_space.pyglet_graphics_default_blit_shader
        )
    except AttributeError:
...
class Batch:
    ...
    def update_shader(
        ...
        for a_name in attributes:
            if (
                ...
                and vertex_list.initial_attribs[a_name]["format"]
                != attributes[a_name]["format"]
            ):
                attributes[a_name]["format"] = vertex_list.initial_attribs[a_name][
                    "format"
                ]

        domain = self.get_domain(
            vertex_list.indexed, vertex_list.instanced, mode, group, attributes
        )
    ...
    def _convert_to_instanced(
        self,
        domain: vertexdomain.VertexDomain | vertexdomain.IndexedVertexDomain,
    ...
    def draw_subset(
        self, vertex_lists: Sequence[VertexList | IndexedVertexList]
    ) -> None:
...
class ShaderGroup(Group):
    ...
    def __init__(
        self, program: ShaderProgram, order: int = 0, parent: Group | None = None
    ) -> None:  # noqa: D107
...
class TextureGroup(Group):
    ...
    def __init__(
        self, texture: pyglet.image.Texture, order: int = 0, parent: Group | None = None
    ) -> None:
