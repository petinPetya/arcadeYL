...
class XlibWindow(BaseWindow):
    ...
    _mouse_exclusive_client: tuple[int, int] | None = (
        None  # x,y of "real" mouse during exclusive
    )
    ...
    def __init__(self, *args, **kwargs) -> None:  # noqa: ANN002, ANN003
        ...
        if _can_detect_autorepeat is None:
            ...
            _can_detect_autorepeat = XkbSetDetectableAutoRepeat(
                self.display._display, c_int(1), byref(supported_rtrn)  # noqa: SLF001
            )
    ...
    def _create_xdnd_atoms(self, display: xlib.Display) -> None:
        self._xdnd_atoms = {
            ...
            "XdndActionCopy": xlib.XInternAtom(
                display, asbytes("XdndActionCopy"), False
            ),
    ...
    def _create(self) -> None:
        ...
        if not self._window:
            ...
            self._enable_xsync = (
                self.display._enable_xsync and self.config.double_buffer
            )

            # Set supported protocols
            protocols = list()
            protocols.append(
                xlib.XInternAtom(self._x_display, asbytes("WM_DELETE_WINDOW"), False)
            )
            if self._enable_xsync:
                protocols.append(
                    xlib.XInternAtom(
                        self._x_display, asbytes("_NET_WM_SYNC_REQUEST"), False
                    )
                )
            protocols = (c_ulong * len(protocols))(*protocols)
            xlib.XSetWMProtocols(
                self._x_display, self._window, protocols, len(protocols)
            )
            ...
            if self._enable_xsync:
                ...
                atom = xlib.XInternAtom(
                    self._x_display, asbytes("_NET_WM_SYNC_REQUEST_COUNTER"), False
                )
            ...
            self._clipboard_atom = xlib.XInternAtom(
                self._x_display, asbytes("CLIPBOARD"), False
            )
            self._utf8_atom = xlib.XInternAtom(
                self._x_display, asbytes("UTF8_STRING"), False
            )
            self._target_atom = xlib.XInternAtom(
                self._x_display, asbytes("TARGETS"), False
            )
        ...
        xlib.XChangeWindowAttributes(
            self._x_display, self._window, attributes_mask, byref(attributes)
        )
        ...
        name = xlib.XInternAtom(
            self._x_display, asbytes("_NET_WM_BYPASS_COMPOSITOR"), False
        )
        ...
        if _have_utf8 and not self._x_ic:
            if not self.display._x_im:  # noqa: SLF001
                ...
                self.display._x_im = xlib.XOpenIM(
                    self._x_display, None, None, None
                )  # noqa: SLF001
            ...
            self._x_ic = xlib.XCreateIC(
                self.display._x_im,  # noqa: SLF001
                asbytes("inputStyle"),
                xlib.XIMPreeditNothing | xlib.XIMStatusNothing,
                asbytes("clientWindow"),
                self._window,
                asbytes("focusWindow"),
                self._window,
                None,
    ...
    def set_mouse_passthrough(self, state: bool) -> None:
        ...
        if state:
            ...
            xsync.XShapeCombineMask(
                self._x_display, self._window, xsync.ShapeInput, 0, 0, 0, xsync.ShapeSet
            )
    ...
    def activate(self) -> None:
        ...
        if self._x_display and self._window:
            xlib.XSetInputFocus(
                self._x_display, self._window, xlib.RevertToParent, xlib.CurrentTime
            )
    ...
    def _create_cursor_from_image(self, cursor: ImageMouseCursor) -> xlib.Cursor:
        ...
        bitmap = xlib.XCreateBitmapFromData(
            self._x_display, self._window, bmp_data, width, height
        )
        mask = xlib.XCreateBitmapFromData(
            self._x_display, self._window, mask_data, width, height
        )
        ...
        cursor = xlib.XCreatePixmapCursor(
            self._x_display, bitmap, mask, white, black, hot_x, hot_y
        )
    ...
    def set_mouse_platform_visible(self, platform_visible: bool | None = None) -> None:
        ...
        if platform_visible is None:
            platform_visible = (
                self._mouse_visible and not self._mouse_cursor.gl_drawable
            )
        ...
        if platform_visible is False:
            ...
            black = xlib.XColor()
            bitmap = xlib.XCreateBitmapFromData(
                self._x_display, self._window, bytes(8), 8, 8
            )
            cursor = xlib.XCreatePixmapCursor(
                self._x_display, bitmap, bitmap, black, black, 0, 0
            )
            xlib.XDefineCursor(self._x_display, self._window, cursor)
            xlib.XFreeCursor(self._x_display, cursor)
            xlib.XFreePixmap(self._x_display, bitmap)
        elif (
            isinstance(self._mouse_cursor, ImageMouseCursor)
            and self._mouse_cursor.hw_drawable
        ):
            ...
            if isinstance(self._mouse_cursor, XlibMouseCursor):
                xlib.XDefineCursor(
                    self._x_display, self._window, self._mouse_cursor.cursor
                )
            else:
    ...
    def _update_exclusivity(self) -> None:
        ...
        if mouse_exclusive != self._applied_mouse_exclusive:
            if mouse_exclusive:
                ...
                if (
                    isinstance(self.screen, XlibScreenXinerama)
                    and self.screen._xinerama
                ):
                    return
    ...
    def get_system_mouse_cursor(
        self, name: str
    ) -> DefaultMouseCursor | XlibMouseCursor:
    ...
    def set_icon(self, *images: pyglet.image.ImageData) -> None:
        ...
        for image in images:
            ...
            memmove(
                s,
                cast((c_ulong * 2)(image.width, image.height), POINTER(c_ubyte)),
                len(s),
            )
    ...
    def set_clipboard_text(self, text: str) -> None:
        xlib.XSetSelectionOwner(
            self._x_display, self._clipboard_atom, self._window, xlib.CurrentTime
        )

        if (
            xlib.XGetSelectionOwner(self._x_display, self._clipboard_atom)
            == self._window
        ):
            ...
            xlib.XChangeProperty(
                ...
                8,
                xlib.PropModeReplace,
                (c_ubyte * size).from_buffer_copy(str_bytes),
                size,
            )
        else:
            if _debug:
    ...
    def get_clipboard_text(self) -> str:
        ...
        if owner == self._window:
            data, size, actual_atom = self.get_single_property(
                self._window, self._clipboard_atom, self._utf8_atom
            )
        else:
            ...
            while not xlib.XCheckTypedWindowEvent(
                self._x_display, self._window, xlib.SelectionNotify, byref(notification)
            ):
    ...
    def _set_text_property(
        self, name: str, value: str, allow_utf8: bool = True
    ) -> None:
    ...
    def _set_atoms_property(
        self, name: str, values: list[str], mode: int = xlib.PropModeReplace
    ) -> None:
        name_atom = xlib.XInternAtom(self._x_display, asbytes(name), False)
        atoms = [
            xlib.XInternAtom(self._x_display, asbytes(value), False) for value in values
        ]
        ...
        if len(atoms):
            ...
            net_wm_state = xlib.XInternAtom(
                self._x_display, asbytes("_NET_WM_STATE"), False
            )
            if net_wm_state:
    ...
    def _set_wm_state(self, *states: str) -> None:
        ...
        atoms = [
            xlib.XInternAtom(self._x_display, asbytes(state), False) for state in states
        ]
        atom_type = xlib.XInternAtom(self._x_display, asbytes("ATOM"), False)
        if len(atoms):
            atoms_ar = (xlib.Atom * len(atoms))(*atoms)
    ...
    def dispatch_events(self) -> None:
        ...
        while xlib.XCheckTypedWindowEvent(
            _x_display, _window, xlib.ClientMessage, byref(e)
        ):
            self.dispatch_platform_event(e)
    ...
    def _event_text_symbol(self, ev: xlib.XEvent) -> tuple[str | None, int]:
        ...
        # keyrelease)
        count = xlib.XLookupString(
            ev.xkey, buffer, len(buffer) - 1, byref(symbol), None
        )
    ...
    def _event_key_view(self, ev: xlib.XEvent) -> None:
        ...
        if ev.type == xlib.KeyPress:
            if symbol and (
                not _can_detect_autorepeat or symbol not in self.pressed_keys
            ):
    ...
    def _event_drag_drop(self, ev: xlib.XEvent) -> None:
        ...
        if self._xdnd_format:
            ...
            xlib.XSendEvent(
                self._x_display, self._xdnd_source, False, xlib.NoEventMask, byref(e)
            )
    ...
    def _event_drag_enter(self, ev: xlib.XEvent) -> None:
        ...
        if three_or_more:
            data, count, _ = self.get_single_property(
                self._xdnd_source, self._xdnd_atoms["XdndTypeList"], XA_ATOM
            )
    ...
    def get_single_property(
        self, window: xlib.Window, atom_property: xlib.Atom, atom_type: int
    ) -> tuple[_Pointer[c_ubyte], int, int]:
    ...
    def _event_selection_notification(self, ev: xlib.XEvent) -> None:
        if (
            ...
            and ev.xselection.selection == self._xdnd_atoms["XdndSelection"]
        ):
            if self._xdnd_format:
                ...
                xlib.XSendEvent(
                    self._x_display, self._get_root(), False, xlib.NoEventMask, byref(e)
                )
                ...
                self.dispatch_event(
                    "on_file_drop",
                    self._xdnd_position[0],
                    self._height - self._xdnd_position[1],
                    formatted_paths,
                )
    ...
    def _sync_resize(self) -> None:
        if self._enable_xsync and self._current_sync_valid:
            if xsync.XSyncValueIsZero(self._current_sync_value):
                ...
                return
            xsync.XSyncSetCounter(
                self._x_display, self._sync_counter, self._current_sync_value
            )
    ...
    def _event_selection_request(self, ev: xlib.XEvent) -> None:
        ...
        if (
            xlib.XGetSelectionOwner(self._x_display, self._clipboard_atom)
            == self._window
            and ev.xselection.target == self._clipboard_atom
        ):
            if request.target == self._target_atom:
                ...
                xlib.XChangeProperty(
                    ...
                    xlib.PropModeReplace,
                    ptr,
                    sizeof(atoms_ar) // sizeof(c_ulong),
                )
                ...
                xlib.XChangeProperty(
                    ...
                    8,
                    xlib.PropModeReplace,
                    (c_ubyte * size).from_buffer_copy(text),
                    size,
                )
