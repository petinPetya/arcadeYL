...
class ImageMouseCursor(MouseCursor):
    ...
    def draw(self, x: int, y: int) -> None:
        ...
        self.texture.blit(
            (x - self.hot_x) / self.scaling, (y - self.hot_y) / self.scaling, 0
        )
...
class _WindowMetaclass(type):
    ...
    def __init__(
        cls: type[_WindowMetaclass], name: str, bases: tuple, dct: dict
    ) -> None:
...
class BaseWindow(EventDispatcher, metaclass=_WindowMetaclass):
        ...
        if not config:
            ...
            for template_config in [
                gl.Config(
                    ...
                    transparent_framebuffer=transparent_fb,
                ),
                gl.Config(
                    double_buffer=True,
                    depth_size=16,
    ...
    def draw_mouse_cursor(self) -> None:
        ...
        if (
            self._mouse_cursor.gl_drawable
            and self._mouse_visible
            and self._mouse_in_window
        ):
    ...
    def set_fullscreen(
        ...
        if (
            ...
            and (screen is None or screen is self._screen)
    ...
    def _set_fullscreen_mode(
        self, mode: ScreenMode, width: int, height: int
    ) -> tuple[int, int]:
        if mode is not None:
...
class FPSDisplay:
    ...
    def __init__(
        ...
        color: tuple[int, int, int, int] = (127, 127, 127, 127),
        samples: int = 240,
    ) -> None:
...
if _is_pyglet_doc_run:
    ...
    for _, method_obj in inspect.getmembers(Window, predicate=inspect.isfunction):
        method_obj.__qualname__ = method_obj.__qualname__.replace(
            "BaseWindow", "Window"
        )
