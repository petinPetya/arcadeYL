...
class Win32Window(BaseWindow):
    ...
    def _create(self) -> None:
        ...
        if self._fullscreen:
            ...
            styles = {
                ...
                self.WINDOW_STYLE_TRANSPARENT: (
                    constants.WS_OVERLAPPEDWINDOW,
                    constants.WS_EX_LAYERED,
                ),
                self.WINDOW_STYLE_OVERLAY: (
                    constants.WS_POPUP,
        ...
        if self._fullscreen:
            ...
            width, height = self._client_to_window_size(
                self._width, self._height, self._dpi
            )
        ...
        if not self._window_class:
            ...
            self._window_class.style = (
                constants.CS_VREDRAW | constants.CS_HREDRAW | constants.CS_OWNDC
            )
            self._window_class.hInstance = 0
            self._window_class.hIcon = _user32.LoadImageW(
                module,
                MAKEINTRESOURCE(1),
                constants.IMAGE_ICON,
                0,
                0,
                constants.LR_DEFAULTSIZE | constants.LR_SHARED,
            )
        ...
        if not self._hwnd:
            ...
            if self._file_drops:
                ...
                if constants.WINDOWS_7_OR_GREATER:
                    _user32.ChangeWindowMessageFilterEx(
                        self._hwnd, constants.WM_DROPFILES, constants.MSGFLT_ALLOW, None
                    )
                    _user32.ChangeWindowMessageFilterEx(
                        self._hwnd, constants.WM_COPYDATA, constants.MSGFLT_ALLOW, None
                    )
                    _user32.ChangeWindowMessageFilterEx(
                        self._hwnd,
                        constants.WM_COPYGLOBALDATA,
                        constants.MSGFLT_ALLOW,
                        None,
                    )
            ...
            _user32.SetWindowLongW(self._hwnd, constants.GWL_STYLE, self._ws_style)
        ...
        if self._fullscreen:
            ...
            _user32.SetWindowPos(
                self._hwnd,
                constants.HWND_NOTOPMOST,
                0,
                0,
                width,
                height,
                constants.SWP_NOMOVE | constants.SWP_FRAMECHANGED,
            )
    ...
    def _update_view_location(self, width: int, height: int) -> None:
        ...
        _user32.SetWindowPos(
            ...
            width,
            height,
            constants.SWP_NOZORDER | constants.SWP_NOOWNERZORDER,
        )
    ...
    def set_vsync(self, vsync: bool) -> None:
        ...
        if not self._fullscreen and (
            self._always_dwm or self._dwm_composition_enabled()
        ):
            vsync = 0
    ...
    def flip(self) -> None:
        ...
        if (
            ...
            and (self._always_dwm or self._dwm_composition_enabled())
            and self._interval
        ):
            _dwmapi.DwmFlush()
    ...
    def set_mouse_passthrough(self, state: bool) -> None:
        ...
        if self._ex_ws_style & constants.WS_EX_LAYERED:
            _user32.GetLayeredWindowAttributes(
                self._hwnd, byref(color_ref), byref(alpha), byref(flags)
            )
        ...
        if state:
            ...
            if (
                ...
                and not flags.value & constants.LWA_ALPHA
            ):
                self._ex_ws_style &= ~constants.WS_EX_LAYERED
        ...
        if state:
            _user32.SetLayeredWindowAttributes(
                self._hwnd, color_ref.value, alpha.value, flags.value
            )
    ...
    def get_window_screen(self):
        ...
        handle = _user32.MonitorFromWindow(
            self._hwnd, constants.MONITOR_DEFAULTTONEAREST
        )
        return [
            screen for screen in self.display.get_screens() if screen._handle == handle
        ][0]
    ...
    def set_mouse_platform_visible(self, platform_visible: bool | None = None) -> None:
        if platform_visible is None:
            platform_visible = (
                self._mouse_visible
                and not self._exclusive_mouse
                and (
                    not self._mouse_cursor.gl_drawable or self._mouse_cursor.hw_drawable
                )
            ) or (not self._mouse_in_window or not self._has_focus)
    ...
    def set_exclusive_mouse(self, exclusive: bool = True) -> None:
        if (
            ...
            and self._exclusive_mouse_focus == self._has_focus
        ):
            return
    ...
    def set_mouse_position(self, x: int, y: int, absolute: bool = False) -> None:
        if not absolute:
            rect = RECT()
            _user32.GetClientRect(self._view_hwnd, byref(rect))
            _user32.MapWindowPoints(
                self._view_hwnd, constants.HWND_DESKTOP, byref(rect), 2
            )
    ...
    def set_exclusive_keyboard(self, exclusive: bool = True) -> None:
        if (
            ...
            and self._exclusive_keyboard_focus == self._has_focus
        ):
            return

        if exclusive and self._has_focus:
            _user32.RegisterHotKey(
                self._hwnd, 0, constants.WIN32_MOD_ALT, constants.VK_TAB
            )
    ...
    def get_system_mouse_cursor(
        self, name: str
    ) -> DefaultMouseCursor | Win32MouseCursor:
    ...
    def set_icon(self, *images: pyglet.image.ImageData) -> None:
        ...
        def best_image(width: int, height: int) -> pyglet.image.ImageData:
            ...
            for img in images:
                ...
                if (
                    img.width >= width
                    and img.width * img.height > b_image.width * b_image.height
                ):
        ...
        def get_icon(img: pyglet.image.ImageData) -> HICON:
            ...
            bitmap = _gdi32.CreateDIBSection(
                hdc, byref(header), constants.DIB_RGB_COLORS, byref(dataptr), None, 0
            )
        ...
        # Set large icon
        image = best_image(
            _user32.GetSystemMetrics(constants.SM_CXICON),
        ...
        # Set small icon
        image = best_image(
            _user32.GetSystemMetrics(constants.SM_CXSMICON),
    ...
    def set_clipboard_text(self, text: str) -> None:
        ...
        memmove(
            locked_data, text, size
        )  # Trying to encode in utf-16 causes garbled text. Accepts str fine?
    ...
    def _client_to_window_size(
        ...
        if constants.WINDOWS_10_ANNIVERSARY_UPDATE_OR_GREATER:
            _user32.AdjustWindowRectExForDpi(
                byref(rect), self._ws_style, False, self._ex_ws_style, dpi
            )
        else:
            _user32.AdjustWindowRectEx(
                byref(rect), self._ws_style, False, self._ex_ws_style
            )
    ...
    def _get_window_proc(
        self, event_handlers: dict
    ) -> Callable[[HWND, MSG, int, int], int | None]:
        def f(hwnd: HWND, msg: MSG, wParam: int, lParam: int) -> int | None:
            ...
            if event_handler:
                if (
                    ...
                    or not self._enable_event_queue
                    or msg in _priority_events
                ):
    ...
    def _event_key(self, msg: int, wParam: int, lParam: int) -> int | None:
        ...
        if (symbol, ctrl) in _motion_map and msg not in (
            constants.WM_KEYUP,
            constants.WM_SYSKEYUP,
        ):
        ...
        if inp.header.dwType == constants.RIM_TYPEMOUSE:
            ...
            if rmouse.usFlags & 0x01 == constants.MOUSE_MOVE_RELATIVE:
                if rmouse.lLastX != 0 or rmouse.lLastY != 0:
                    ...
                    if self._exclusive_mouse_buttons:
                        ...
                        self.dispatch_event(
                            "on_mouse_motion", 0, 0, rmouse.lLastX, -rmouse.lLastY
                        )
            else:
            ...
            if inp.data.keyboard.MakeCode == 0x02A:  # LEFT_SHIFT
                if not key_up and not self._keyboard_state[0x02A]:
                    ...
                    self.dispatch_event(
                        "on_key_press", key.LSHIFT, self._get_modifiers()
                    )

                elif key_up and self._keyboard_state[0x02A]:
                    self._keyboard_state[0x02A] = False
                    self.dispatch_event(
                        "on_key_release", key.LSHIFT, self._get_modifiers()
                    )
                ...
                if not key_up and not self._keyboard_state[0x036]:
                    ...
                    self.dispatch_event(
                        "on_key_press", key.RSHIFT, self._get_modifiers()
                    )

                elif key_up and self._keyboard_state[0x036]:
                    self._keyboard_state[0x036] = False
                    self.dispatch_event(
                        "on_key_release", key.RSHIFT, self._get_modifiers()
                    )
    ...
    def _event_mousemove(self, msg: int, wParam: int, lParam: int) -> int:
        ...
        if not self._tracking:
            ...
            self.set_mouse_platform_visible()
            self.dispatch_event(
                "on_mouse_enter", x / self._mouse_scale, y / self._mouse_scale
            )
            self._tracking = True
        ...
        if buttons:
            ...
            self.dispatch_event(
                ...
                x / self._mouse_scale,
                y / self._mouse_scale,
                dx / self._mouse_scale,
                dy / self._mouse_scale,
                buttons,
                modifiers,
            )
        else:
    ...
    def _event_mouseleave(self, msg: int, wParam: int, lParam: int) -> int:
        ...
        self.set_mouse_platform_visible()
        self.dispatch_event(
            "on_mouse_leave", x / self._mouse_scale, y / self._mouse_scale
        )
        return 0
    ...
    def _event_mousewheel(self, msg: int, wParam: int, lParam: int) -> int:
        ...
        self.dispatch_event(
            ...
            self._mouse_y,
            0,
            delta / float(constants.WHEEL_DELTA),
        )
        return 0
    ...
    def _event_setcursor(self, msg: int, wParam: int, lParam: int) -> int | None:
        if self._exclusive_mouse and not self._mouse_platform_visible:
            ...
            if lo == constants.HTCLIENT:  # In frame
                ...
                constants.HTCAPTION,
                constants.HTCLOSE,
                constants.HTMAXBUTTON,  # noqa: RET505
                constants.HTMINBUTTON,
            ):  # Allow in
    ...
    def _event_dpi_scaled_size(self, msg: int, wParam: int, lParam: int) -> int | None:
        ...
        if constants.WINDOWS_10_CREATORS_UPDATE_OR_GREATER:
            ...
            _user32.AdjustWindowRectExForDpi(
                byref(result), self._ws_style, False, self._ex_ws_style, dpi
            )
    ...
    def _event_dpi_change(self, msg: int, wParam: int, lParam: int) -> int:
        ...
        if not self._fullscreen and (
            ...
            or constants.WINDOWS_10_CREATORS_UPDATE_OR_GREATER
        ):
