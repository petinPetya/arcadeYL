...
def _get_segment(
    ...
    thickness: float = 1.0,
    prev_miter: Vec2 | None = None,
    prev_scale: Vec2 | None = None,
) -> tuple[
    Vec2,
    Vec2,
    float,
    ...
    if prev_miter and prev_scale:
        ...
        v_miter1 = Vec2(
            v_normal_p0p1.x + v_normal.x, v_normal_p0p1.y + v_normal.y
        ).normalize()
    ...
    if p3:
        ...
        # Add the 2 normal vectors and normalize to get miter vector
        v_miter2 = Vec2(
            v_normal_p2p3.x + v_normal.x, v_normal_p2p3.y + v_normal.y
        ).normalize()
...
class ShapeBase(ABC):
    ...
    def get_shape_group(self) -> _ShapeGroup | Group:
        ...
        return self.group_class(
            self._blend_src, self._blend_dest, self._program, self._user_group
        )
    ...
    def _create_vertex_list(self) -> None:
        ...
        """
        raise NotImplementedError(
            "_create_vertex_list must be defined in order to use group or batch properties"
        )
    ...
    def program(self, program: ShaderProgram) -> None:
        ...
        if self._batch and self._batch.update_shader(
            self._vertex_list, GL_TRIANGLES, self._group, program
        ):
...
class Arc(ShapeBase):
    ...
    def _get_vertices(self) -> Sequence[float]:
        ...
        for i in range(len(points) - 1):
            ...
            prev_miter, prev_scale, *segment = _get_segment(
                prev_point,
                points[i],
                points[i + 1],
                next_point,
...
class Circle(ShapeBase):
    ...
    def __contains__(self, point: tuple[float, float]) -> bool:
        assert len(point) == 2
        return (
            math.dist((self._x - self._anchor_x, self._y - self._anchor_y), point)
            < self._radius
        )
...
class Ellipse(ShapeBase):
    ...
    def _get_vertices(self) -> Sequence[float]:
        ...
        points = [
            (x + self._a * math.cos(i * tau_segs), y + self._b * math.sin(i * tau_segs))
            for i in range(self._segments)
        ]
...
class Sector(ShapeBase):
    ...
    def __contains__(self, point: tuple[float, float]) -> bool:
        ...
        point = _rotate_point((self._x, self._y), point, math.radians(self._rotation))
        angle = math.atan2(
            point[1] - self._y + self._anchor_y, point[0] - self._x + self._anchor_x
        )
        if angle < 0:
            angle += 2 * math.pi
...
class RoundedRectangle(pyglet.shapes.ShapeBase):
    ...
    def _set_radius(
        self, radius: _RadiusT | tuple[_RadiusT, _RadiusT, _RadiusT, _RadiusT]
    ) -> None:
    ...
    def _get_vertices(self) -> Sequence[float]:
        ...
        arc_positions = [
            # bottom-left
            (x + self._radius[0][0], y + self._radius[0][1], math.pi * 3 / 2),
            # top-left
            (x + self._radius[1][0], y + self._height - self._radius[1][1], math.pi),
            # top-right
            (
                x + self._width - self._radius[2][0],
                y + self._height - self._radius[2][1],
                math.pi / 2,
            ),
            # bottom-right
            (x + self._width - self._radius[3][0], y + self._radius[3][1], 0),
        ]

        for (rx, ry), (arc_x, arc_y, arc_start), segments in zip(
            self._radius, arc_positions, self._segments
        ):
            tau_segs = -math.pi / 2 / segments
            points.extend(
                [
                    (
                        arc_x + rx * math.cos(i * tau_segs + arc_start),
                        arc_y + ry * math.sin(i * tau_segs + arc_start),
                    )
    ...
    @radius.setter
    def radius(
        self, value: _RadiusT | tuple[_RadiusT, _RadiusT, _RadiusT, _RadiusT]
    ) -> None:
...
class MultiLine(ShapeBase):
    ...
    def _get_vertices(self) -> Sequence[float]:
        ...
        coords: list[list[float]] = [
            [x - trans_x, y - trans_y] for x, y in self._coordinates
        ]
