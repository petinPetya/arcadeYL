...
DebugLibrary = lambda lib: ctypes.WinDLL(
    lib, use_last_error=True if _debug_win32 else False
)
...
_ole32.CoSetProxyBlanket.argtypes = (
    c_void_p,
    DWORD,
    DWORD,
    c_void_p,
...
if _debug_win32:
    ...
    def win32_errcheck(result, func, args):
        ...
        if last_err != 0:  # If the result is not success and last error is invalid.
            ...
            print(
                f"[Result {result}] Error #{last_err} - {ctypes.FormatError(last_err)}",
                file=_log_win32,
            )
        return args
...
try:
    ...
    if err.winerror == constants.RPC_E_CHANGED_MODE:
        warnings.warn(
            "COM mode set by another library in a different mode. Unexpected behavior may occur."
        )
    else:
