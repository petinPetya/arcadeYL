...
def cfarray_to_list(cfarray):
    ...
    return [
        cftype_to_value(c_void_p(cf.CFArrayGetValueAtIndex(cfarray, i)))
...
NSApplicationDidUpdateNotification = c_void_p.in_dll(
    appkit, "NSApplicationDidUpdateNotification"
)
NSPasteboardURLReadingFileURLsOnlyKey = c_void_p.in_dll(
    appkit, "NSPasteboardURLReadingFileURLsOnlyKey"
)
...
NSTrackingInVisibleRect = (
    0x200  # If set, tracking occurs in visibleRect of view and rect is ignored
)
...
quartz.CGContextShowGlyphsAtPositions.argtypes = [
    CGContextRef,
    POINTER(CGGlyph),
    POINTER(CGPoint),
    c_size_t,
...
quartz.CTFontDrawGlyphs.argtypes = [
    CTFontRef,
    POINTER(CGGlyph),
    POINTER(CGPoint),
    c_size_t,
...
kCTForegroundColorFromContextAttributeName = c_void_p.in_dll(
