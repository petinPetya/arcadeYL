...
class Screen(abc.ABC):
    ...
    def __init__(
        self, display: Display, x: int, y: int, width: int, height: int
    ) -> None:
    ...
    def get_best_config(self, template: Config = None) -> Config:
        ...
        if template is None:
            for template_config in [
                gl.Config(
                    double_buffer=True, depth_size=24, major_version=3, minor_version=3
                ),
                gl.Config(
                    double_buffer=True, depth_size=16, major_version=3, minor_version=3
                ),
                None,
            ]:
                try:
    ...
    def get_closest_mode(self, width: int, height: int) -> ScreenMode:
        ...
        for mode in self.get_modes():
            ...
            if (
                ...
                and mode.height <= best.height
                and (mode.width < best.width or mode.height < best.height)
            ):
                best = mode
