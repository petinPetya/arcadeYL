...
class Win32Display(Display):  # noqa: D101
    ...
    def get_screens(self) -> list[Win32Screen]:
        ...
        def enum_proc(
            hMonitor: HMONITOR, hdcMonitor: HDC, lprcMonitor: LPRECT, dwData: LPARAM
        ) -> bool:  # noqa: N803, ARG001
...
class Win32Screen(Screen):  # noqa: D101
    ...
    def _get_friendly_name_display_config_api(self) -> str:
        ...
        for i in range(path_count.value):
            ...
            result = _user32.DisplayConfigGetDeviceInfo(
                ctypes.byref(source_name.header)
            )
            if result != 0:
                continue
            ...
            if (
                _user32.DisplayConfigGetDeviceInfo(ctypes.byref(target_name.header))
                == 0
            ):
    ...
    def get_mode(self) -> Win32ScreenMode:
        mode = DEVMODE()
        mode.dmSize = sizeof(DEVMODE)
        _user32.EnumDisplaySettingsW(
            self.get_device_name(), ENUM_CURRENT_SETTINGS, byref(mode)
        )
    ...
    def set_mode(self, mode: Win32ScreenMode) -> None:
        ...
        r = _user32.ChangeDisplaySettingsExW(
            self.get_device_name(), byref(mode._mode), None, CDS_FULLSCREEN, None
        )
...
class Win32ScreenMode(ScreenMode):  # noqa: D101
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(width={self.width!r}, height={self.height!r}, depth={self.depth!r},"
...
class Win32Canvas(Canvas):  # noqa: D101
    def __init__(
        self, display: Win32Display, hwnd: HWND, hdc: HDC
    ) -> None:  # noqa: D107
