...
class XlibDisplay(XlibSelectDevice, Display):
    ...
    def __init__(self, name=None, x_screen=None):
        ...
        if _have_xsync:
            event_base = c_int()
            error_base = c_int()
            if xsync.XSyncQueryExtension(
                self._display, byref(event_base), byref(error_base)
            ):
                major_version = c_int()
                minor_version = c_int()
                if xsync.XSyncInitialize(
                    self._display, byref(major_version), byref(minor_version)
                ):
    ...
    def get_screens(self) -> list[XlibScreen]:
        ...
        if _have_xrandr:
            ...
            if res_ptr:
                ...
                for i in range(res.noutput):
                    ...
                    output_info_ptr = xrandr.XRRGetOutputInfo(
                        self._display, res_ptr, output_id
                    )
                    ...
                    crtc_info_ptr = xrandr.XRRGetCrtcInfo(
                        self._display, res_ptr, output_info.crtc
                    )
                    ...
                    self._screens.append(
                        XlibScreenXrandr(
                            ...
                            output_id,
                            ctypes.string_at(
                                output_info.name, output_info.nameLen
                            ).decode(),
                            output_id == primary,
                        ),
                    )
        ...
        if (
            not self._screens
            and _have_xinerama
            and xinerama.XineramaIsActive(self._display)
        ):
            number = c_int()
            infos = xinerama.XineramaQueryScreens(self._display, byref(number))
            infos = cast(
                infos, POINTER(xinerama.XineramaScreenInfo * number.value)
            ).contents
            ...
            screen = XlibScreen(
                self, 0, 0, screen_info.contents.width, screen_info.contents.height
            )
...
class XlibScreen(Screen):
    ...
    def set_mode(self, mode: XlibScreenModeXF86):
        ...
        xf86vmode.XF86VidModeSwitchToMode(
            self.display._display, self.display.x_screen, mode.info
        )
        xlib.XFlush(self.display._display)
        xf86vmode.XF86VidModeSetViewPort(
            self.display._display, self.display.x_screen, 0, 0
        )
        xlib.XFlush(self.display._display)
...
class XlibScreenXrandr(XlibScreen):
    ...
    @staticmethod
    def _get_mode_info(
        resource: xrandr.XRRScreenResources, rrmode_id: int
    ) -> xrandr.XRRModeInfo | None:
    ...
    def set_mode(self, mode: XlibScreenModeXrandr) -> None:
        ...
        if res_ptr:
            crtc_info_ptr = xrandr.XRRGetCrtcInfo(
                self.display._display, res_ptr, self.crtc_id
            )
            if crtc_info_ptr:
    ...
    def get_modes(self) -> list[XlibScreenModeXrandr]:
        ...
        res_ptr = xrandr.XRRGetScreenResourcesCurrent(self.display._display, root)
        output_info_ptr = xrandr.XRRGetOutputInfo(
            self.display._display, res_ptr, self.output_id
        )
...
class XlibScreenMode(ScreenMode):
    def __init__(
        self, screen: XlibScreen, width: int, height: int, rate: int, depth: int
    ):
...
class XlibScreenModeXF86(XlibScreenMode):
    def __init__(
        self, screen: XlibScreen, info: xf86vmode.XF86VidModeModeInfo, depth: int
    ) -> None:
        self.info = info
        width = info.hdisplay
...
class XlibScreenModeXrandr(XlibScreenMode):
    def __init__(
        ...
        super().__init__(
            ...
            mode_info.width,
            mode_info.height,
            self._calculate_refresh_rate(mode_info),
            depth,
        )
...
class XlibCanvas(Canvas):  # noqa: D101
    ...
    def __init__(
        self, display: XlibDisplay, x_window: xlib.Window
    ) -> None:  # noqa: D107
