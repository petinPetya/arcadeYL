...
class ResourceNotFoundException(Exception):
    ...
    def __init__(self, name):
        message = (
            f"Resource '{name}' was not found on the path.  "
...
class UndetectableShaderType(Exception):
    ...
    def __init__(self, name):
        message = (
            f"The Shader type of '{name}' could not be determined. "
...
class ZIPLocation(Location):
    ...
    def open(self, filename: str, mode="rb") -> BytesIO | StringIO:
        ...
        _forward_slash_path = _path.replace(
            os.sep, "/"
        )  # zip can only handle forward slashes
...
class Loader:
    ...
    def __init__(
        self, pathlist: list[str] | None = None, script_home: str | None = None
    ) -> None:
    ...
    def reindex(self):
        ...
        for _path_name in self.path:
            ...
            if _path_name.startswith("@"):
                ...
                assert (
                    r"\\" not in _path_name
                ), "Backslashes are not permitted in relative paths"
            ...
            if os.path.isdir(_path_name):
                ...
                for dirpath, dirnames, filenames in os.walk(_path_name):
                    ...
                    if dirpath:
                        parts = [
                            part for part in dirpath.split(os.sep) if part is not None
                        ]
                        dirpath = "/".join(parts)
    ...
    def _get_texture_atlas_bin(
        self, width: int, height: int, border: int
    ) -> TextureBin | None:
    ...
    def image(
        ...
        if name in self._cached_images:
            ...
            identity = self._cached_images[name] = self._alloc_image(
                name, atlas, border
            )
    ...
    def animation(
        ...
        try:
            ...
            texture_bin = self._get_texture_atlas_bin(
                _animation.get_max_width(), _animation.get_max_height(), border
            )
            if texture_bin:
