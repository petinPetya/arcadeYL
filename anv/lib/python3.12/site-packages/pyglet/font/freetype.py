...
class FreeTypeGlyphRenderer(base.GlyphRenderer):
    ...
    def _convert_mono_to_gray_bitmap(self) -> None:
        if self._bitmap.buffer:
            bitmap_data = cast(
                self._bitmap.buffer, POINTER(c_ubyte * (self._pitch * self._height))
            ).contents
...
class FreeTypeFont(base.Font):
    ...
    def _load_font_face_from_system(self) -> None:
        match = get_fontconfig().find_font(
            self._name, self.size, self.weight, self.italic, self.stretch
        )
        if not match:
    ...
    def get_glyphs(
        self, text: str
    ) -> tuple[list[base.Glyph], list[base.GlyphPosition]]:
...
class FreeTypeFace:
    ...
    def set_char_size(self, size: float, dpi: int) -> bool:
        if self.face_flags & FT_FACE_FLAG_SCALABLE:
            ...
            if self.ft_face.contents.num_fixed_sizes:
                ...
                warnings.warn(
                    f"{self.name} no fixed sizes, but is flagged as a fixed size font."
                )
    ...
    def get_font_metrics(self, size: float, dpi: int) -> FreeTypeFontMetrics:
        if self.set_char_size(size, dpi):
            ...
            return FreeTypeFontMetrics(
                ascent=int(f26p6_to_float(metrics.ascender)),
...
class FreeTypeMemoryFace(FreeTypeFace):
    def __init__(self, data: bytes, face_index: int = 0) -> None:
        ...
        ft_face = FT_Face()
        FT_New_Memory_Face(
            ft_library, self.font_data, len(self.font_data), face_index, byref(ft_face)
        )
