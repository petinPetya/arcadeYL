...
class FontConfig:
    _search_cache: OrderedDict[
        tuple[str, float, str, bool, str], FontConfigSearchResult
    ]
    ...
    def _add_to_search_cache(
        self,
        search_pattern: FontConfigSearchPattern,
        ...
        self._search_cache[
            (
                ...
                search_pattern.stretch,
            )
        ] = result_pattern
        if len(self._search_cache) > self._cache_size:
            self._search_cache.popitem(last=False)[1].dispose()

    def _get_from_search_cache(
        self, name: str, size: float, weight: str, italic: bool, stretch: str
    ) -> FontConfigSearchResult | None:
    ...
    def _load_fontconfig_library() -> CDLL:
        ...
        fontconfig.FcFreeTypeQueryFace.argtypes = [
            c_void_p,
            c_char_p,
            c_int,
            POINTER(c_int),
...
class FontConfigPattern:
    ...
    def _get_value(self, name: bytes) -> FcValue | None:
        ...
        value = FcValue()
        result: FcResult = self._fontconfig.FcPatternGet(
            self._pattern, name, 0, byref(value)
        )
...
class FontConfigSearchPattern(FontConfigPattern):
    ...
    def _get_match(self) -> c_void_p | None:
        ...
        match_pattern = self._fontconfig.FcFontMatch(
            0, self._pattern, byref(match_result)
        )
