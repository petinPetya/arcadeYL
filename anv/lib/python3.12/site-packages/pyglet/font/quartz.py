...
if harfbuzz_available():
    ...
    def py_coretext_table_callback(face: c_void_p, tag: int, user_data: c_void_p):
        ...
        blob = hb_lib.hb_blob_create(
            ...
            HB_MEMORY_MODE_READONLY,
            table_data,
            py_coretext_table_data_destroy_c,
        )
        return blob
...
class QuartzGlyphRenderer(base.GlyphRenderer):
    ...
    def render_index(self, glyph_index: int):
        ...
        quartz.CGContextSetShouldAntialias(
            bitmap_context, pyglet.options.text_antialiasing
        )
        ...
        if buffer_ptr:
            ...
            glyph_image = pyglet.image.ImageData(
                width, height, "RGBA", buffer, bytes_per_row
            )
    ...
    def render(self, text: str) -> base.Glyph:
        ...
        cf.CFDictionaryAddValue(
            attributes,
            cocoapy.kCTForegroundColorFromContextAttributeName,
            cocoapy.kCFBooleanTrue,
        )
        ...
        if glyphs[0] == 0:
            ...
            advance = width = int(
                ct.CTLineGetTypographicBounds(line, byref(ascent), byref(descent), None)
            )
        ...
        quartz.CGContextSetRGBFillColor(
            bitmap_context, 1, 1, 1, 1
        )  # Render white for multiplying.
        quartz.CGContextTranslateCTM(
            bitmap_context, 0, height
        )  # Move origin to top-left
        quartz.CGContextScaleCTM(bitmap_context, 1, -1)  # Flip vertically
...
class QuartzFont(base.Font):
    ...
    def __init__(
        ...
        if result:
            ...
            self.ctFont = c_void_p(
                ct.CTFontCreateWithGraphicsFont(cgFont, self.pixel_size, None, None)
            )
        else:
            # Create a font descriptor for given name and traits and use it to create font.
            descriptor = self._create_font_descriptor(
                name, traits, self.weight_value, self.stretch_value
            )
            self.ctFont = c_void_p(
                ct.CTFontCreateWithFontDescriptor(descriptor, self.pixel_size, None)
            )
    ...
    def _get_hb_face(self):
        ...
        return hb_lib.hb_face_create_for_tables(
            py_coretext_table_callback, self._cg_font, _destroy_callback_null
        )
    ...
    def get_font_data(self) -> bytes:
        ...
        if filename == "Unknown":
            ...
            if result:
                ...
                raise Exception(
                    "Couldn't load font data by name and traits. Report as a bug with the font file."
                )
        else:
    ...
    def filename(self) -> str:
        descriptor = self._create_font_descriptor(
            self.name, self.traits, self.weight_value, self.stretch_value
        )
        ref = c_void_p(
            ct.CTFontDescriptorCopyAttribute(descriptor, kCTFontURLAttribute)
        )
        if ref:
    ...
    def _lookup_font_with_family_and_traits(
        self, family: str, traits: int
    ) -> tuple[c_void_p, bytes] | None:
    ...
    def _create_font_descriptor(
        ...
        symTraits = c_void_p(
            cf.CFNumberCreate(None, cocoapy.kCFNumberSInt32Type, byref(itraits))
        )
        if symTraits:
            ...
            if traitsDict:
                if weight is not None:
                    ...
                    cfWeight = c_void_p(
                        cf.CFNumberCreate(
                            None, cocoapy.kCFNumberFloatType, byref(weight_value)
                        )
                    )
                    if cfWeight:
                        cf.CFDictionaryAddValue(
                            traitsDict, cocoapy.kCTFontWeightTrait, cfWeight
                        )
                ...
                if stretch is not None:
                    ...
                    cfWidth = c_void_p(
                        cf.CFNumberCreate(
                            None, cocoapy.kCFNumberFloatType, byref(stretch_value)
                        )
                    )
                    if cfWidth:
                        cf.CFDictionaryAddValue(
                            traitsDict, cocoapy.kCTFontWidthTrait, cfWidth
                        )
                        cf.CFRelease(cfWidth)

                # Add CFNumber traits to traits dictionary.
                cf.CFDictionaryAddValue(
                    traitsDict, cocoapy.kCTFontSymbolicTrait, symTraits
                )
                # Add traits dictionary to attributes.
                cf.CFDictionaryAddValue(
                    attributes, cocoapy.kCTFontTraitsAttribute, traitsDict
                )
    ...
    def _get_font_weight(self):
        ...
        font_weight = cfnumber_to_number(
            c_void_p(cf.CFDictionaryGetValue(traits, kCTFontWeightTrait))
        )
        cf.CFRelease(traits)
        return font_weight
    ...
    def _get_font_stretch(self):
        traits = ct.CTFontCopyTraits(self.ctFont)
        font_width = cfnumber_to_number(
            c_void_p(cf.CFDictionaryGetValue(traits, cocoapy.kCTFontWidthTrait))
        )
        cf.CFRelease(traits)
        return font_width
