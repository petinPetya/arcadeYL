...
_LOGICAL_ORDER_EXCEPTION = {chr(x) for x in range(0xE40, 0xE45)} | {
    chr(x) for x in range(0xEC0, 0xEC4)
}
...
class GlyphTextureAtlas(image.atlas.TextureAtlas):
    ...
    def __init__(
        ...
        height: int = 2048,
        fmt: int = GL_RGBA,
        min_filter: int = GL_LINEAR,  # noqa: D107
        mag_filter: int = GL_LINEAR,
    ) -> None:
        super().__init__(width, height)
        self.texture = self.texture_class.create(
            width, height, GL_TEXTURE_2D, fmt, min_filter, mag_filter, fmt=fmt
        )
...
class GlyphTextureBin(image.atlas.TextureBin):
    ...
    def add(
        ...
        atlas = GlyphTextureAtlas(
            self.texture_width, self.texture_height, fmt, min_filter, mag_filter
        )
    ...
    def create_glyph(self, img: image.AbstractImage, fmt: int | None = None) -> Glyph:
        ...
        if self.texture_bin is None:
            if self.optimize_fit:
                self.texture_width, self.texture_height = self._get_optimal_atlas_size(
                    img
                )
        ...
        return self.texture_bin.add(
            img,
            fmt or self.texture_internalformat,
            self.texture_min_filter,
            self.texture_mag_filter,
            border=1,
        )

    def _get_optimal_atlas_size(
        self, image_data: image.AbstractImage
    ) -> tuple[int, int]:
