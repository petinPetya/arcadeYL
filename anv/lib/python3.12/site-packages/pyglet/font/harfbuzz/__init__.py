...
def _get_fallback_glyph_shape(
    ...
    for fallback in font.fallbacks:
        ...
        for glyph_data in glyph_info:
            ...
            glyphs.append(glyph)
            offset = GlyphPosition(
                glyph_data["x_advance"] - glyph.advance,
                glyph_data["y_advance"],
                glyph_data["x_offset"],
                glyph_data["y_offset"],
            )
...
def get_harfbuzz_shaped_glyphs(
    font: Font, text: str
) -> tuple[list[Glyph], list[GlyphPosition]]:
    ...
    for cluster_id, cluster_ct in empty_clusters.items():
        ...
        if missing_text not in font.glyphs:
            ...
            font.glyphs[missing_text] = (
                fb_glyph_info  # Cache by string to prevent future fallback paths.
            )
    ...
    for i in range(len(text)):
        ...
        # If this cluster is missing, add some zero glyphs until count matches.
            ...
            for glyph_info in cluster_glyph_info:
                ...
                offsets.append(
                    GlyphPosition(
                        glyph_info["x_advance"] - glyph.advance,
                        glyph_info["y_advance"],
                        glyph_info["x_offset"],
                        glyph_info["y_offset"],
                    )
                )
...
class _HarfbuzzResources:
    ...
    def load_from_memory(self, data: bytes) -> None:
        ...
        self.blob = hb_lib.hb_blob_create(
            data_buf, data_len, HB_MEMORY_MODE_READONLY, None, None
        )
