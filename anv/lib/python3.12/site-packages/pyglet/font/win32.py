...
from pyglet.image.codecs.gdiplus import (
    BitmapData,
    ImageLockModeRead,
...
class GDIPlusGlyphRenderer(base.GlyphRenderer):
    ...
    def _create_bitmap(self, width: int, height: int) -> None:
        ...
        gdiplus.GdipSetTextRenderingHint(
            self._graphics, _text_quality[pyglet.options.text_antialiasing]
        )
    ...
    def render(self, text: str) -> Glyph:
        ...
        rect = Rectf(
            0, self._bitmap_height - self.font.ascent + self.font.descent, width, height
        )
        ...
        for codepoint in [ord(c) for c in text]:
            if gdi32.GetCharABCWidthsW(
                self._dc, codepoint, codepoint, ctypes.byref(abc)
            ):
                lsb += abc.abcA
                width += abc.abcB
        ...
        image = pyglet.image.ImageData(
            width, height, "BGRA", buffer, -bitmap_data.Stride
        )
...
class Win32Font(base.Font):
    ...
    @staticmethod
    def get_logfont(
        name: str, size: float, bold: bool, italic: bool, dpi: int | None = None
    ) -> LOGFONTW:
...
class GDIPlusFont(Win32Font):
    ...
    def __init__(
        ...
        if self._private_collection:
            gdiplus.GdipCreateFontFamilyFromName(
                name, self._private_collection, ctypes.byref(family)
            )
        ...
        if not family:
            if _debug_font:
                print(
                    f"Warning: Font '{name}' was not found. Defaulting to: {self._default_name}"
                )
        ...
        if not family:
            ...
            gdiplus.GdipCreateFontFamilyFromName(
                ctypes.c_wchar_p(self._name), None, ctypes.byref(family)
            )
        ...
        gdiplus.GdipCreateFont(
            family, ctypes.c_float(size), style, unit, ctypes.byref(self._gdipfont)
        )
    ...
    def have_font(cls: type[GDIPlusFont], name: str) -> bool:
        ...
        if cls._private_collection and _font_exists_in_collection(
            cls._private_collection, name
        ):
            return True
