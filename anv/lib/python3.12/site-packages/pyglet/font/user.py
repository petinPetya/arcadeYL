...
class UserDefinedGlyphRenderer(base.GlyphRenderer):
    ...
    def render(self, image_data: ImageData) -> Glyph:
        if self._font._scaling:  # noqa: SLF001
            image_original = Image.frombytes(
                "RGBA",
            ...
            image_resized = image_original.resize(
                (
                    int(image_data.width * scale_ratio),
                    int(image_data.height * scale_ratio),
                ),
                Resampling.NEAREST,
            )
...
def get_scaled_user_font(
    font_base: UserDefinedMappingFont, size: int
    ...
    """
    new_font = UserDefinedMappingFont(
        font_base.name,
        font_base.default_char,
        size,
        font_base.mappings,
        font_base.ascent,
        font_base.descent,
