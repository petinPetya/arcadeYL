...
from pyglet.font.harfbuzz import (
    get_harfbuzz_shaped_glyphs,
    get_resource_from_dw_font,
    harfbuzz_available,
)
...
class _DWriteTextRenderer(com.COMObject):
    ...
    def IsPixelSnappingDisabled(
        self, _draw_ctx: c_void_p, is_disabled: POINTER(FLOAT)
    ) -> int:  # noqa: N802
        is_disabled[0] = self.pixel_snapping
        return 0

    def GetPixelsPerDip(
        self, _draw_ctx: c_void_p, pixels_per_dip: POINTER(FLOAT)
    ) -> int:  # noqa: N802
        pixels_per_dip[0] = self.pixels_per_dip
        return 0

    def GetCurrentTransform(
        self, _draw_ctx: c_void_p, transform: POINTER(DWRITE_MATRIX)
    ) -> int:  # noqa: N802
    ...
    def DrawGlyphRun(
        ...
        glyph_renderer: DirectWriteGlyphRenderer = cast(
            drawing_context, py_object
        ).value
        ...
        for i in range(glyph_run.glyphCount):
            ...
            if (
                font_ref,
                glyph_indice,
            ) not in glyph_renderer.font.glyphs and glyph_indice not in missing:
                missing.append(glyph_indice)

        # Missing glyphs, get their info.
        if missing:
            metrics = get_glyph_metrics(
                glyph_run.fontFace, (UINT16 * len(missing))(*missing), len(missing)
            )

            for idx, glyph_indice in enumerate(missing):
                glyph = glyph_renderer.render_single_glyph(
                    glyph_run.fontFace, glyph_indice, metrics[idx], mode
                )
...
def get_glyph_metrics(
    ...
    for metric in glyph_metrics:
        glyph_width = (
            metric.advanceWidth
            + abs(metric.leftSideBearing)
            + abs(metric.rightSideBearing)
        )
        glyph_height = (
            metric.advanceHeight - metric.topSideBearing - metric.bottomSideBearing
        )
...
class DirectWriteGlyphRenderer(base.GlyphRenderer):  # noqa: D101
    ...
    draw_options = (
        ...
        else D2D1_DRAW_TEXT_OPTIONS_NONE
    )
    ...
    def render_to_image(self, text: str, width: int, height: int) -> ImageData:
        ...
        width, height = int(math.ceil(layout_metrics.width)), int(
            math.ceil(layout_metrics.height)
        )
        ...
        d2d_factory.CreateWicBitmapRenderTarget(
            bitmap, default_target_properties, byref(rt)
        )
    ...
    def render_single_glyph(
        ...
        if glyph_width == 0 and glyph_height == 0:
            ...
            render_width = (
                int(
                    math.ceil(
                        max(glyph_width, glyph_advance) * self.font.font_scale_ratio
                    )
                )
                + 1
            )
            ...
            while True:
                ...
                if color_run.contents.paletteIndex == DWRITE_NO_PALETTE_INDEX:
                    ...
                    if not temp_brush:
                        ...
                        self._render_target.CreateSolidColorBrush(
                            color_run.contents.runColor, None, byref(temp_brush)
                        )
                    else:
                ...
                if glyph_image_fmt == DWRITE_GLYPH_IMAGE_FORMATS_SVG:
                    ...
                    self._render_target.DrawGlyphRun(
                        baseline_offset, glyph_run, brush, mode
                    )
            enumerator.Release()
            if temp_brush:
                temp_brush.Release()
    ...
    def render_using_layout(self, text: str) -> Glyph | None:
        ...
        self._render_target.DrawTextLayout(
            point, text_layout, self._brush, self.draw_options
        )
    ...
    def _create_bitmap(self, width: int, height: int) -> None:
        ...
        if self._bitmap_dimensions[0] != width or self._bitmap_dimensions[1] != height:
            ...
            d2d_factory.CreateWicBitmapRenderTarget(
                self._bitmap, default_target_properties, byref(_render_target)
            )
...
def _get_font_ref(
    font_file: IDWriteFontFile, release_file: bool = True
) -> tuple[c_void_p, int]:
...
class Win32DirectWriteFont(base.Font):
    ...
    def __init__(
        ...
        size: float,
        weight: str = "normal",
        italic: bool | str = False,  # noqa: D107
        ...
        if pyglet.options["dw_legacy_naming"] and (
            self._font_index is None and self._collection is None
        ):
            write_font, self._collection = self.find_font_face(
                name, self._weight, self._style, self._stretch
            )

        assert (
            self._collection is not None
        ), f"Font: '{name}' not found in loaded or system font collection."
    ...
    def get_font_data(self) -> bytes | None:
        ...
        if font_filestream:
            ...
            void_data = c_void_p()
            context = c_void_p()
            font_filestream.ReadFileFragment(
                byref(void_data), 0, size.value, byref(context)
            )
    ...
    def filename(self) -> str:
        ...
        self._filename = pathlib.PureWindowsPath(
            buffer.value
    ...
    def render_to_image(
        self, text: str, width: int = 10000, height: int = 80
    ) -> ImageData:
    ...
    def render_glyph_indices(self, indices: list[int]) -> None:
        ...
        if missing:
            metrics = get_glyph_metrics(
                self.font_face, (UINT16 * len(missing))(*missing), len(missing)
            )
    ...
    def get_glyphs(self, text: str) -> tuple[list[Glyph], list[base.GlyphPosition]]:
        ...
        if pyglet.options.text_shaping == "platform":
            ...
            return (
                self._glyph_renderer.current_glyphs,
                self._glyph_renderer.current_offsets,
            )
    ...
    def _initialize_direct_write(cls: type[Win32DirectWriteFont]) -> None:
        ...
        if WINDOWS_10_1809_OR_GREATER:  # Added Bitmap based image glyphs.
            ...
            WINDOWS_10_CREATORS_UPDATE_OR_GREATER
        ):  # Added memory loader. Added SVG color glyphs.
    ...
    def _initialize_custom_loaders(cls: type[Win32DirectWriteFont]) -> None:
        ...
        if WINDOWS_10_CREATORS_UPDATE_OR_GREATER:
            ...
            cls._write_factory.RegisterFontFileLoader(
                cls._font_loader.as_interface(IDWriteFontFileLoader_LI)
            )

            cls._font_collection_loader = LegacyCollectionLoader(
                cls._write_factory, cls._font_loader
            )
            cls._write_factory.RegisterFontCollectionLoader(cls._font_collection_loader)

            cls._font_loader_key = cast(
                create_unicode_buffer("legacy_font_loader"), c_void_p
            )
    ...
    def add_font_data(cls: type[Win32DirectWriteFont], data: BinaryIO) -> None:
        ...
        if WINDOWS_10_CREATORS_UPDATE_OR_GREATER:
            ...
            cls._write_factory.CreateFontCollectionFromFontSet(
                cls._font_set, byref(cls._custom_collection)
            )
            ...
            if cls._custom_collection:
                ...
                cls._write_factory.UnregisterFontCollectionLoader(
                ...
                cls._write_factory.RegisterFontCollectionLoader(
                    cls._font_collection_loader
                )
                cls._write_factory.RegisterFontFileLoader(
                    cls._font_loader.as_interface(IDWriteFontFileLoader_LI)
    ...
    @classmethod
    def get_collection(
        cls: type[Win32DirectWriteFont], font_name: str
    ) -> tuple[int | None, IDWriteFontCollection1 | None]:
        ...
        if cls._custom_collection:
            cls._custom_collection.FindFamilyName(
                create_unicode_buffer(font_name), byref(font_index), byref(font_exists)
            )
    ...
    def find_font_face(
        ...
        """
        p_weight, p_italic, p_stretch = cls.parse_name(
            font_name, weight, italic, stretch
        )

        _debug_print(
            f"directwrite: '{font_name}' not found. Attempting legacy name lookup in all collections."
        )
        if cls._custom_collection:
            collection_idx = cls.find_legacy_font(
                cls._custom_collection, font_name, p_weight, p_italic, p_stretch
            )
        ...
        collection_idx = cls.find_legacy_font(
            sys_collection, font_name, p_weight, p_italic, p_stretch
        )
        if collection_idx is not None:
    ...
    @staticmethod
    def parse_name(
        font_name: str, weight: str, style: int, stretch: int
    ) -> tuple[str, int, int]:
    ...
    def find_legacy_font(
        ...
        for i in range(coll_count):
            ...
            family_names = Win32DirectWriteFont.unpack_localized_string(
                family_name_str, locale
            )
            ...
            for j in range(ft_ct):
                ...
                if _debug_font and full_debug:
                    ...
                    strings = Win32DirectWriteFont.unpack_localized_string(
                        fc_str, locale
                    )
                ...
                match_found = False
                if exists.value != 0:
                    for compat_name in Win32DirectWriteFont.unpack_localized_string(
                        compat_names, locale
                    ):
            ...
            if matches:
                write_font = Win32DirectWriteFont.match_closest_font(
                    matches, weight, italic, stretch
                )
    ...
    def match_closest_font(
        font_list: list[tuple[int, int, int, IDWriteFont]],
        weight: str,
        italic: int,
        stretch: int,
    ) -> IDWriteFont | None:
        ...
        for match in font_list:
            ...
            if f_weight == weight:
                ...
                and f_weight != DWRITE_FONT_WEIGHT_NORMAL
            ):
                similar_match += 1
            ...
            if f_style == italic:
                ...
                italic != DWRITE_FONT_STYLE_NORMAL
                and f_style != DWRITE_FONT_STYLE_NORMAL
            ):
                similar_match += 1
            ...
            if stretch == f_stretch:
                ...
                and f_stretch != DWRITE_FONT_STRETCH_NORMAL
            ):
                similar_match += 1
    ...
    @staticmethod
    def unpack_localized_string(
        local_string: IDWriteLocalizedStrings, locale: str
    ) -> list[str]:
...
hr = D2D1CreateFactory(
    D2D1_FACTORY_TYPE_SINGLE_THREADED, IID_ID2D1Factory, None, byref(d2d_factory)
)
