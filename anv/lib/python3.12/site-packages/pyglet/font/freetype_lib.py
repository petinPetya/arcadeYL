...
class FT_StreamDesc(Union):
    _fields_ = [("pointer", c_void_p), ("value", c_long)]
...
def _get_function_with_error_handling(
    name: str, argtypes: Iterable[Any], rtype: Any
) -> Callable:
...
FT_LOAD_NO_HINTING = (
    1 << 1
)  # Disable font hinting (may cause blurriness at small sizes)
...
FT_LOAD_PEDANTIC = (
    1 << 7
)  # Enable strict font parsing (useful for debugging malformed fonts)
FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH = (
    1 << 9
)  # Ignore font-wide advance width; use glyph-specific advance
FT_LOAD_NO_RECURSE = (
    1 << 10
)  # Load only composite glyphs, do not load sub-glyphs recursively
FT_LOAD_IGNORE_TRANSFORM = (
    1 << 11
)  # Disable any transformations with FT_Set_Transform()
FT_LOAD_MONOCHROME = 1 << 12  # Render glyphs in monochrome (1-bit, no anti-aliasing)
FT_LOAD_LINEAR_DESIGN = (
    1 << 13
)  # Use linear design metrics instead of transformed metrics
FT_LOAD_SBITS_ONLY = (
    1 << 14
)  # Load only embedded bitmap strikes (ignore vector outlines)
...
FT_LOAD_BITMAP_METRICS_ONLY = (
    1 << 22
)  # Compute bitmap glyph metrics but do not load the glyph itself
FT_LOAD_NO_SVG = (
    1 << 24
)  # Disable loading of SVG glyphs (for fonts that contain both vector and SVG glyphs)
...
FT_Init_FreeType = _get_function_with_error_handling(
    "FT_Init_FreeType", [POINTER(FT_Library)], FT_Error
)
FT_Done_FreeType = _get_function_with_error_handling(
    "FT_Done_FreeType", [FT_Library], FT_Error
)

FT_New_Face = _get_function_with_error_handling(
    "FT_New_Face", [FT_Library, c_char_p, FT_Long, POINTER(FT_Face)], FT_Error
)
FT_Done_Face = _get_function_with_error_handling("FT_Done_Face", [FT_Face], FT_Error)
FT_Reference_Face = _get_function_with_error_handling(
    "FT_Reference_Face", [FT_Face], FT_Error
)
FT_New_Memory_Face = _get_function_with_error_handling(
    "FT_New_Memory_Face",
    [FT_Library, POINTER(FT_Byte), FT_Long, FT_Long, POINTER(FT_Face)],
    FT_Error,
)

FT_Set_Char_Size = _get_function_with_error_handling(
    "FT_Set_Char_Size", [FT_Face, FT_F26Dot6, FT_F26Dot6, FT_UInt, FT_UInt], FT_Error
)
FT_Set_Pixel_Sizes = _get_function_with_error_handling(
    "FT_Set_Pixel_Sizes", [FT_Face, FT_UInt, FT_UInt], FT_Error
)
FT_Load_Glyph = _get_function_with_error_handling(
    "FT_Load_Glyph", [FT_Face, FT_UInt, FT_Int32], FT_Error
)
FT_Get_Char_Index = _get_function("FT_Get_Char_Index", [FT_Face, FT_ULong], FT_UInt)
FT_Load_Char = _get_function_with_error_handling(
    "FT_Load_Char", [FT_Face, FT_ULong, FT_Int32], FT_Error
)
FT_Get_Kerning = _get_function_with_error_handling(
    "FT_Get_Kerning", [FT_Face, FT_UInt, FT_UInt, FT_UInt, POINTER(FT_Vector)], FT_Error
)

FT_Get_Sfnt_Table = _get_function("FT_Get_Sfnt_Table", [FT_Face, FT_Sfnt_Tag], c_void_p)

try:
    FT_Select_Size = _get_function_with_error_handling(
        "FT_Select_Size", [FT_Face, FT_Int], FT_Error
    )
except ImportError:
    FT_Select_Size = None
...
FT_Get_Sfnt_Name_Count = _get_function("FT_Get_Sfnt_Name_Count", [FT_Face], FT_UInt)
FT_Get_Sfnt_Name = _get_function_with_error_handling(
    "FT_Get_Sfnt_Name", [FT_Face, FT_UInt, POINTER(FT_SfntName)], FT_Error
)
