...
class Accessor:
    def __init__(self, data, owner):
        ...
        self._byte_length = (
            _gl_type_sizes[self.component_type] * _accessor_type_counts[self.type]
        )
...
class Attribute(BaseAttribute):
    def __init__(self, name, index, owner):
        ...
        super().__init__(
            name, accessor.fmt, accessor.type, accessor.count, accessor.as_array()
        )
...
class Primitive(BasePrimitive):
    def __init__(self, data, owner):
        attributes = [
            Attribute(name, index, owner)
            for name, index in data.get("attributes").items()
        ]

        indices_index = data.get("indices")
        indices_accessor = (
            owner.accessors[indices_index] if indices_index is not None else None
        )
        ...
        material = (
            owner.materials[material_index] if material_index is not None else None
        )
...
class Image:
    def __init__(self, data, owner):
        ...
        self._buffer_view_index = data.get("bufferView")
        self.buffer_view = (
            owner.buffer_views[self._buffer_view_index]
...
class Mesh:
    def __init__(self, data, owner):
        self.data = data
        self.primitives = [
            Primitive(primitive_data, owner)
            for primitive_data in data.get("primitives")
        ]
...
class GLTF:
    def __init__(self, gltf_data: dict, binary_buffer: bytes | None = None):
        ...
        self.buffer_views = [
            BufferView(data=data, owner=self) for data in gltf_data["bufferViews"]
        ]
        self.accessors = [
            Accessor(data=data, owner=self) for data in gltf_data["accessors"]
        ]
        ...
        self.images = [
            Image(data=data, owner=self) for data in gltf_data.get("images", [])
        ]
        self.samplers = [Sampler(data=data) for data in gltf_data.get("samplers", [])]
        self.textures = [
            Texture(data=data, owner=self) for data in gltf_data.get("textures", [])
        ]
        ...
        self.cameras = [
            Camera(cam["type"], cam[cam["type"]])
            for cam in gltf_data.get("cameras", [])
        ]

        self.scenes = [
            Scene(nodes=[self.nodes[i] for i in data["nodes"]])
            for data in gltf_data["scenes"]
        ]
...
def load_gltf(filename, file=None) -> GLTF:
    ...
    if filename.endswith("glb"):
        ...
        if chunk_0_type != b"JSON":
            raise ModelDecodeException(
                f"glTF 'chunk 0 type' is not 'JSON': {chunk_0_type}"
            )
        ...
        if chunk_1_type != b"BIN\x00":
            raise ModelDecodeException(
                f"glTF 'chunk 1 type' is not 'BIN': {chunk_0_type}"
            )
    ...
    try:
        ...
        raise ModelDecodeException(
            f"Json error. Does not appear to be a valid glTF file. {e}"
        )
    finally:
        file.close()
