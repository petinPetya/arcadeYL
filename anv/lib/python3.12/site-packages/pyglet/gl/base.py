...
class Context:
    ...
    def __init__(
        self, config: DisplayConfig, context_share: Context | None = None
    ) -> None:
    ...
    def set_current(self) -> None:
        ...
        if self.object_space.doomed_shader_programs:
            self._delete_objects_one_by_one(
                self.object_space.doomed_shader_programs, gl.glDeleteProgram
            )
        if self.object_space.doomed_shaders:
            self._delete_objects_one_by_one(
                self.object_space.doomed_shaders, gl.glDeleteShader
            )
        if self.object_space.doomed_renderbuffers:
            self._delete_objects(
                self.object_space.doomed_renderbuffers, gl.glDeleteRenderbuffers
            )
    ...
    @staticmethod
    def _delete_objects(
        list_: list, deletion_func: Callable[[int, Array[gl.GLuint]], None]
    ) -> None:
    ...
    @staticmethod
    def _delete_objects_one_by_one(
        list_: list, deletion_func: Callable[[gl.GLuint], None]
    ) -> None:
        """Release all OpenGL objects in the given list.
    ...
    def _safe_to_operate_on_object_space(self) -> bool:
        ...
        """
        return (
            self.object_space is gl.current_context.object_space
    ...
    def create_program(self, *sources: tuple[str, str]) -> ShaderProgram:
        ...
        shaders = (
            pyglet.graphics.shader.Shader(src, srctype) for (src, srctype) in sources
        )
