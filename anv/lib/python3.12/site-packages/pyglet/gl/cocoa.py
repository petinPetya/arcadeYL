...
"""
os_x_release: dict[
    str, tuple[float] | tuple[float, float] | tuple[float, float, float]
] = {
    "pre-release": (0, 1),
    "kodiak": (1, 2, 1),
    "cheetah": (1, 3, 1),
...
class CocoaConfig(Config):  # noqa: D101
    ...
    def match(self, canvas: CocoaCanvas) -> list[CocoaDisplayConfig]:
        ...
        if _os_x_version < os_x_release["snow_leopard"]:
            ...
            version = (
                getattr(self, "major_version", None) or 3,
...
class CocoaDisplayConfig(DisplayConfig):  # noqa: D101
    ...
    def create_context(self, share: CocoaContext | None) -> CocoaContext:
        ...
        if self.transparent_framebuffer:
            opaque = c_int(0)
            nscontext.setValues_forParameter_(
                byref(opaque), cocoapy.NSOpenGLCPSurfaceOpacity
            )
...
class CocoaContext(Context):  # noqa: D101
    ...
    def __init__(
        self,
        config: CocoaDisplayConfig,
        nscontext: NSOpenGLContext,
        share: CocoaContext | None,
    ) -> None:  # noqa: D107
    ...
    def set_vsync(self, vsync: bool = True) -> None:
        ...
        self._nscontext.setValues_forParameter_(
            byref(vals), cocoapy.NSOpenGLCPSwapInterval
        )
