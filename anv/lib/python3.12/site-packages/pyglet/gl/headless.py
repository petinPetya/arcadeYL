...
class HeadlessConfig(Config):  # noqa: D101
    def match(self, canvas: HeadlessCanvas) -> list[HeadlessDisplayConfig]:
        ...
        egl.eglChooseConfig(
            display_connection, attrs_list, configs, num_config.value, byref(num_config)
        )
...
class HeadlessDisplayConfig(DisplayConfig):  # noqa: D101
    ...
    def __init__(
        self, canvas: HeadlessCanvas, egl_config: egl.EGLConfig, config: HeadlessConfig
    ) -> None:  # noqa: D107
        super().__init__(canvas, config)
        self._egl_config = egl_config
        context_attribs = (
            egl.EGL_CONTEXT_MAJOR_VERSION,
            config.major_version or 2,
            egl.EGL_CONTEXT_MINOR_VERSION,
            config.minor_version or 0,
            egl.EGL_CONTEXT_OPENGL_FORWARD_COMPATIBLE,
            config.forward_compatible or 0,
            egl.EGL_CONTEXT_OPENGL_DEBUG,
            config.debug or 0,
            egl.EGL_NONE,
        )
        self._context_attrib_array = (egl.EGLint * len(context_attribs))(
            *context_attribs
        )
        ...
        for name, attr in self.attribute_ids.items():
            ...
            egl.eglGetConfigAttrib(
                canvas.display._display_connection, egl_config, attr, byref(value)
            )  # noqa: SLF001
...
class HeadlessContext(Context):  # noqa: D101
    ...
    def __init__(
        self, config: HeadlessDisplayConfig, share: HeadlessContext | None
    ) -> None:  # noqa: D107
        super().__init__(config, share)

        self.display_connection = (
            config.canvas.display._display_connection
        )  # noqa: SLF001
    ...
    def _create_egl_context(self, share: HeadlessContext | None) -> egl.EGLContext:
        ...
        return egl.eglCreateContext(
            self.config.canvas.display._display_connection,  # noqa: SLF001
            self.config._egl_config,
            share_context,  # noqa: SLF001
