...
class Win32Config(Config):  # noqa: D101
    ...
    def match(
        self, canvas: Win32Canvas
    ) -> list[Win32DisplayConfig] | list[Win32DisplayConfigARB]:
    ...
    def _get_pixel_format_descriptor_matching_configs(
        self, canvas: Win32Canvas
    ) -> list[Win32DisplayConfig]:
    ...
    def _get_arb_pixel_format_matching_configs(
        self, canvas: Win32Canvas
    ) -> list[Win32DisplayConfigARB]:
        """Get configs using the WGL_ARB_pixel_format extension.

        This method assumes a (dummy) GL context is already created.
        """
        # Check for required extensions
        if (self.sample_buffers or self.samples) and not gl_info.have_extension(
            "GL_ARB_multisample"
        ):
            return []
        ...
        pformats = (c_int * 16)()
        nformats = c_uint(16)
        wglext_arb.wglChoosePixelFormatARB(
            canvas.hdc, attrs, None, nformats, pformats, nformats
        )

        formats = [
            Win32DisplayConfigARB(canvas, pf, self) for pf in pformats[: nformats.value]
        ]
        return formats
...
class Win32DisplayConfig(DisplayConfig):  # noqa: D101
    def __init__(
        self, canvas: Win32Canvas, pf: int, config: Win32Config
    ) -> None:  # noqa: D107
        ...
        _gdi32.DescribePixelFormat(
            canvas.hdc, pf, sizeof(PIXELFORMATDESCRIPTOR), byref(self._pfd)
        )
...
class Win32DisplayConfigARB(DisplayConfig):  # noqa: D101
    ...
    def __init__(
        ...
        wglext_arb.wglGetPixelFormatAttribivARB(
            canvas.hdc, pf, 0, len(attrs), attrs, values
        )
    ...
    def create_context(
        self, share: Win32ARBContext | None
...
class _BaseWin32Context(Context):
    def __init__(
        self,
        config: Win32DisplayConfig | Win32DisplayConfigARB,
        share: Win32Context | Win32ARBContext,
    ) -> None:
...
class Win32ARBContext(_BaseWin32Context):  # noqa: D101
    ...
    def attach(self, canvas: Win32Canvas) -> None:
        ...
        if self.config.major_version is not None:
            attribs.extend(
                [wglext_arb.WGL_CONTEXT_MAJOR_VERSION_ARB, self.config.major_version]
            )
        if self.config.minor_version is not None:
            attribs.extend(
                [wglext_arb.WGL_CONTEXT_MINOR_VERSION_ARB, self.config.minor_version]
            )
        flags = 0
