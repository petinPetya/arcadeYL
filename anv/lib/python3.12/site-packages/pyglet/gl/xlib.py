...
class XlibConfig(Config):  # noqa: D101
    ...
    def match(self, canvas: XlibCanvas) -> list[XlibDisplayConfig]:
        ...
        configs = glx.glXChooseFBConfig(
            x_display, x_screen, attrib_list, byref(elements)
        )
        if not configs:
            return []
...
class XlibDisplayConfig(DisplayConfig):  # noqa: D101
    ...
    def __init__(
        ...
        canvas: XlibCanvas,
        info: GLXInfo,
        fbconfig: glx.GLXFBConfig,  # noqa: D107
        config: XlibConfig,
    ) -> None:
        ...
        if config.transparent_framebuffer:
            xvi_ptr = glx.glXGetVisualFromFBConfig(
                canvas.display._display, self.fbconfig
            )  # noqa: SLF001
            if xvi_ptr:
                self.transparent = self._is_visual_transparent(
                    xvi_ptr.contents.visual
                )  # noqa: SLF001
                xlib.XFree(xvi_ptr)
    ...
    def get_visual_info(self) -> glx.XVisualInfo:
        return glx.glXGetVisualFromFBConfig(
            self.canvas.display._display, self.fbconfig
        ).contents  # noqa: SLF001
...
class XlibContext(Context):  # noqa: D101
    ...
    def __init__(
        self, config: XlibDisplayConfig, share: XlibContext | None
    ) -> None:  # noqa: D107
        ...
        self._use_video_sync = self._have_SGI_video_sync and not (
            self._have_EXT_swap_control or self._have_MESA_swap_control
        )
    ...
    def _create_glx_context(self, share: XlibContext | None) -> glx.GLXContext:
        ...
        if self.config.major_version is not None:
            attribs.extend(
                [glxext_arb.GLX_CONTEXT_MAJOR_VERSION_ARB, self.config.major_version]
            )
        if self.config.minor_version is not None:
            attribs.extend(
                [glxext_arb.GLX_CONTEXT_MINOR_VERSION_ARB, self.config.minor_version]
            )

        if self.config.opengl_api == "gl":
            attribs.extend(
                [
                    glxext_arb.GLX_CONTEXT_PROFILE_MASK_ARB,
                    glxext_arb.GLX_CONTEXT_CORE_PROFILE_BIT_ARB,
                ]
            )
            ...
            attribs.extend(
                [
                    glxext_arb.GLX_CONTEXT_PROFILE_MASK_ARB,
                    glxext_arb.GLX_CONTEXT_ES2_PROFILE_BIT_EXT,
                ]
            )
        ...
        return glxext_arb.glXCreateContextAttribsARB(
            self.config.canvas.display._display,  # noqa: SLF001
    ...
    def attach(self, canvas: XlibCanvas) -> None:
        ...
        self.glx_window = glx.glXCreateWindow(
            self.x_display, self.config.fbconfig, canvas.x_window, None
        )
        self.set_current()
    ...
    def set_vsync(self, vsync: bool = True) -> None:
        ...
        try:
            if not self._use_video_sync and self._have_EXT_swap_control:
                glxext_arb.glXSwapIntervalEXT(
                    self.x_display, glx.glXGetCurrentDrawable(), interval
                )
