...
class OpenALDevice(OpenALObject):
    ...
    def get_version(self):
        ...
        minor = alc.ALCint()
        alc.alcGetIntegerv(
            self._al_device, alc.ALC_MAJOR_VERSION, ctypes.sizeof(major), major
        )
        self.check_context_error("Failed to get version.")
        alc.alcGetIntegerv(
            self._al_device, alc.ALC_MINOR_VERSION, ctypes.sizeof(minor), minor
        )
        self.check_context_error("Failed to get version.")
    ...
    def unqueue_buffers(self):
        ...
        if processed > 0:
            ...
            self.buffer_pool.return_buffers(
                [self._owned_buffers.pop(bn) for bn in buffers]
            )
        return processed
...
class OpenALBuffer(OpenALObject):
    ...
    def data(self, audio_data, audio_format):
        ...
        try:
            al_format = self._format_map[
                (audio_format.channels, audio_format.sample_size)
            ]
        except KeyError:
            raise MediaException(
                f"OpenAL does not support '{audio_format.sample_size}bit' audio."
            )
