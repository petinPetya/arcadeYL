...
class AbstractAudioPlayer(metaclass=ABCMeta):
    ...
    def __init__(self, source, player):
        ...
        self._precision_buffer = (
            None if source.is_precise() else SourcePrecisionBuffer(source)
        )
        ...
        self._buffered_data_comfortable_limit = int(
            self._buffered_data_ideal_size * (2 / 3)
        )
    ...
    def _raw_play_cursor_to_time(self, cursor):
        if cursor is None:
            return None
        return (
            self._to_perceived_play_cursor(cursor)
    ...
    def append_events(self, start_index, events):
        ...
        for event in events:
            ...
            assert _debug(
                f"AbstractAudioPlayer: Adding event {event} at {event_cursor}"
            )
    ...
    def get_audio_time_diff(self, audio_time):
        ...
        if audio_time is not None:
            ...
            diff_bytes = self.source.audio_format.timestamp_to_bytes_aligned(
                audio_time - p_time
            )
    ...
    def _get_and_compensate_audio_data(self, requested_size, audio_position=None):
        ...
        if desync_bytes > 0:
            ...
            if audio_data is not None:
                ...
                ad[0:compensated_bytes] = first_frame * (
                    compensated_bytes // afmt.bytes_per_frame
                )
