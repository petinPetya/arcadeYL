...
def load_xaudio2(dll_name):
    ...
    if platform.architecture()[0] == "32bit" and platform.machine().endswith(
        ...
        xaudio2 = os.path.join(
            os.environ["WINDIR"], "SysWOW64", "{}.dll".format(xaudio2)
        )
...
class XAUDIO2_PERFORMANCE_DATA(Structure):
    ...
    def __repr__(self):
        return "XAUDIO2PerformanceData(active_voices={}, total_voices={}, glitches={}, latency={} samples, memory_usage={} bytes)".format(
            self.ActiveSourceVoiceCount,
            self.TotalSourceVoiceCount,
            self.GlitchesSinceEngineStarted,
            self.CurrentLatencyInSamples,
...
class XAUDIO2_VOICE_STATE(Structure):
    ...
    def __repr__(self):
        return "XAUDIO2_VOICE_STATE(BuffersQueued={0}, SamplesPlayed={1})".format(
            self.BuffersQueued, self.SamplesPlayed
        )
...
XAUDIO2_MAX_BUFFERS_SYSTEM = (
    2  # Maximum buffers allowed for system threads (Xbox 360 only)
)
...
LowPassFilter = (
    0  # Attenuates frequencies above the cutoff frequency (state-variable filter).
)
BandPassFilter = (
    1  # Attenuates frequencies outside a given range      (state-variable filter).
)
HighPassFilter = (
    2  # Attenuates frequencies below the cutoff frequency (state-variable filter).
)
NotchFilter = (
    3  # Attenuates frequencies inside a given range       (state-variable filter).
)
...
XAUDIO2_NO_VIRTUAL_AUDIO_CLIENT = (
    0x10000  # Used in CreateMasteringVoice to create a virtual audio client
)


class IXAudio2VoiceCallback(com.Interface):
    _methods_ = [
        ("OnVoiceProcessingPassStart", com.VOIDMETHOD(UINT32)),
        ("OnVoiceProcessingPassEnd", com.VOIDMETHOD()),
        ("OnStreamEnd", com.VOIDMETHOD()),
        ("OnBufferStart", com.VOIDMETHOD(c_void_p)),
        ("OnBufferEnd", com.VOIDMETHOD(c_void_p)),
        ("OnLoopEnd", com.VOIDMETHOD(c_void_p)),
        ("OnVoiceError", com.VOIDMETHOD(c_void_p, HRESULT)),
    ]
...
class IXAudio2Voice(com.pInterface):
    _methods_ = [
        ("GetVoiceDetails", com.VOIDMETHOD(POINTER(XAUDIO2_VOICE_DETAILS))),
        ("SetOutputVoices", com.STDMETHOD(POINTER(XAUDIO2_VOICE_SENDS))),
        ("SetEffectChain", com.STDMETHOD(POINTER(XAUDIO2_EFFECT_CHAIN))),
        ("EnableEffect", com.STDMETHOD(UINT32, UINT32)),
        ("DisableEffect", com.STDMETHOD(UINT32, UINT32)),
        ("GetEffectState", com.VOIDMETHOD(UINT32, POINTER(BOOL))),
        ("SetEffectParameters", com.STDMETHOD(UINT32, c_void_p, UINT32, UINT32)),
        ("GetEffectParameters", com.VOIDMETHOD(UINT32, POINTER(BOOL))),
        (
            "SetFilterParameters",
            com.STDMETHOD(POINTER(XAUDIO2_FILTER_PARAMETERS), UINT32),
        ),
        ("GetFilterParameters", com.VOIDMETHOD(POINTER(XAUDIO2_FILTER_PARAMETERS))),
        (
        ...
        ("SetVolume", com.STDMETHOD(c_float, UINT32)),
        ("GetVolume", com.VOIDMETHOD(POINTER(c_float))),
        ("SetChannelVolumes", com.STDMETHOD(UINT32, POINTER(c_float), UINT32)),
        ("GetChannelVolumes", com.VOIDMETHOD(UINT32, POINTER(c_float))),
        (
            "SetOutputMatrix",
            com.STDMETHOD(c_void_p, UINT32, UINT32, POINTER(c_float), UINT32),
        ),
        ("GetOutputMatrix", com.STDMETHOD(c_void_p, UINT32, UINT32, POINTER(c_float))),
        ("DestroyVoice", com.VOIDMETHOD()),
    ]
...
class IXAudio2SourceVoice(IXAudio2Voice):
    _methods_ = [
        ...
        ("SubmitSourceBuffer", com.STDMETHOD(POINTER(XAUDIO2_BUFFER), c_void_p)),
        ("FlushSourceBuffers", com.STDMETHOD()),
        ("Discontinuity", com.STDMETHOD()),
        ("ExitLoop", com.STDMETHOD(UINT32)),
        ("GetState", com.VOIDMETHOD(POINTER(XAUDIO2_VOICE_STATE), UINT32)),
        ("SetFrequencyRatio", com.STDMETHOD(c_float, UINT32)),
        ("GetFrequencyRatio", com.VOIDMETHOD(POINTER(c_float))),
        ("SetSourceSampleRate", com.STDMETHOD(UINT32)),
    ]
...
class IXAudio2EngineCallback(com.Interface):
    _methods_ = [
        ("OnProcessingPassStart", com.VOIDMETHOD()),
        ("OnProcessingPassEnd", com.VOIDMETHOD()),
        ("OnCriticalError", com.VOIDMETHOD(HRESULT)),
    ]
...
class X3DAUDIO_CONE(Structure):
    _fields_ = [
        (
            "InnerAngle",
            FLOAT32,
        ),  # inner cone angle in radians, must be within [0.0f, X3DAUDIO_2PI]
        (
            "OuterAngle",
            FLOAT32,
        ),  # outer cone angle in radians, must be within [InnerAngle, X3DAUDIO_2PI]
        (
            "InnerVolume",
            FLOAT32,
        ...
        (
            "OuterReverb",
            FLOAT32,
        ),  # reverb send level scaler on/beyond outer cone, used only for reverb calculations, must be within [0.0f, 2.0f] when used
    ]
...
class X3DAUDIO_LISTENER(Structure):
    _fields_ = [
        ...
        (
            "pCone",
            POINTER(X3DAUDIO_CONE),
        ),  # sound cone, used only for matrix, LPF (both direct and reverb paths), and reverb calculations, NULL specifies omnidirectionality
    ]
...
DBL_MAX = 1.7976931348623158e308  # max value
...
FLT_MAX = 3.402823466e38  # max value
...
X3DAUDIO_CALCULATE_DELAY = (
    0x00000002  # enable delay time array calculation (stereo final mix only)
)
X3DAUDIO_CALCULATE_LPF_DIRECT = (
    0x00000004  # enable LPF direct-path coefficient calculation
)
X3DAUDIO_CALCULATE_LPF_REVERB = (
    0x00000008  # enable LPF reverb-path coefficient calculation
)
...
X3DAUDIO_CALCULATE_EMITTER_ANGLE = (
    0x00000040  # enable emitter-to-listener interior angle calculation
)
...
X3DAudioCalculate.argtypes = [
    POINTER(X3DAUDIO_HANDLE),
    POINTER(X3DAUDIO_LISTENER),
    POINTER(X3DAUDIO_EMITTER),
    UINT32,
    POINTER(X3DAUDIO_DSP_SETTINGS),
]
...
class XAUDIO2FX_REVERB_PARAMETERS(Structure):
    _fields_ = [
        ...
        (
            "SideDelay",
            UINT32,
        ),  # .1: [0, 5] in ms, all other: not used, but still validated  # WIN 10 only.
        # Indexed Parameters
...
class IXAudio2(com.pIUnknown):
    _methods_ = [
        ("RegisterForCallbacks", com.STDMETHOD(POINTER(IXAudio2EngineCallback))),
        ("UnregisterForCallbacks", com.VOIDMETHOD(POINTER(IXAudio2EngineCallback))),
        (
            "CreateSourceVoice",
            com.STDMETHOD(
                POINTER(IXAudio2SourceVoice),
                POINTER(WAVEFORMATEX),
                UINT32,
                c_float,
                POINTER(IXAudio2VoiceCallback),
        ...
        (
            ...
            com.STDMETHOD(
                POINTER(IXAudio2MasteringVoice),
                UINT32,
                UINT32,
                UINT32,
                LPCWSTR,
        ...
        ("StopEngine", com.VOIDMETHOD()),
        ("CommitChanges", com.STDMETHOD(UINT32)),
        ("GetPerformanceData", com.VOIDMETHOD(POINTER(XAUDIO2_PERFORMANCE_DATA))),
        (
