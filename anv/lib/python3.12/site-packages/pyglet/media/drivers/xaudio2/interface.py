...
class XAudio2Driver:
    ...
    def __init__(self):
        ...
        self._emitting_voices = (
            []
        )  # Contains all playing source voices with an emitter.
        ...
        self._players = (
            []
        )  # Used for resetting/restoring xaudio2. Stores high-level players to callback.
        ...
        if self.restart_on_error:
            ...
            if audio_devices:
                ...
                if audio_devices.get_default_output() is None:
                    raise ImportError(
                        "No default audio device found, can not create driver."
                    )
    ...
    def on_default_changed(self, device, flow: DeviceFlow):
        if flow == DeviceFlow.OUTPUT:
            ...
            if device is None:
                assert _debug(
                    "Error: Default audio device was removed or went missing."
                )
                self._dead = True
            else:
                if self._dead:
                    assert _debug(
                        "Warning: Default audio device added after going missing."
                    )
                    self._dead = False
    ...
    def enable_3d(self):
        ...
        lib.X3DAudioInitialize(
            channel_mask.value, lib.X3DAUDIO_SPEED_OF_SOUND, self._x3d_handle
        )
    ...
    def _create_new_voice(self, audio_format):
        ...
        return XA2SourceVoice(
            voice, callback, audio_format.channels, audio_format.sample_size
        )
...
class XA2SourceVoice:
    def __init__(self, voice, callback, channel_count, sample_size):
        self._voice_state = (
            lib.XAUDIO2_VOICE_STATE()
        )  # Used for buffer state, will be reused constantly.
        self._voice = voice
    ...
    def buffers_queued(self):
        ...
        self._voice.GetState(
            byref(self._voice_state), lib.XAUDIO2_VOICE_NOSAMPLESPLAYED
        )
    ...
    def position(self):
        if self.is_emitter:
            return (
                self._emitter.Position.x,
                self._emitter.Position.y,
                self._emitter.Position.z,
            )
        else:
            return 0, 0, 0
    ...
    def cone_orientation(self):
        ...
        if self.is_emitter:
            return (
                self._emitter.OrientFront.x,
                self._emitter.OrientFront.y,
                self._emitter.OrientFront.z,
            )
        else:
            return 0, 0, 0
    ...
    def cone_angles(self):
        ...
        if self.is_emitter:
            return self._ConeAngles(
                self._emitter.pCone.contents.InnerAngle,
                self._emitter.pCone.contents.OuterAngle,
            )
        else:
...
class XAudio2Listener:
    ...
    def position(self):
        return (
            self.listener.Position.x,
            self.listener.Position.y,
            self.listener.Position.z,
        )
