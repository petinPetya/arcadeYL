...
class XAudio2Listener(AbstractListener):
    ...
    def _set_orientation(self) -> None:
        self._xa2_listener.orientation = _convert_coordinates(
            self._forward_orientation
        ) + _convert_coordinates(self._up_orientation)


class XAudio2AudioPlayer(AbstractAudioPlayer):
    def __init__(
        self, driver: "XAudio2Driver", source: "Source", player: "Player"
    ) -> None:
        ...
        self._xa2_source_voice = self.driver._xa2_driver.get_source_voice(
            source.audio_format, self
        )
    ...
    def delete(self) -> None:
        ...
        self.stop()
        self.driver._xa2_driver.return_voice(
            self._xa2_source_voice, self._audio_data_in_use
        )
        self.driver = None
        self._xa2_source_voice = None
    ...
    def clear(self) -> None:
        ...
        if self._xa2_source_voice.is_emitter:
            ...
            self._xa2_source_voice.cone_orientation = _convert_coordinates(
                self.player.cone_orientation
            )
    ...
    def on_buffer_end(self, buffer_context_ptr: int) -> None:
        ...
        with self._audio_data_lock:
            ...
            if self._audio_data_in_use:
                assert _debug(
                    f"Buffer ended, others remain: {len(self._audio_data_in_use)=}"
                )
                return
    ...
    def set_cone_orientation(
        self, cone_orientation: Tuple[float, float, float]
    ) -> None:
        if self._xa2_source_voice.is_emitter:
            self._xa2_source_voice.cone_orientation = _convert_coordinates(
                cone_orientation
