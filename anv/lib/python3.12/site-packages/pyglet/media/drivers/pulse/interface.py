...
T = TypeVar("T")
PulseAudioStreamSuccessCallback = Callable[
    [ctypes.POINTER(pa.pa_stream), int, Any], Any
]
PulseAudioStreamRequestCallback = Callable[
    [ctypes.POINTER(pa.pa_stream), int, Any], Any
]
PulseAudioStreamNotifyCallback = Callable[[ctypes.POINTER(pa.pa_stream), Any], Any]
PulseAudioContextSuccessCallback = Callable[
    [ctypes.POINTER(pa.pa_context), int, Any], Any
]
...
class PulseAudioMainloop:
    def __init__(self) -> None:
        ...
        self._pa_mainloop_vtab = pa.pa_threaded_mainloop_get_api(
...
class PulseAudioContext(PulseAudioMainloopChild):
    ...
    _state_name = {
        pa.PA_CONTEXT_UNCONNECTED: "Unconnected",
    ...
    def protocol_version(self) -> Optional[str]:
        if self._pa_context is not None:
            return get_uint32_or_none(
                pa.pa_context_get_protocol_version(self._pa_context)
            )
        return None
    ...
    def server_protocol_version(self) -> Optional[str]:
        if self._pa_context is not None:
            return get_uint32_or_none(
                pa.pa_context_get_server_protocol_version(self._pa_context)
            )
        return None
    ...
    def set_input_volume(
        self, stream: "PulseAudioStream", volume: float
    ) -> "PulseAudioOperation":
        """
    ...
    def _set_state_callback(
        self, py_callback: Optional[Callable[["PulseAudioContext", Any], Any]]
    ) -> None:
        ...
        pa.pa_context_set_state_callback(
            self._pa_context, self._pa_state_change_callback, None
        )

    def _state_callback(self, context: "PulseAudioContext", _userdata) -> None:
        self.state = pa.pa_context_get_state(self._pa_context)
        assert _debug(
            f"PulseAudioContext: state changed to {self._state_name[self.state]}"
        )
...
class PulseAudioStream(PulseAudioMainloopChild):
    ...
    _state_name = {
        pa.PA_STREAM_UNCONNECTED: "Unconnected",
    ...
    def connect_playback(
        self, tlength: int = _UINT32_MAX, minreq: int = _UINT32_MAX
    ) -> None:
        context = self.context()
        assert self._pa_stream is not None
        ...
        context.check(
            pa.pa_stream_connect_playback(
                self._pa_stream, device, buffer_attr, flags, volume, sync_stream
            )
        )
    ...
    def begin_write(self, nbytes: Optional[int] = None) -> Tuple[ctypes.c_void_p, int]:
        ...
        context.check(
            pa.pa_stream_begin_write(
                self._pa_stream, ctypes.byref(addr), ctypes.byref(nbytes_st)
            )
        )
        context.check_ptr_not_null(addr)

        assert _debug(
            f"PulseAudioStream: begin_write nbytes={nbytes} nbytes_n={nbytes_st.value}"
        )
    ...
    def write(self, data, length: int, seek_mode=pa.PA_SEEK_RELATIVE) -> int:
        ...
        context.check(
            pa.pa_stream_write(
                self._pa_stream, data, length, pa.pa_free_cb_t(0), 0, seek_mode
            )
        )
        return length
    ...
    def _state_callback(self, _stream, _userdata) -> None:
        self._refresh_state()
        assert _debug(
            f"PulseAudioStream: state changed to {self._state_name[self.state]}"
        )
        self.mainloop.signal()
...
class PulseAudioOperation(PulseAudioMainloopChild):
    ...
    _state_name = {
        pa.PA_OPERATION_RUNNING: "Running",
