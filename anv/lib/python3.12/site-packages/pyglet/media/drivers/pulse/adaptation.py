...
class PulseAudioPlayer(AbstractAudioPlayer):
    def __init__(
        self, source: "Source", player: "Player", driver: "PulseAudioDriver"
    ) -> None:
    ...
    def _maybe_fill_audio_data_buffer(self) -> None:
        ...
        new_data = self._get_and_compensate_audio_data(
            refill_size, self._get_read_index()
        )
        ...
        if new_data is None:
            ...
            self.append_events(
                self._audio_data_buffer.virtual_write_index, new_data.events
            )
