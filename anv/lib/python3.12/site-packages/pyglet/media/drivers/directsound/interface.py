...
def _create_primary_buffer_desc():
    ...
    buffer_desc.dwFlags = (
        lib.DSBCAPS_CTRL3D | lib.DSBCAPS_CTRLVOLUME | lib.DSBCAPS_PRIMARYBUFFER
    )
...
class DirectSoundDriver:
    ...
    def create_buffer(self, audio_format, buffer_size):
        ...
        return DirectSoundBuffer(
            self._create_native_buffer(buffer_desc), audio_format, buffer_size
        )
    ...
    def _create_primary_buffer(self):
        return DirectSoundBuffer(
            self._create_native_buffer(_create_primary_buffer_desc()), None, 0
        )
...
class DirectSoundBuffer:
    def __init__(self, native_buffer, audio_format, buffer_size):
        ...
        if audio_format is not None and audio_format.channels == 1:
            ...
            self._native_buffer.QueryInterface(
                lib.IID_IDirectSound3DBuffer, ctypes.byref(self._native_buffer3d)
            )
        else:
    ...
    def cone_angles(self):
        ...
        if self.is3d:
            ...
            outside = lib.DWORD()
            _check(
                self._native_buffer3d.GetConeAngles(
                    ctypes.byref(inside), ctypes.byref(outside)
                )
            )
    ...
    def create_listener(self):
        ...
        self._native_buffer.QueryInterface(
            lib.IID_IDirectSound3DListener, ctypes.byref(native_listener)
        )
...
class DirectSoundListener:
    ...
    def orientation(self, orientation):
        _check(
            self._native_listener.SetOrientation(
                *(list(orientation) + [lib.DS3D_IMMEDIATE])
            )
        )
