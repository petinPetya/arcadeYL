...
class DirectSoundDriver(AbstractAudioDriver):
    def __init__(self):
        ...
        self._listener = DirectSoundListener(
            self._ds_listener, self._ds_driver.primary_buffer
        )
...
class DirectSoundListener(AbstractListener):
    ...
    def _set_orientation(self):
        self._ds_listener.orientation = _convert_coordinates(
...
class DirectSoundAudioPlayer(AbstractAudioPlayer):
    def __init__(self, driver, source, player):
        ...
        self._ds_buffer = self.driver._ds_driver.create_buffer(
            source.audio_format, self._buffer_size
        )
    ...
    def work(self):
        ...
        # filled.
        if (
            used := self._get_used_buffer_space()
    ...
    def _refill(self, size):
        ...
        if audio_data is None:
            ...
            assert _debug(
                f"DirectSoundAudioPlayer: Got {audio_data.length} bytes of audio data"
            )
    ...
    def _write(self, audio_data, region_size):
        ...
        assert _debug(
            f"Writing {region_size}B ({audio_size}B data, {region_size - audio_size}B silence)"
        )
        ...
        if audio_size < a1_size:
            ...
            ctypes.memset(
                write_ptr.audio_ptr_1.value + audio_size, s, a1_size - audio_size
            )
        else:
            if a1_size > 0:
        ...
        if write_ptr.audio_ptr_2 and a2_silence > 0:
            ctypes.memset(
                write_ptr.audio_ptr_2.value + (a2_size - a2_silence), s, a2_silence
            )
