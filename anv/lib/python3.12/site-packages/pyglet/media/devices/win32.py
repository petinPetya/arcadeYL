...
CLSID_MMDeviceEnumerator = com.GUID(
    0xBCDE0395, 0xE52F, 0x467C, 0x8E, 0x3D, 0xC4, 0x57, 0x92, 0x91, 0x69, 0x2E
)
IID_IMMDeviceEnumerator = com.GUID(
...
class IMMNotificationClient(com.IUnknown):
    _methods_ = [
        ("OnDeviceStateChanged", com.STDMETHOD(LPCWSTR, DWORD)),
        ("OnDeviceAdded", com.STDMETHOD(LPCWSTR)),
        ("OnDeviceRemoved", com.STDMETHOD(LPCWSTR)),
        ("OnDefaultDeviceChanged", com.STDMETHOD(EDataFlow, ERole, LPCWSTR)),
        ("OnPropertyValueChanged", com.STDMETHOD(LPCWSTR, PROPERTYKEY)),
    ]


IID_IMMEndpoint = com.GUID(
...
class IMMEndpoint(com.pIUnknown):
    _methods_ = [
        ("GetDataFlow", com.STDMETHOD(POINTER(EDataFlow))),
    ]
...
class AudioNotificationCB(com.COMObject):
    ...
    def OnDeviceStateChanged(self, pwstrDeviceId, dwNewState):
        ...
        self.audio_devices.dispatch_event(
            "on_device_state_changed", device, pyglet_old_state, pyglet_new_state
        )
...
class Win32AudioDeviceManager(base.AbstractAudioDeviceManager):
    ...
    def get_output_devices(self, state=DEVICE_STATE_ACTIVE) -> List[Win32AudioDevice]:
        return [
            device
            for device in self.devices
            if device.state == state and device.flow == eRender
        ]
    ...
    def get_input_devices(self, state=DEVICE_STATE_ACTIVE) -> List[Win32AudioDevice]:
        return [
            device
            for device in self.devices
            if device.state == state and device.flow == eCapture
        ]
    ...
    def _query_all_devices(self) -> List[Win32AudioDevice]:
        return self.get_devices(
            flow=eRender, state=DEVICE_STATEMASK_ALL
        ) + self.get_devices(flow=eCapture, state=DEVICE_STATEMASK_ALL)
