...
class _MessageHandler:
    ...
    def notify_caps(self, pad, *args):
        ...
        self.source._duration = (
            pad.get_peer().query_duration(Gst.Format.TIME).duration / Gst.SECOND
        )
        ...
        self.source.audio_format = AudioFormat(
            channels=channels, sample_size=sample_size, sample_rate=sample_rate
        )
...
class GStreamerSource(StreamingSource):
    ...
    def __init__(self, filename, file=None):
        ...
        self.appsink.set_property(
            "caps", Gst.Caps.from_string("audio/x-raw,format=S16LE,layout=interleaved")
        )
        ...
        self.caps_handler = self.appsink.get_static_pad("sink").connect(
            "notify::caps", msg_handler.notify_caps
        )
