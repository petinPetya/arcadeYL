...
if _debug:
    if (
        ...
        and not pyogg.PYOGG_VORBIS_AVAIL
        and not pyogg.PYOGG_VORBIS_FILE_AVAIL
    ):
...
if (
    ...
    and (not pyogg.PYOGG_OPUS_AVAIL and not pyogg.PYOGG_OPUS_FILE_AVAIL)
    and not pyogg.PYOGG_FLAC_AVAIL
):
...
if pyogg.PYOGG_FLAC_AVAIL:
    ...
    FLAC__StreamDecoderEofCallback = CFUNCTYPE(
        c_int, POINTER(pyogg.flac.FLAC__StreamDecoder), c_void_p
    )

    # Override explicits with c_void_p, so we can support non-seeking FLAC's (CFUNCTYPE does not accept None).
    pyogg.flac.libflac.FLAC__stream_decoder_init_stream.restype = (
        pyogg.flac.FLAC__StreamDecoderInitStatus
    )
    pyogg.flac.libflac.FLAC__stream_decoder_init_stream.argtypes = [
        POINTER(pyogg.flac.FLAC__StreamDecoder),
    ...
    def metadata_callback(self, decoder, metadata, client_data):
        self.bits_per_sample = (
            metadata.contents.data.stream_info.bits_per_sample
        )  # missing from pyogg
...
class MemoryVorbisObject:
    def __init__(self, file):
        ...
        self.callbacks = pyogg.vorbis.ov_callbacks(
            self.read_func, self.seek_func, self.close_func, self.tell_func
        )
...
class MemoryOpusObject:
    def __init__(self, filename, file):
        ...
        self.callbacks = pyogg.opus.OpusFileCallbacks(
            self.read_func, self.seek_func, self.tell_func, self.close_func
        )
    ...
    def __init__(self, filename, file):
        ...
        if error.value != 0:
            raise DecodeException(
                "file-like object: {} couldn't be processed. Error code : {}".format(
                    filename, error.value
                )
            )
        ...
        self.bfarr_t = pyogg.opus.opus_int16 * (
            pyogg.PYOGG_STREAM_BUFFER_SIZE * self.channels * 2
        )
...
class MemoryVorbisFileStream(UnclosedVorbisFileStream):
    def __init__(self, path, file):
        ...
        error = pyogg.vorbis.libvorbisfile.ov_open_callbacks(
            buff, self.vf, None, 0, self.memory_object.callbacks
        )
        if error != 0:
            raise DecodeException(
                "file couldn't be opened or doesn't exist. Error code : {}".format(
                    error
                )
            )
...
class MemoryFLACFileStream(UnclosedFLACFileStream):
    def __init__(self, path, file):
        ...
        self.write_callback_ = pyogg.flac.FLAC__StreamDecoderWriteCallback(
            self.write_callback
        )
        self.metadata_callback_ = pyogg.flac.FLAC__StreamDecoderMetadataCallback(
            self.metadata_callback
        )
        self.error_callback_ = pyogg.flac.FLAC__StreamDecoderErrorCallback(
            self.error_callback
        )
        self.read_callback_ = pyogg.flac.FLAC__StreamDecoderReadCallback(
            self.read_callback
        )

        if self.seekable:
            self.seek_callback_ = pyogg.flac.FLAC__StreamDecoderSeekCallback(
                self.seek_callback
            )
            self.tell_callback_ = pyogg.flac.FLAC__StreamDecoderTellCallback(
                self.tell_callback
            )
            self.length_callback_ = pyogg.flac.FLAC__StreamDecoderLengthCallback(
        ...
        if init_status:  # error
            raise DecodeException(
                "An error occurred when trying to open '{}': {}".format(
                    path, pyogg.flac.FLAC__StreamDecoderInitStatusEnum[init_status]
                )
            )

        metadata_status = pyogg.flac.FLAC__stream_decoder_process_until_end_of_metadata(
            self.decoder
        )
        if not metadata_status:  # error
            raise DecodeException(
                "An error occurred when trying to decode the metadata of {}".format(
                    path
                )
            )
...
class PyOggFLACSource(PyOggSource):
    ...
    def _load_source(self):
        ...
        if self._stream.total_samples == 0:
            if _debug:
                warnings.warn(
                    f"Unknown amount of samples found in {self.filename}. Seeking may be limited."
                )
    ...
    def seek(self, timestamp):
        if self._stream.seekable:
            ...
            if self._duration_per_frame:
                ...
                position = 0
            seek_succeeded = pyogg.flac.FLAC__stream_decoder_seek_absolute(
                self._stream.decoder, position
            )
...
class PyOggVorbisSource(PyOggSource):
    ...
    def _load_source(self):
        ...
        self._duration = pyogg.vorbis.libvorbisfile.ov_time_total(
            byref(self._stream.vf), -1
        )
...
class PyOggDecoder(MediaDecoder):
    vorbis_exts = (
        (".ogg",)
        if pyogg.PYOGG_OGG_AVAIL
        and pyogg.PYOGG_VORBIS_AVAIL
        and pyogg.PYOGG_VORBIS_FILE_AVAIL
        else ()
    )
    flac_exts = (".flac",) if pyogg.PYOGG_FLAC_AVAIL else ()
    opus_exts = (
        (".opus",) if pyogg.PYOGG_OPUS_AVAIL and pyogg.PYOGG_OPUS_FILE_AVAIL else ()
    )
