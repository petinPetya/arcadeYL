...
def ffmpeg_open_memory_file(
    filename: bytes, file_object: BinaryIO
) -> tuple[FFmpegFile, MemoryFileObject]:
    ...
    if result != 0:
        raise FFmpegException(
            "avformat_open_input in ffmpeg_open_filename returned an error opening file "
...
def ffmpeg_file_info(file: FFmpegFile) -> FileInfo:
    ...
    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("title"), None, 0
    )
    if entry:
        info.title = asstr(entry.contents.value)

    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("artist"), None, 0
    ) or avutil.av_dict_get(
        file.context.contents.metadata, asbytes("album_artist"), None, 0
    )
    if entry:
        info.author = asstr(entry.contents.value)

    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("copyright"), None, 0
    )
    if entry:
        info.copyright = asstr(entry.contents.value)

    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("comment"), None, 0
    )
    if entry:
        info.comment = asstr(entry.contents.value)

    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("album"), None, 0
    )
    if entry:
    ...
    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("track"), None, 0
    )
    if entry:
        info.track = asstr(entry.contents.value)

    entry = avutil.av_dict_get(
        file.context.contents.metadata, asbytes("genre"), None, 0
    )
    if entry:
...
def ffmpeg_stream_info(
    file: FFmpegFile, stream_index: int
) -> StreamAudioInfo | StreamVideoInfo | None:
    ...
    if context.codec_type == AVMEDIA_TYPE_VIDEO:
        if _debug:
            ...
            print(
                " codec name=",
                avcodec.avcodec_get_name(context.codec_id).decode("utf-8"),
            )
...
def ffmpeg_open_stream(file: FFmpegFile, index: int) -> FFmpegStream:
    if not 0 <= index < file.context.contents.nb_streams:
        raise FFmpegException(
            f"index out of range. Only {file.context.contents.nb_streams} streams."
        )
    ...
    result = avcodec.avcodec_parameters_to_context(
        codec_context, file.context.contents.streams[index].contents.codecpar
    )
    if result < 0:
    ...
    if not codec:
        raise FFmpegException("No codec found for this media. " f"codecID={codec_id}")
...
def ffmpeg_seek_file(file: FFmpegFile, timestamp: float) -> None:
    ...
    if result < 0:
        ...
        descr = buf.value
        raise FFmpegException("Error occurred while seeking. " + descr.decode())
...
class FFmpegSource(StreamingSource):
    ...
    def __init__(self, filename: str, file: BinaryIO | None = None):
            ...
            if isinstance(info, StreamVideoInfo) and self._video_stream is None:
                ...
                if info.sample_aspect_num != 0:
                    self.video_format.sample_aspect = (
                        float(info.sample_aspect_num) / info.sample_aspect_den
                    )
                self.video_format.frame_rate = (
                    float(info.frame_rate_num) / info.frame_rate_den
                )
                ...
                self.audio_convert_ctx = self.get_formatted_swr_context(
                    channel_output, info.sample_rate, channel_input, info.sample_format
                )
        ...
        self._max_len_audioq = (
            self.MAX_QUEUE_SIZE
        )  # Need to figure out a correct amount
    ...
    def get_formatted_swr_context(
        self,
        channel_output: AVChannelLayout | int,
        sample_rate: int,
    ...
    def _fillq(self) -> None:
        ...
        self._fillq_scheduled = False
        while (
            len(self.audioq) < self._max_len_audioq
            and len(self.videoq) < self._max_len_videoq
        ):
            if self._get_packet():
    ...
    def _process_packet(self) -> AudioPacket | VideoPacket | None:
        ...
        if (
            self.audio_format
            and self._packet.contents.stream_index == self._audio_stream_index
        ):
    ...
    def get_audio_data(
        ...
        while len(data) < num_bytes:
            ...
            buffer, timestamp, duration = self._decode_audio_packet(
                audio_packet, compensation_time
            )
        ...
        if _debug:
            print(
                f"get_audio_data returning ts {audio_data.timestamp} with events {audio_data.events}"
            )
    ...
    def _decode_audio_packet(
        self, audio_packet: AudioPacket, compensation_time: float
    ) -> tuple[bytes | None, float, float]:
        while True:
            try:
                size_out = self._ffmpeg_decode_audio(
                    audio_packet.packet, self._audio_buffer, compensation_time
                )
    ...
    def _ffmpeg_decode_audio(
        self, packet: AVPacket, data_out: Array[c_uint8], compensation_time: float
    ) -> int:
        ...
        if sent_result < 0:
            ...
            descr = buf.value
            raise FFmpegException(
                f"Error occurred sending packet to decoder. {descr.decode()}"
            )
        ...
        total_samples_out = swresample.swr_convert(
            self.audio_convert_ctx, byref(p_data_out), out_samples, data_in, nb_samples
        )
        while True:
            ...
            samples_out = swresample.swr_convert(
                self.audio_convert_ctx,
                byref(p_data_offset),
                out_samples - total_samples_out,
                None,
                0,
            )
            if samples_out == 0:
    ...
    def _decode_video_packet(self, video_packet: VideoPacket) -> None:
        ...
        pitch = width * 4
        buf_size = (
            avutil.av_image_get_buffer_size(AV_PIX_FMT_RGBA, width, height, 1)
            + AV_INPUT_BUFFER_PADDING_SIZE
        )
        ...
        if _debug:
            print(
                "Decoding video packet at timestamp",
                video_packet,
    ...
    def _ffmpeg_decode_video(self, packet: AVPacket, data_out: Array[c_uint8]) -> int:
        ...
        if sent_result < 0:
            ...
            descr = buf.value
            raise FFmpegException(
                f"Video: Error occurred sending packet to decoder. {descr.decode()}"
            )
        ...
        if receive_result < 0:
            ...
            descr = buf.value
            raise FFmpegException(
                f"Video: Error occurred receiving frame. {descr.decode()}"
            )
        ...
        avutil.av_image_fill_arrays(
            rgba_ptrs, rgba_stride, data_out, AV_PIX_FMT_RGBA, width, height, 1
        )
        ...
        self.img_convert_ctx = swscale.sws_getCachedContext(
            ...
            width,
            height,
            stream.codec_context.contents.pix_fmt,
            width,
            height,
            AV_PIX_FMT_RGBA,
            SWS_FAST_BILINEAR,
        ...
        swscale.sws_scale(
            self.img_convert_ctx,
            cast(stream.frame.contents.data, POINTER(POINTER(c_uint8))),
        ...
        def start_times(streams) -> Iterator[int]:
            ...
            for stream in streams:
                ...
                if start == AV_NOPTS_VALUE:
                    yield 0
                start_time = avutil.av_rescale_q(
                    start, stream.time_base, AV_TIME_BASE_Q
                )
...
class FFmpegDecoder(MediaDecoder):
    ...
    def decode(
        self, filename: str, file: BinaryIO | None, streaming: bool = True
