...
compat.add_version_changes(
    "avformat", 58, AVStream, AVStream_Fields, removals=("av_class",)
)
...
class AVIOInterruptCB(Structure):
    _fields_ = [("callback", CFUNCTYPE(c_int, c_void_p)), ("opaque", c_void_p)]
...
if avformat_version >= 61:
    AVFormatContext_Fields = [
        ...
        (
            "control_message_cb",
            CFUNCTYPE(c_int, POINTER(AVFormatContext), c_int, c_void_p, c_size_t),
        ),
        ...
        (
            "io_close2",
            CFUNCTYPE(c_int, POINTER(AVFormatContext), POINTER(AVIOContext)),
        ),  # Added in 59.
    ]
    ...
    AVFormatContext_Fields = [
        ...
        ("io_close", CFUNCTYPE(None, POINTER(AVFormatContext), POINTER(AVIOContext))),
        ("protocol_blacklist", c_char_p),
        ("max_streams", c_int),
        ("skip_estimate_duration_from_pts", c_int),  # Added in 59.
...
avformat.av_seek_frame.argtypes = [POINTER(AVFormatContext), c_int, c_int64, c_int]
avformat.avformat_seek_file.restype = c_int
avformat.avformat_seek_file.argtypes = [
    POINTER(AVFormatContext),
    c_int,
    c_int64,
    c_int64,
...
avformat.av_guess_frame_rate.argtypes = [
    POINTER(AVFormatContext),
    POINTER(AVStream),
    POINTER(AVFrame),
]
...
avformat.avio_alloc_context.argtypes = [
    c_char_p,
    c_int,
    c_int,
    c_void_p,
