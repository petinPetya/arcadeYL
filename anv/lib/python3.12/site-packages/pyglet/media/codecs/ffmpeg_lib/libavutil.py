...
class AVChannelCustom(Structure):
    _fields_ = [("id", AVChannel), ("name", c_char * 16), ("opaque", c_void_p)]
...
AVFrame_Fields = [
    ...
    (
        "interlaced_frame",
        c_int,
    ),  # deprecated in 59. Targeted for removal. Use AV_FRAME_FLAG_INTERLACED (removed in 60)
    (
        "top_field_first",
        c_int,
    ),  # deprecated in 59. Targeted for removal. Use AV_FRAME_FLAG_TOP_FIELD_FIRST (removed in 60)
    (
        "palette_has_changed",
        c_int,
    ),  # deprecated in 59. Targeted for removal. (removed in 60)
    ...
    (
        "pkt_size",
        c_int,
    ),  # deprecated in 59.  use AV_CODEC_FLAG_COPY_OPAQUE to pass through arbitrary user data from packets to frames
...
avutil.av_dict_get.argtypes = [
    POINTER(AVDictionary),
    c_char_p,
...
avutil.av_samples_get_buffer_size.argtypes = [POINTER(c_int), c_int, c_int, c_int]
...
avutil.av_image_fill_arrays.argtypes = [
    POINTER(c_uint8) * 4,
    c_int * 4,
    POINTER(c_uint8),
    c_int,
