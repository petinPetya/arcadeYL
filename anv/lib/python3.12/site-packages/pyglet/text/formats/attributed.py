...
class AttributedTextDecoder(pyglet.text.DocumentDecoder):  # noqa: D101
    ...
    def decode(
        self, text: str, location: Location | None = None
    ) -> pyglet.text.document.FormattedDocument:  # noqa: ARG002
            ...
            if group == "text":
                ...
                if name[0] == ".":
                    if trailing_newline:
                        ...
                        self.doc.set_paragraph_style(
                            self.length, self.length, {name[1:]: value}
                        )
                else:
