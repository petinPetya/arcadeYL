...
def get_default_layout_shader() -> ShaderProgram:
    ...
    return pyglet.gl.current_context.create_program(
        (layout_vertex_source, "vertex"), (layout_fragment_source, "fragment")
    )
...
def get_default_image_layout_shader() -> ShaderProgram:
    ...
    return pyglet.gl.current_context.create_program(
        (layout_vertex_source, "vertex"), (layout_fragment_image_source, "fragment")
    )
...
def get_default_decoration_shader() -> ShaderProgram:
    ...
    return pyglet.gl.current_context.create_program(
        (decoration_vertex_source, "vertex"), (decoration_fragment_source, "fragment")
    )
...
class _AbstractBox(ABC):
    ...
    def __init__(
        self, ascent: float, descent: float, advance: float, length: int
    ) -> None:
        self.owner = None
        self.ascent = ascent
...
class _GlyphBox(_AbstractBox):
    ...
    def _add_vertex_list(
        self, vertex_list: _LayoutVertexList | VertexList, context: _LayoutContext
    ) -> None:
        self.vertex_lists.append(vertex_list)
    ...
    def place(
        ...
        try:
            ...
            group = layout.group_class(
                self.owner, layout.program, order=1, parent=layout.group
            )
        ...
        for start, end, baseline_ in context.baseline_iter.ranges(i, i + n_glyphs):
            ...
            for kern, glyph, glyph_pos in self.glyphs[start - i : end - i]:
                ...
                vertices.extend(
                    map(round, [v0, v1, 0, v2, v1, 0, v2, v3, 0, v0, v3, 0])
                )
                t = glyph.tex_coords
                tex_coords.extend(t)
        ...
        for glyph_idx in range(n_glyphs):
            indices.extend(
                [element + (glyph_idx * 4) for element in [0, 1, 2, 0, 2, 3]]
            )
        ...
        for start, end, decoration in context.decoration_iter.ranges(i, i + n_glyphs):
            ...
            if underline is not None:
                ...
                underline_vertices.extend(
                    [x1, line_y + baseline - 2, 0, x2, line_y + baseline - 2, 0]
                )
        ...
        if background_vertices:
            ...
            background_indices = [
                (0, 1, 2, 0, 2, 3)[i % 6] for i in range(bg_count * 3)
            ]
    ...
    def update_colors(self, colors: list[int], start: int, end: int) -> None:
        ...
        for _vertex_list in self.vertex_lists:
            ...
            if vertices_per_char == 4:
                ...
                _vertex_list.colors[vertex_start_index:vertex_end_index] = (
                    colors[:color_end_index] * vertices_per_char
                )
...
class _InlineElementBox(_AbstractBox):
    ...
    def place(
        ...
        self.element.place(
            layout, x, y, z, line_x, line_y, rotation, visible, anchor_x, anchor_y
        )
        self.placed = True
...
class TextLayoutGroup(graphics.Group):
    ...
    def __init__(
        ...
        parent: graphics.Group | None = None,
    ) -> None:
    ...
    def __eq__(self, other: object) -> bool:
        return (
            ...
            and self.program.id is other.program.id
...
class TextLayout:
    ...
    def _initialize_groups(self) -> None:
        ...
        self.background_decoration_group = self.decoration_class(
            decoration_shader, order=0, parent=self._user_group
        )
        self.foreground_decoration_group = self.decoration_class(
            decoration_shader, order=2, parent=self._user_group
        )
    ...
    def get_as_texture(
        self, min_filter: int = GL_NEAREST, mag_filter: int = GL_NEAREST
    ) -> Texture:
        ...
        texture = pyglet.image.Texture.create(
            width, height, min_filter=min_filter, mag_filter=mag_filter
        )
        depth_buffer = pyglet.image.buffer.Renderbuffer(
    ...
    def _update(self) -> None:
        ...
        context = _StaticLayoutContext(
            self, self._document, colors_iter, background_iter
        )
    ...
    def _get_glyphs(
        self,
    ) -> tuple[list[_InlineElementBox | Glyph], list[tuple[int, int]]]:
        glyphs = []
        offsets = []
    ...
    def _flow_glyphs_wrap(
        ...
        for start, end, owner in owner_iterator:
            ...
            for text, glyph, offset in zip(
                self.document.text[start:end], glyphs[start:end], offsets[start:end]
            ):
                if nokern:
                    kern = 0
                    nokern = False
                ...
                if wrap != "char" and text in "\u0020\u200b\t":
                    ...
                    if (
                        wrap
                        and self._wrap_lines
                        and x + kern + glyph.advance + offset.x_advance >= width
                    ) or new_line:
                        ...
                        if line.boxes or new_line:
                            ...
                            if (
                                run_accum
                                and hasattr(run_accum, "glyphs")
                                and run_accum.glyphs
                            ):
                            ...
                            if self._wrap_lines:
                                width = (
                                    self._width - line.margin_left - line.margin_right
                                )
            ...
            if owner_accum_commit:
                line.add_box(
                    _GlyphBox(owner, font, owner_accum_commit, owner_accum_commit_width)
                )
            if owner_accum:
    ...
    def _flow_lines(self, lines: list[_Line], start: int, end: int) -> int:
        ...
        for line in lines[start:]:
            ...
            if line.align == "left" or line.width > self.width:
                ...
                line.x = (
                    self.width - line.margin_left - line.margin_right - line.width
                ) // 2 + line.margin_left
            elif line.align == "right":
                line.x = self.width - line.margin_right - line.width

            self._content_width = max(
                self._content_width, line.width + line.margin_left
            )
