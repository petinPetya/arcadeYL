...
class IncrementalTextLayout(TextLayout, EventDispatcher):
    ...
    decoration_class: ClassVar[type[IncrementalTextDecorationGroup]] = (
    ...
    def on_style_text(self, start: int, end: int, attributes: dict[str, Any]) -> None:
        if (
            "font_name" in attributes
            or "font_size" in attributes
            or "weight" in attributes
            or "italic" in attributes
        ):
    ...
    def _update(self) -> None:
        ...
        trigger_update_event = (
            self._invalid_glyphs.is_invalid()
            or self._invalid_flow.is_invalid()
            or self._invalid_lines.is_invalid()
        )
    ...
    def _update_flow_glyphs(self) -> None:
        ...
        for line in self._flow_glyphs(
            ...
            try:
                ...
                if (
                    old_line_width == self._content_width
                    and new_line_width < old_line_width
                ):
    ...
    def set_selection(self, start: int, end: int) -> None:
        ...
        if end > self._selection_start and start < self._selection_end:
            ...
            self._invalid_style.invalidate(
                min(start, self._selection_start), max(start, self._selection_start)
            )
            self._invalid_style.invalidate(
                min(end, self._selection_end), max(end, self._selection_end)
            )
        else:
    ...
    def selection_background_color(
        self, background_color: tuple[int, int, int, int]
    ) -> None:
    ...
    def get_point_from_position(
        self, position: int, line_idx: int | None = None
    ) -> tuple[float, float]:
    ...
    def ensure_x_visible(self, x: float) -> None:
        ...
        if x <= self.view_x:
            ...
            x >= self.view_x + self.width
            or (x >= self.view_x + self.width)
            and (self._content_width > self.width)
        ):
