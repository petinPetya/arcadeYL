...
def test_ipset_constructor():
    ...
    assert IPSet(IPRange("10.0.0.0", "10.0.1.31")) == IPSet(
        ["10.0.0.0/24", "10.0.1.0/27"]
    )
    assert IPSet(IPRange("0.0.0.0", "255.255.255.255")) == IPSet(["0.0.0.0/0"])
    assert IPSet([IPRange("10.0.0.0", "10.0.1.31")]) == IPSet(
        IPRange("10.0.0.0", "10.0.1.31")
    )
...
def test_ipset_unions():
    ...
    assert IPSet(["192.0.2.0"]) | IPSet(["192.0.2.1"]) | IPSet(
        ["192.0.2.3/30"]
    ) == IPSet(["192.0.2.0/30"])
    assert IPSet(["192.0.2.0"]) | IPSet(["192.0.2.1"]) | IPSet(
        ["192.0.2.3/31"]
    ) == IPSet(["192.0.2.0/30"])
    assert IPSet(["192.0.2.0/24"]) | IPSet(["192.0.3.0/24"]) | IPSet(
        ["192.0.4.0/24"]
...
def test_ipset_supersets_and_subsets():
    ...
    assert [
        tuple(map(str, (cidr, cidr[0], cidr[-1]))) for cidr in available.iter_cidrs()
    ] == [
...
def test_ipset_updates():
    ...
    s4.update(
        [IPRange("192.0.2.0", "192.0.2.100"), IPRange("192.0.2.50", "192.0.2.255")]
    )
    assert s4 == expected
...
def test_converting_ipsets_to_ipranges():
    ...
    assert list(
        IPSet([IPAddress("10.0.0.1"), IPAddress("10.0.0.2")]).iter_ipranges()
