...
def test_cidr_exclude_v4():
    ...
    assert cidr_exclude("192.0.1.255/32", "192.0.2.0/28") == [
        IPNetwork("192.0.1.255/32")
    ]


def test_cidr_merge_v4():
    assert cidr_merge(["192.0.128.0/24", "192.0.129.0/24"]) == [
...
def test_whole_network_cidr_merge_v4():
    ...
    assert cidr_merge(["0.0.0.0/0", "192.0.2.0/24", "10.0.0.0/8"]) == [
        IPNetwork("0.0.0.0/0")
    ]


def test_largest_matching_cidr_v4():
    assert largest_matching_cidr("192.0.2.0", ["192.0.2.0"]) == IPNetwork(
        "192.0.2.0/32"
    )
    assert largest_matching_cidr("192.0.2.0", ["10.0.0.1", "192.0.2.0"]) == IPNetwork(
        "192.0.2.0/32"
    )
    assert largest_matching_cidr(
        "192.0.2.0", ["10.0.0.1", "192.0.2.0", "224.0.0.1"]
