...
class IPAddress(BaseIP):
    ...
    def __init__(self, addr, version=None, flags=0):
        ...
        if isinstance(addr, BaseIP):
            ...
            if self._module is None:
                ...
                if self._module is None:
                    raise AddrFormatError(
                        "failed to detect a valid IP " "address from %r" % addr
                    )
            else:
    ...
    def netmask_bits(self):
        ...
        if not 0 <= mask_length <= self._module.width:
            raise ValueError(
                "Unexpected mask length %d for address type!" % mask_length
            )
    ...
    def is_ipv4_private_use(self):
        ...
        * ``192.168.0.0/16``
        ...
        .. versionadded:: 0.10.0
        """
        return self._module.version == 4 and any(
            self in cidr for cidr in IPV4_PRIVATE_USE
        )
...
def parse_ip_network(module, addr, flags=0, *, expand_partial=False):
    if isinstance(addr, tuple):
        ...
        if not (0 <= value <= module.max_int):
            raise AddrFormatError(
                "invalid address value for %s tuple!" % module.family_name
            )
...
class IPNetwork(BaseIP, IPListMixin):
    ...
    def __init__(self, addr, version=None, flags=0, *, expand_partial=False):
        ...
        if hasattr(addr, "_prefixlen"):
            ...
            value, prefixlen = parse_ip_network(
                _ipv4, addr, flags, expand_partial=expand_partial
            )
            module = _ipv4
        elif version == 6:
    ...
    def _set_prefixlen(self, value):
        ...
        if not 0 <= value <= self._module.width:
            raise AddrFormatError(
                "invalid prefix for an %s address!" % self._module.family_name
            )
    ...
    def ipv6(self, ipv4_compatible=False):
        ...
        if self._module.version == 6:
            ...
            if ipv4_compatible:
                ...
                #   IPv4-Mapped IPv6 address
                ip = klass(
                    (0xFFFF00000000 + self._value, self._prefixlen + 96), version=6
                )
    ...
    def previous(self, step=1):
        ...
        """
        ip_copy = self.__class__(
            "%s/%d" % (self.network, self.prefixlen), self._module.version
        )
        ip_copy -= step
        return ip_copy
    ...
    def subnet(self, prefixlen, count=None, fmt=None):
        ...
        i = 0
        while i < count:
            subnet = self.__class__(
                "%s/%d" % (base_subnet, prefixlen), self._module.version
            )
...
class IPRange(BaseIP, IPListMixin):
    ...
    def __contains__(self, other):
        if isinstance(other, BaseIP):
            ...
            if isinstance(other, IPAddress):
                return (
                    self._start._value <= other._value
                    and self._end._value >= other._value
                )
            if isinstance(other, IPRange):
                return (
            ...
            if isinstance(other, IPNetwork):
                ...
                return (
                    self._start._value <= other_start and self._end._value >= other_end
                )
        ...
        if "/" in abbrev_cidr:
            ...
            try:
                if not 0 <= int(prefix) <= 32:
                    raise ValueError(
                        "prefixlen in address %r out of range"
...
IPV4_PRIVATE_USE = [
    ...
    IPNetwork(
        "192.168.0.0/16"
    ),  #  Class B private network local communication (RFC 1918)
]
