...
class PlayerSelectView(arcade.View):
    ...
    def on_draw(self):
        ...
        skin = self.player.skins[self.selected_skin_index]
        panel_x = SCREEN_WIDTH // 2
        panel_y = SCREEN_HEIGHT // 2

        arcade.draw_lbwh_rectangle_filled(
            panel_x - 200, panel_x + 200, panel_y + 150, panel_y - 150,
            arcade.color.DARK_SLATE_BLUE
        )
        ...
        arcade.draw_circle_filled(panel_x, panel_y + 70, 60, arcade.color.LIGHT_GRAY)
        arcade.draw_circle_filled(
            panel_x, panel_y + 90, 20, arcade.color.SKY_BLUE
        )
    ...
    def prev_skin(self):
        self.selected_skin_index = (self.selected_skin_index - 1) % len(self.player.skins)
    
    def next_skin(self):
        self.selected_skin_index = (self.selected_skin_index + 1) % len(self.player.skins)
    
    def select_current(self):
        self.player.current_skin_index = self.selected_skin_index
        if hasattr(self.main_menu_view, 'current_skin_index'):
            self.main_menu_view.current_skin_index = self.selected_skin_index
        print(f"Выбран персонаж: {self.player.current_skin.name}")
        self.go_back()
    
    def upgrade_current(self):
        skin = self.player.skins[self.selected_skin_index]
        if hasattr(self.main_menu_view, 'player_money'):
            if self.main_menu_view.player_money >= skin.upgrade_cost:
                self.main_menu_view.player_money -= skin.upgrade_cost
                skin.upgrade()
                print(f"Персонаж {skin.name} улучшен до уровня {skin.level}")
            else:
                print(f"Недостаточно денег! Нужно: {skin.upgrade_cost}")
        else:
            skin.upgrade()
            print(f"Персонаж {skin.name} улучшен до уровня {skin.level}")
